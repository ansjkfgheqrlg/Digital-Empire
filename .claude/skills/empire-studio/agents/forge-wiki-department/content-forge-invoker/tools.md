...
## Strumenti che usa
1. **content-forge (/forge)** - pipeline forging -> wiki (skill esistente)
   ```
   /forge runs/<run-id>/forge-input/ --target=wiki --name <slug>
   ```
2. **content-forge-bridge-skill** - prepara input + sync memory
   ```
   python skills/tier2-functional/content-forge-bridge-skill/scripts/prepare_forge_input.py --run <run-id>
   ```
