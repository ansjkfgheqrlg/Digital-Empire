# Anthropic `skill-creator` — Mirror

> Copia della guida ufficiale Anthropic `skill-creator`, mantenuta in repo per uso **offline** da parte di `B4 skill-builder-agent`.
> Source originale fornita dall'utente in `/home/user/uploads/name skill-creator.md`.
>
> Letto come **ground truth**

> ⚠️ **Nota di lettura per `B4 skill-builder-agent`**:
> Questa guida contiene pointer tipo `agents/grader.md`, `agents/comparator.md`, `agents/analyzer.md`,
> `references/schemas.md` ecc. Quei pointer si riferiscono alla **skill `skill-creator` ufficiale**,
> NON alla nostra `content-forge`. Sono presenti solo nella sua struttura interna.
> Per `content-forge` non sono pointer da risolvere: ignora gli avvisi del cross-reference check su questi.

 dal `skill-builder-agent` per produrre skill conformi.
