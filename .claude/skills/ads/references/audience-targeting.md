...
## Content-Based Targeting (Modo Moderno — non retargeting)

Oltre al targeting classico (interessi/demografiche impostati manualmente + fonti come Pixel/customer list), esiste un secondo meccanismo, meno discusso ma verificato empiricamente: è il contenuto stesso della creativa a targetizzare. Se la creativa dichiara esplicitamente il target nell'hook (es. "Se sei un imprenditore con X dipendenti nella provincia di Y"), la piattaforma tende a distribuire l'ad alle persone che si riconoscono in quella descrizione — funziona anche a livello di puro copy, indipendentemente dai settaggi di targeting media. Non si applica al retargeting (dove l'audience è già definita da altro).

**Applicazione pratica per B2B locale — scaling tramite variabili nel hook**: invece di creare decine di concept creativi diversi, parametrizzare un unico hook efficace sostituendo variabili demografiche/geografiche (settore, numero dipendenti, provincia/regione) per generare molte versioni ad alta specificità percepita a basso costo di produzione.

*(Fonte: caso studio singolo — Andrei Pascu, consulenza reale settore fotovoltaico, 2026-08-26.)*
