---
agent_id: A-ATTENTION_
name: attention-writer
family: apsoc
...
> Agente specialista dentro `preventivi-workflow`. Originale spec: # attention-writer
Sub-agent APSOC.
...
Dettagliato in `attention-writer.system_prompt.md`. Sommario: ricevo input → applico mia logica specialistica → produco output structured → handoff al next agent.
...
Vedi `attention-writer.tools.md`.
...
Vedi `attention-writer.failure_modes.md`.
