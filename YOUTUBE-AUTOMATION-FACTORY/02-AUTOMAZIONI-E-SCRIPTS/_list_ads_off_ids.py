...
with sync_playwright() as p:
    ...
    for i in range(n):
        ...
        try:
            ...
            continue
        if "Ads off" in txt:
            href = row.locator("a#video-title, a[href*='/video/']").first
            link = href.get_attribute("href") if href.count() > 0 else None
            title_el = row.locator("#video-title, .ytcp-video-row .title").first
