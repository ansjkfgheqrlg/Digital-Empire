...
with sync_playwright() as p:
    ...
    none_link = page.get_by_text("None of the above", exact=False)
    print("none_link count:", none_link.count())
    if none_link.count() > 0:
        none_link.first.scroll_into_view_if_needed()
        page.wait_for_timeout(500)
        page.screenshot(path="../memory/_q_none_visible.png")
        none_link.first.click(timeout=8000)
        page.wait_for_timeout(1000)
        print("cliccato None of the above")
    page.screenshot(path="../memory/_q_after_none.png")
    ctx.close()
