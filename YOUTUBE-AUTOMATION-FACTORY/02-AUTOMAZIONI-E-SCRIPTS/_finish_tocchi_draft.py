...
with sync_playwright() as p:
    ...
    page.screenshot(path="../memory/_content_list_now.png")

    rows = page.locator("ytcp-video-row").filter(has_text="7 Tocchi Che Fanno Innamorare")
    n = rows.count()
    print(f"Righe '7 Tocchi' trovate: {n}")
    for i in range(n):
        row = rows.nth(i)
        txt = row.inner_text(timeout=5000)
        print(f"--- riga {i}: {txt[:150].replace(chr(10),' | ')}")
