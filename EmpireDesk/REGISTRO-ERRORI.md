...
| **EDE-6** (superato da EDE-8, storico) | I 2 bottoni header ("Pannelli"+"Selftest") erano posizionati con `right:Npx` calcolato a mano su elementi `position:absolute` indipendenti → a font/finestra diversi rischiavano di sovrapporsi visivamente (difetto grafico silenzioso, nessun errore in console) | Calcolo manuale dei pixel invece di un layout che si adatta da solo | Header convertito a `display:flex` (`.htext`+`.hactions`) — **poi l'intera UI è stata sostituita dal redesign di Max (EDE-8)**, che risolve lo stesso problema con `.head-row` flex; `.htext`/`.hactions` non esistono più nel file | Mai posizionare più elementi UI con `position:absolute` + offset calcolati a mano quando un contenitore flex risolve lo stesso layout senza rischio |
| **EDE-7** (trovato in autorevisione, prima di qualsiasi lancio — B1, ANCORA VALIDO) | Una tile fornita da un modulo con schema sbagliato (es. manca `script`/`cwd`, o `kind` sconosciuto) avrebbe fatto **KeyError** in `TileManager._resolve_check`/`_build_argv` al primo `list_tiles()`/selftest — un modulo scritto male avrebbe fatto cadere la lettura di TUTTE le tile, incluse le 8 core | `_load_modules()` metteva la tile del modulo in `_MODULE_TILES` senza validarne lo schema prima | `_validate_module_tile()` verifica campi obbligatori (id/icon/name/desc/kind + script+cwd o path) e collisioni di `id` con tile già esistenti PRIMA di accettarla; se invalida, va in `_MODULE_LOAD_ERRORS` (visibile nel selftest) e le routes/panel dello stesso modulo restano comunque montati (`app.py`, non toccato dal redesign UI) | "Un modulo rotto non deve mai far cadere l'app" vale per OGNI pezzo che un modulo può fornire (tile, routes, panel) — vanno validati e isolati singolarmente, non solo l'import del file |
| **EDE-8** (collisione reale — sessioni parallele Gael+Max su `ui/index.html`) | Stessa sera, in parallelo: io (Gael) ho costruito uno switcher pannelli in `ui/index.html` con endpoint `/api/panels` (`{"panels":[{id,html}]}`); Max ha costruito UN ALTRO switcher (nav a tab, redesign "Empire Premium" completo) aspettandosi `/api/modules` (`{"modules":[{id,tile,panel_html}]}`) — 2 design della STESSA feature con contratti di rete diversi, scoperto solo al `git pull --rebase` (8 blocchi in conflitto su `ui/index.html`) | Nessuno dei due sapeva dell'altro: il dossier 17 §5.1 assegnava `ui/index.html` a Gael, ma Max lo ha ripreso in autonomia la stessa sera (poi formalizzato in STATO-EMPIRE come cambio ownership) | Merge manuale hunk-per-hunk: tenuto il design visivo di Max (più ricco, nav-tab unificata) + adattato `app.py` al SUO contratto (`modules_public()`, route `api/modules`, chiave `panel_html`) invece di imporre il mio — è lui il nuovo owner UI, il backend si adegua alla UI, non viceversa. Zero perdita di lavoro da nessuna delle due parti | Su un task "focus totale, priorità assoluta" con 2 sessioni Claude attive (Gael+Max) sullo STESSO file, il rischio di collisione è ALTO anche con una regola di ownership scritta — leggere `STATO-EMPIRE.md` PRIMA di ogni sessione di lavoro (non solo all'inizio task) e ripetere `git pull` appena prima di editare un file condiviso, non solo a inizio sessione |
