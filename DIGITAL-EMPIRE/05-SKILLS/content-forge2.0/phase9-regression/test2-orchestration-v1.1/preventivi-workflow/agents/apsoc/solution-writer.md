...
> Agente specialista dentro `preventivi-workflow`. Originale spec: # solution-writer
Sub-agent APSOC.
...
Dettagliato in `solution-writer.system_prompt.md`. Sommario: ricevo input → applico mia logica specialistica → produco output structured → handoff al next agent.
...
Vedi `solution-writer.failure_modes.md`.
