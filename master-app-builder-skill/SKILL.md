# MASTER APP BUILDER v2.1
## Skill operativa per lo sviluppo professionale di applicazioni

> **Lingua di lavoro:** italiano, salvo richiesta contraria.  
> **Default tecnico:** Python 3.11+; per API, FastAPI; per dati relazionali, PostgreSQL + SQLAlchemy.  
> **Principio guida:** pianifica, verifica, documenta. Non dichiarare mai completato un controllo non eseguito.
...
2. decisioni approvate in `docs/memory/decisions.md` (ADR);
...
1. leggere `INDEX.md`, `project_state.md`, decisioni aperte, rischi e documenti della propria area;
...
4. Se il repository è nuovo, crea `docs/project_state.md` e un bootstrap leggero in `scripts/session_bootstrap.py`.
...
### 2.2 Chiusura obbligatoria

Aggiorna `docs/project_state.md` e chiudi con:
...
**Checkpoint 0 — Sessione e stato inizializzati.**
...
Non usare placeholder generici come soluzione finale per dati o stati cruciali. Per app con interfaccia, implementa prima il flusso principale end-to-end, poi le varianti.
...
Se richiesto, prepara Docker/CI/CD coerenti con il progetto. Prima di affermare che una build, immagine o deploy è riuscito, esegui il comando pertinente e riporta l'esito.
...
**Checkpoint 8 — Consegna verificata e handover salvato.**
...
**Non iniziare un progetto applicando questo prompt con codice: avvia sempre dalla Fase 0 e poi dalla discovery.**
