...
### AI Orchestration
- [[sources/Source_Funnel_Operator_Sito_2026|funneloperator.it — il sito nuovo di Andrei Pascu (studio 13/09)]] — 4 rapporti + sintesi (3.174 righe) sul sito che Max indica come il suo migliore: volto all'8,5%, acquisto al 64%, colonna 832, immagini 2× mai ritagliate, design system `fo-`. Da qui **CLAUDE-SITI §14 e §15** (ADR-031), 14 pattern candidati, B-083…B-088.
- [[tools/Tool_Fabbrica_Siti|Fabbrica Siti — legge (13 articoli), canone, due corsie, gate]] — Il sistema unico per ogni sito/landing dell'Impero (`.claude/skills/fabbrica-siti/`, dal 2026-09-06). `CLAUDE-SITI.md` vince sul gusto e si cambia solo via ADR; §13 (ADR-030, 2026-09-12): **un sito online si può solo aggiungere**, con gate meccanico `gate_solo_aggiunte.py` (testo del live ⊂ testo della build). 19 pattern, stampi `precassa.py` e `og_stampo.py`, gate `gate_fatti`/`gate_voce`/`gate_siti`. Cantiere vivo: il sito dell'agenzia (`agency-empire-landing`).
...
- [[tools/Tool_Tesoreria_Digital_Empire|TESORERIA — il reparto che conta i soldi]] — 14° ecosistema (ADR-020, 2026-09-03), nato dal fatto misurato lo stesso giorno: DE non contava un solo euro. `scripts/tesoreria.py` + 5 agenti (`tesoreria-conductor/-entrate/-spese/-report/-previsione`) + due JSONL ad accodamento. Tre leggi: previsto non è incassato mai, un numero che non esiste si dichiara non si stima, la storia si annota non si riscrive. Registro parte vuoto per scelta esplicita — i mesi prima del 2026-09-03 restano vuoti.
- [[tools/Tool_Lanci_Digital_Empire|LANCI - l'ecosistema che porta un prodotto da pubblicato a venduto]] - 15° ecosistema (ADR-025, firmato da Max l'08/09/2026, cartella creata il 10/09). Anello mancante fra ULTIMO METRO (finito→pubblicato) e TESORERIA (l'euro nei conti). Tredici artefatti tipizzati, ognuno con schema, produttore unico e **giudice diverso dal produttore** (INV-01); `lan-gate` non ha `Write` né `Edit` (INV-09). Macchina a stati, non motore di orchestrazione (decisione 5, motivata sul codice). Tetto 15 $ per lancio. **Stato: S0 aperto, S1 compilato e ha bocciato il primo lancio** — pubblico verificato 0, ricavo atteso 0 €, certificato non-consegnabile.
...
- [[sources/Source_Simone_Rizzo_Loop_Engineering|Simone Rizzo — Se usi ancora i prompt... devi vedere questa evoluzione (Loop Engineering) (batch max17, v07)]] — Video BSUHmVcaO1g, 31m23s, **176/942 frame letti (18,7%), 133/224 scene (59,4%)**, NO-FINTO PASS con copertura parziale dichiarata (nessun capitolo scoperto). Pila a quattro livelli **Prompt → Context → Harness → Loop Engineering**, ognuno nato per risolvere il fallimento del precedente. Il pezzo di maggior valore: i **5 Livelli di Verifica** (Deterministico / Regole-vincoli / Verità terrena ritardata / LLM giudice / Checkpoint umano — "conosci quale il tuo gate usa davvero"), e la sintassi esatta di `/loop [interval] [prompt]` e `/goal [<condition>|clear]` con le due regole operative (condizione di terminazione obbligatoria dopo `|`, goal valutabile dall'agente stesso). Demo reale: script di prodotto fra matrici ottimizzato in 10 tentativi tracciati (`OPTIMIZATION_LOG.md`), 320x speedup via GPU tensor core. 71 KA. **0 patch applicate** — 2 proposte in "Consigli" (non costruite in questa sessione: `.claude/agents/guild-prompt.md`, `.claude/skills/prompt-engegniring-skill/SKILL.md`), coerente col perimetro del checkpoint `EMP-QQ2R` (nessuna modifica a skill/agenti condivisi mentre altre sentinelle lavoravano in parallelo).
- [[sources/Source_Jack_Roberts_7_Claude_Design_Skills|Jack Roberts — Insane Claude Design Skills You Actually Need To Build Beautiful Sites (batch max17, v11)]] — Video pUu4G2lINnk, 22m56, **108/270 frame citati per numero (40,0%)**, NO-FINTO PASS con copertura parziale dichiarata **e una discrepanza di conteggio dichiarata, non corretta silenziosamente** (l'intestazione originale diceva 182/270, la verifica Stage 5 ha tracciato 108/270 — dettaglio in `coverage.md`). Sette skill (Reference → Sitemap → Hero → Mobile → Copy → Dettagli UI → SEO) con strumento+prompt verbatim per ciascuna. Il pezzo di maggior valore: l'invariante del **Design Loop** ("un critico che condivide la memoria del costruttore giudica i suoi stessi compiti" — adattamento dichiarato del Gauntlet Loop di Matt Shumer, lo stesso citato in [[sources/Source_Simone_Rizzo_Loop_Engineering|Rizzo]]) e la tabella dei **6 "Signs of AI writing"** con regola+esempio prima/dopo per ciascuno. 67 KA (66 osservati, 1 inferito e marcato ➕). **0 patch applicate** — 4 proposte in "Consigli", verificate con grep prima di essere scritte (il gap sul Design Loop e su 390px è confermato reale in `.claude/agents/`; il gap sulle "AI tells" è ridimensionato — `copy-editing/SKILL.md` ha già una tabella di sostituzione parola-per-parola, manca solo il livello frase).
- [[sources/Source_Giovanni_Beggiato_Corso_Agenti_AI|Giovanni Beggiato — CORSO COMPLETO AGENTI AI: le tecniche che NESSUNO ti spiega (batch max18, v06)]] — Video JTn5pqm9ecM, 1h50m20s, **228 KA** (il più denso del lotto), 427 archi, 1 sola componente/0 orfani. Otto tecniche dimostrate dal vivo sul progetto di produzione reale dell'autore: **Adaptive Guardrails** (correction log ALWAYS/NEVER/WHEN:surface con Supersession→Specificity→Recency), **4 livelli di specificità del prompt** (Global CLAUDE.md→Project→Skills→Inline, "più specifico vince"), **Step-Back/Reverse Prompting**, **Prompt Contracts** (Obiettivo/Vincoli/Formato/Fallimento), **Pre-Mortem**, **Agent Polling** (N pareri indipendenti + confidenza + outlier), **Agent Debates** (Sostenitore/Critico a 3 round), **Browser Swarm** (N browser paralleli via CDP) e **Audit** a 3 ispettori zero-contesto. Gap-analysis: match diretto e verificato su Pre-Mortem (già `ADR-006`, step obbligatorio) e sulla gerarchia di specificità (già l'architettura reale di questa sessione); gap reali su Agent Polling e Prompt Contracts leggero (nessun equivalente trovato con Grep in `swarm-orchestration`/`sparc-methodology`); Audit: DE è avanti (5 Sentinelle già più granulari dei 3 ispettori del corso).
...
- [[projects/Agency/Progetto_Sito_Agency_Vivo|Sito Agency Vivo — dossier 37]] — BOCCIATO da Max il 2026-09-12 sera: la riscrittura a 20 sezioni è stata online un'ora, poi «riporta il sito esattamente com'era»; live e codice tornati a giugno, aggiunte SOLO 3 sezioni (specchio, prove-vere, cosa-ottieni) + `/prenota/` con CSS scopato. Legge: su un sito online di Max si aggiunge soltanto. CP-20260912-7ZNY; la v2 resta in `6565545f`.
...
### Dev Tooling & Cross-Model Review
- [[sources/Source_Riccardo_Belli_Claude_Codex_Setup|Riccardo Belli Contarini (Martes AI) — Claude Code + Codex: Il Setup di cui NESSUNO Parla (batch max17, 6/8)]] — Video T7PPX5M6Puo, 30m52s, **197/197 frame unici letti — coverage 100%**, 70 KA, NO-FINTO PASS. Plugin ufficiale OpenAI "Codex Plugin" dentro Claude Code (5 comandi: review/adversarial-review/rescue/transfer/status-result) per implementare "CHI COSTRUISCE ≠ CHI GIUDICA" — Claude scrive, Codex (modello di famiglia diversa) contesta. **VERDETTO: il setup completo non serve a DE** — il principio è già in ADR-006 (step REVIEW indipendente) e nei sentinel esistenti (`sentinel-security`, `sentinel-quality`, `sentinel-drift`, `review-and-heal`, `security.agent`). **Il gap reale, unico e provato 3 casi su 3**: tutti quei giudici DE girano su modelli della **stessa famiglia** di chi produce — MaReply (clone ManyChat) e un form candidature erano stati dichiarati "pronti per la produzione" da Claude Code, Codex ha trovato falle **Alte** in entrambi; sul piano di un clone Bitly, Claude stesso conferma 4/5 obiezioni Codex fondate (incluso un 301 vs 302 già noto da Bitly dal 2016). **0 patch, 0 skill, 0 agenti, 0 strumenti installati** — per volontà esplicita del verdetto. Unico artefatto: **proposta di ADR** (non attiva) in `company/Memory/decisions/ADR-PROPOSTA-cross-model-review.md` — audit cross-model solo in fase GATE, solo su deliverable ad alto rischio dati/credenziali (Preventa Outreach, Formazione Empire, PreventivoForge). Backlog **B-042**, da approvare da Max.
- [[sources/Source_Riccardo_Belli_Risparmiare_Token_Claude_Code|Riccardo Belli Contarini (Martes AI) — Dammi 36 Minuti e Ti Farò Risparmiare MILIONI di Token su Claude (max18, v03)]] — Video 1Dyld3y-V7Y, 36m01s, **138/138 frame unici letti — coverage 100%**, 47 KA con **96 archi tipizzati** (primo atoms.json di DE con relazioni fra atomi), NO-FINTO PASS. Lavagna Excalidraw a 6 blocchi + screen-recording di Claude Code: `/clear` a ogni cambio di task, `/rewind` e file di handoff al posto di `/compact`, CLAUDE.md come indice sotto le 200 righe, un CLAUDE.md per cartella (-7.160/sessione), spegnere gli MCP inutili (-47.000/sessione = GitHub 26.000 + Slack 21.000), **CLI invece di MCP** ("il taglio più grosso, e non è un tool": MCP 26.000 token contro CLI ~40, verifica `/context` deve dire 'deferred'), hook PDF→testo, "l'esecuzione ripetibile è codice", sub-agenti che **spostano** il conto invece di ridurlo (9.800 spesi per risparmiarne 5.700, usarli solo in bulk e su Haiku), soglia ~500 file sotto cui CodeGraph/Graphify non convengono. **VERDETTO: DE è già conforme sulle due regole grosse** (CLAUDE.md root = 153 righe, 48 CLAUDE.md nel repo, gerarchia haiku/sonnet/opus già in `emperator.md`) **e ne viola una in modo documentato**: `agente-max/knowledge/K05-context.md:531` raccomanda Autocompact **on** e `/compact` manuale, mentre `company/Memory` ha **0 occorrenze** di "compact" — nessuna regola dell'Impero, serve decisione di Max. Gap più preciso: `scripts/peso_skill.py` misura il **corpo** delle skill ma **non** le `description` (0 occorrenze nel file), che è il costo pagato a ogni sessione per **tutte** le skill installate. **⚠️ Cifra "1.500-3.000 token/pagina PDF" tracciata alla fonte**: a schermo (`frame-353`) si vede che viene da un **AI Overview di Google** che cita GitHub/Reddit/Medium — **non** documentazione Anthropic; riportata solo come "detta dal relatore". Contraddizione trovata e mai commentata dall'autore: il pannello `/usage` che lui stesso mostra consiglia "Usa /compact a metà attività". **0 patch applicate** (Fase 1 = solo studio). ⚠️ Sorgente video a **640x360**: citazioni testuali solo dalle viste in cui l'autore zooma, vincolo dichiarato in `coverage.md`.

### Claude Cowork & Produttività AI
- [[sources/Source_Giovanni_Beggiato_Claude_Cowork_Corso|Giovanni Beggiato — Claude Cowork Corso Completo: Da ZERO ad Impiegato AI (batch max18, v08)]] — Video DI5aWJiFAt8, 1h53m31s, **229 KA**, 226 archi, 18 componenti connesse, 1 orfano reale. Corso a 18 capitoli e 4 demo dal vivo (report CRM, riordino fatture, proposta commerciale, PowerPoint brand guidelines) sui **tre volti di Claude** (Chat/Code/Cowork), sul ciclo **FAI-VERIFICARE-GAP** per delegare task, sul parallelo esplicito **Project/Global Instructions ↔ CLAUDE.md**, e sui **subagenti pipeline vs paralleli** (quantificato: 12s→7s). **DE aveva già studiato Cowork/Skills/Connectors da un corso diverso e molto più superficiale** ([[sources/Source_CS2_Bonus_05_Projects_Cowork]], 3 pagine stub da 19-22 righe, Claude Speedrun 2 di Andrei Pascu): questa fonte espande lo stesso prodotto con un ordine di grandezza di profondità in più. Conferma cross-fonte (quarta occorrenza, primo corso diverso) di un gap reale già noto in `beast-preventivi` (assenza pattern "rifiutati se mancano dati obbligatori").

### Second Brain & Knowledge Architecture
- [[sources/Source_Nate_Herk_Claude_Second_Brain_Levels|Nate Herk — Every Level of a Claude Second Brain Explained (batch max17, 8/8)]] — Video DTCyvo6cC54, 30m59, **130/130 frame unici letti — coverage 100%**, 55 KA, NO-FINTO PASS. Tassonomia a 5 livelli di retrieval per un second brain su Claude Code (L1 parola esatta, L2 wiki curata, L3 ricerca semantica, L4 knowledge graph, L5 always-on), ognuno risposta a una domanda diversa, non a un budget diverso: **"reverse engineer based on the question."** Ogni livello ha una cartella demo con `CLAUDE.md` integrale mostrato a schermo. Confronto verificato sulla wiki reale DE (1.831 pagine): DE opera a **Livello 1-2** (router curato, ricerca lessicale/wikilink), oltre la soglia in cui il video consiglia il salto al Livello 3 — nessuna ricerca semantica configurata. **2 patch, +28 righe nette, 0 cancellazioni di contenuto**: `sync-wiki-totale/SKILL.md` (+12, nuova valutazione del livello di maturità per area nel report MATCH/GAP), `conoscenza-empire.md` (+16, nota di onestà epistemica: la ricerca su 1.800+ pagine è lessicale non semantica, provare più formulazioni prima di dichiarare un vuoto). Perimetro del brief limitato a questi due soli artefatti. 2 proposte non costruite in backlog (B-040 ricerca semantica/plugin Smart Connections, B-041 logica di pruning two-bucket).
- [[sources/Source_Jay_E_Agentic_OS_Claude5|Jay E | RoboNuggets — The NEW Agentic OS standard for Claude 5 Models is here (batch max17, 5/8)]] — Video 8NSyI-npJCU, 21m38s (EN), **181/181 frame letti — coverage 100%**, 70 KA (10 ufficiale/56 proprietario/4 riferimento-esterno), NO-FINTO PASS. **⚠️ Fonte in parte promozionale** (teaser del corso a pagamento dell'autore): solo le **6 regole "Then→Now" di context engineering** (attribuite a Thariq/Anthropic) e 7 funzioni native di Claude Code confermate a schermo hanno fonte ufficiale verificabile; il framework proprietario **ARMS** (Applications/Routines/Memory/Skills) con schema "Level 1/2/3" è costruzione dell'autore, dichiarata come tale. **Gap misurato su DE**: 115 delle 170 `SKILL.md` (68%) superano le 150 righe, la soglia usata nel video per definire una skill da spezzare in router + file dedicati (`B-039`, da approvare da Max). **Questa è la fonte esatta della regola "un puntatore stale è peggio di nessun puntatore"** ora in `CLAUDE.md` radice — pagina rimasta orfana (mai agganciata qui) fino al 2026-09-09, corretto in questo stesso turno.
- [[sources/Source_Giovanni_Beggiato_Company_Brain_Karpathy|Giovanni Beggiato — Claude Code + Karpathy = Agenti AI da 10.000€ (max18, v02)]] — Video LCNk5e5EiCA, 25m03s, **69/69 frame unici letti (100%), 80/501 densi (16,0%), trascrizione 100%**, 43 KA, NO-FINTO PASS. Il **pattern LLM Wiki di Andrej Karpathy** (post originale letto a schermo) calato su un'azienda come **Company Brain**: tre strati (fonti grezze immutabili / wiki di note collegate mantenuto dall'AI / schema di regole) e tre operazioni (**INGEST, QUERY con citazione delle note, LINT** su contraddizioni-dati vecchi-note orfane). Dimostrazione A/B live: lo stesso transcript di call a due Claude Code, uno in cartella vuota e uno nel cervello — e **la differenza non è estetica ma di verificabilità** (il primo chiede all'umano di confermare cifre che ha inventato, il secondo dichiara 5 controlli superati contro `offerta/offerta.md` e registra una nota di memoria). Recuperati parola per parola i due prompt e le tre regole del `CLAUDE.md` (*i prezzi vivono SOLO in offerta.md* · *nessuna proposta senza nota* · *mai mostrare .env*), più il pattern di integrazione API (*interroga il template prima di scrivere il codice* + *uploaded → aspetti → draft*) e il value-based pricing con la proxy `tempo persona × proposte/mese` e il criterio del retainer. **0 patch applicate** (perimetro `EMP-QQ2R`, fase di studio) — 7 consigli tutti verificati con `Grep`, tra cui: il listino DE è copiato in **68 file** e **non esiste nessun `offerta.md` canonico**; `/lint-wiki` `/query-wiki` `/synthesis` `/research-topic` sono documentati in `second-brain-vault/CLAUDE.md` ma **nessuna delle quattro skill esiste**; `proposal-gate` e `beast-preventivi` hanno **zero** riferimenti a memoria/registro; e la **contraddizione sul retainer** fra `agency-scalping:68`, `cro-call:1293/2234/3776` e `proposal-gate:46`, che il video scioglie con un criterio invece di uno slogan.

### Learning Science & Metodo di Studio
- [[sources/Source_Justin_Sung_Guida_Apprendimento|Justin Sung — Guida completa all'apprendimento, 4 capitoli (max18)]] — **fonte TESTUALE, non un video: zero frame visti, il video non è disponibile** (chiude l'ultimo buco dichiarato di `EMP-QQ2R`). Documento consegnato da Max, 285.119 caratteri, **100% letto**, 88 KA con offset verificato nel testo, NO-FINTO PASS con natura testuale dichiarata. Le 3 fondamenta (**encoding** / retrieval / enablers) e l'ordine controintuitivo in cui vanno sviluppate; **teoria del carico cognitivo** a U rovesciata (la confusione è il segnale giusto, noia e sonnolenza sono la firma di una tecnica inefficiente); la critica ad active recall/spaced repetition ("riempire un secchio bucato") e il metodo delle 3 volte per fondere le flashcard; **PACER** (procedural/analogous/conceptual/evidence/reference, ognuno col suo processo); il **metodo della scala** per studiare da esausti; le **12 regole dell'encoding**; **GRIND** e i livelli 0-3 delle mappe mentali, col passo *emphasized* che quasi tutti saltano; **delayed note-taking** e l'interruttore juggle mode→organizing mode; **RAIL** e le 4 fasi dell'acquisizione di un'abilità + **latent learning** e **theory overload** (≥5 ore di pratica per ogni ora di teoria). 7 consigli per DE tutti verificati con `Grep`: **zero learning science in tutto l'ecosistema di studio dell'Impero**, `book-to-skill` senza passo "emphasis/backbone", contraddizione "summaries" vs "mai riassunti", e **`atoms.json` senza archi — DE produce isole, non reti**.

### Mindset & Sviluppo Personale
- [[sources/Source_Jim_Rohn_Seminario_4_Emozioni|Jim Rohn — Come Trasformare Completamente la Tua Vita in un Solo Giorno, Seminario (batch max18, v04)]] — Video 140FuW7b9pk, 14m05s, canale "Imprenditor Tips" (**non un video tech come gli altri del lotto**, seminario motivazionale d'archivio rimontato), 120/120 scene analizzate (100%), **78 KA, 105 archi, 1 componente connessa, 0 orfani**. Framework a **quattro emozioni** (Disgusto→Decisione→Desiderio→Determinazione) + concetto-ponte **Azione** + **quattro domande di riflessione** (Perché?/Perché no?/Perché non tu?/Perché non adesso?), con la definizione-chiave **"determinazione = promettere a te stesso"** e l'aforisma **"l'affermazione senza disciplina è l'inizio dell'illusione"**. Gap analysis reale (grep su skills/Memory/wiki): **nessuna sovrapposizione** con DE — solo `pillar-09-mindset.md` di agency-scalping condivide la parola "mindset" ma copre resilienza imprenditoriale (Eric Siu), contenuto complementare non ridondante; materiale genuinamente nuovo per l'Impero.

### Sistemi di Vendita & Sales Process
- [[sources/Source_Will_Barron_Sistema_Vendita_5_Fasi|Will Barron — If You Don't Understand Sales Systems, You Don't Understand Business (batch max17, v10)]] — Video 5swDtQFyIws, 24m06, **218/218 frame unici letti — coverage 100%**, 55 KA, NO-FINTO PASS (con incidente dichiarato e sanato: i frame 001-479 erano stati descritti senza vederli, poi tutti ri-osservati). Il fatturato oscilla perché **il delivery è un processo documentato e le vendite no**: sistema a 5 fasi **ICP → Meetings → Indoctrinate → Discovery Call → Business Case**, con i 6 passaggi della discovery call (Pain, Trigger, Future Reality, ROI, Budget, Next Step) e la regola *"Questions first, Solutions later"*. Archiviati verbatim gli artefatti mostrati a schermo: 2 email a confronto (ICP generico vs ICP con fascia $20K-$200K/mese), email di indottrinamento pre-call, landing di conferma, 4 voci FAQ col prezzo pubblicato ($8.000). **Il buco vero di DE non è nella call** — lì `cro-call` (5.146 righe) è più profondo della fonte — **ma intorno**: zero azioni verso il prospect fra prenotazione e call, nessun tasso di conversione per fase, nessun ICP scritto e vincolante per l'agenzia. **3 patch, +64 righe, 0 cancellazioni**: `cro-call` (+24, citazione diretta obbligatoria in apertura del documento strategico e bias di coerenza cognitiva), `icp-radar` (+29, campo `trigger_evento` e test del riconoscimento in 1 secondo), `discovery-call-brief` (+11, data e ora del prossimo step come criterio di gate: senza, la discovery non è chiusa). 4 proposte non costruite (B-042 skill `pre-call-indoctrination`, B-043 agente `sales-funnel-auditor`, B-044 workflow post-call→business case+calendario, B-045 ADR sul prezzo pubblicato prima della call).

### Storytelling & Public Speaking
- [[sources/Source_Vishen_Lakhiani_Master_Storyteller_HSTSS|Vishen Lakhiani — Become a Master Storyteller (The Dopamine Trick Elite Speakers Use) (batch max17, v14)]] — Video P-BQ-AGS0ck, 25m55, **83/338 frame guardati (24,6%), campionamento sistematico dichiarato**, copertura non-zero su tutti i 10 capitoli, 35 KA (tutti osservati), NO-FINTO PASS con copertura parziale dichiarata. **Correzione a `EMP-QQ2R.md`**: il video di Vishen non era "mai trovato", era proprio questo run (identità confermata da `info.json`: uploader/channel "Vishen", `@vishen`). Framework a 5 passi **HSTSS** ("Heroes Start Their Stories Somewhere" → mnemonica studenti "Holy Sh\*t That's So Smart") = Hook/Stake(s)/Turn/Scene/Shatter, illustrato con la storia reale del fondatore (quasi-senzatetto a Berkeley dopo la bolla dot-com → scoperta dell'intuizione → oggi corso Silva Ultramind rivenduto da Mindvalley per 40M$/anno). Il pezzo di maggior valore per DE: la tecnica **"idea collision"** per costruire hook (scontrare due idee incompatibili nella stessa frase) e il device di montaggio **reveal progressivo** (la tabella mnemonica si sblocca una riga per capitolo). **0 patch applicate** — 5 proposte in "Consigli" (skill `mnemonic-forge`, agente story-mining per `case-study-forge`, workflow "fatto aziendale → storia HSTSS", potenziamento `cro-copy-architect` con idea collision), verificate con grep, non costruite (perimetro `EMP-QQ2R`, nessuna modifica a sistemi condivisi mentre altre sentinelle lavoravano in parallelo).

### Automazione & Trading AI
- [[sources/Source_Giovanni_Beggiato_Bot_Crypto_Trading|Giovanni Beggiato — Ho Creato un Bot Crypto che Fa Trading 24/7 con Claude Code (batch max18, v05)]] — Video RnNSRF4s9nk, 23m37s, **72 KA, 151 archi, 1 componente connessa, 0 orfani**, framework replicabile a **3 fasi** (Comprensione del problema → Design del sistema → Implementazione) dimostrato costruendo un bot Bitcoin su Binance Testnet con Claude Code in **plan mode**: strategia **ensemble/voting a 5 indicatori non correlati** (EMA/RSI/OBV/Bollinger/ADX), risk management (2% capitale/trade, stop-loss 1,5×ATR), skill personali `/diagram-generator` e `/prompt-contracts`, disciplina chiave API "solo lettura". **DE ha già un bot di trading più rigoroso** (`12-STREAM-S7-BOT`/APEX-7, NFT/token su Solana, gate L5, paper-trading forzato) ma non le due skill di diagramma/contract dell'autore, verificate assenti.

### Voice AI & Agenti Vocali
- [[sources/Source_Giovanni_Beggiato_Agenti_Vocali|Giovanni Beggiato — AGENTI VOCALI IN CLAUDE CODE: Costruisci e Vendi (batch max18, v09)]] — Video NmoOZVTrTXA, 133m11s (il più lungo del lotto), **323 KA**, NO-FINTO PASS. Corso completo Vapi/Retell/n8n/Claude Code: **anatomia a 3 blocchi** (ASR→LLM→TTS), **system prompt a 9 componenti**, **3 metodi di Knowledge Base** (tramite tool = "sempre la soluzione", onDemand vs ricarica a ogni turno), **latency tuning** su 3 leve, **4 file .md** (CLAUDE/n8n/vapi/voice_agent.md) come motore di build in Claude Code, AI Receptionist end-to-end con debugging reale (2 sub-agenti in parallelo), **4 offerte aziendali** ($20k-700k/anno). Due discrepanze titolo-vs-contenuto verificate (5→4 tool, 4→3 metodi KB). **Gap DE reale confermato via Grep**: nessun canale vocale esiste nell'outreach automatico di DE (solo WhatsApp/email/LinkedIn) — candidato diretto per un nuovo servizio produttizzato dell'agenzia CRO, stesso pattern di scalabilità già validato su PreventivoForge.

### Finance AI & Tesoreria
- [[sources/Source_Giovanni_Beggiato_CFO_AI_Claude|Giovanni Beggiato — Ho creato un CFO AI che controlla l'azienda H24 con Claude (batch max17, v15)]] — Video sno_IcNbYFM, 34m52s, **82/226 frame unici letti (36,3%)**, 40 KA, NO-FINTO PASS con copertura parziale dichiarata (tutti i 6 prompt e le sezioni chiave del report letti per intero, codice sorgente negli editor non ingrandito — dettaglio in `coverage.md`). Sistema a tre fasi mai mescolate: **estrazione** (QuickBooks via OAuth, solo dati grezzi) → **calcolo deterministico** (motore Python, un test di riproducibilità SHA-256 ha trovato 2 bug reali) → **interpretazione** (due skill Claude separate, `analista-finanziario` che calcola e non interpreta, `ai-cfo` che interpreta e non ricalcola), chiusa da un **cancello anti-invenzione automatico** (`verifica_dashboard.py`) che blocca la consegna se un numero della dashboard non risale a un dato calcolato davvero. **Deliverable unico di questo run, non presente in nessun altro video del lotto**: `confronto-tesoreria.md`, confronto punto per punto con [[tools/Tool_Tesoreria_Digital_Empire|la Tesoreria di DE]] (ADR-020, nata lo stesso giorno) — 5 consigli concreti presi dal video (soglie di allerta in codice, scadenzario crediti con campo data-scadenza, cancello anti-invenzione sulle risposte in prosa degli agenti, test di determinismo/regressione, terzo tipo di dato "parametro esterno"). Nessuna patch applicata a skill/agenti esistenti in questa sessione (perimetro del checkpoint `EMP-QQ2R`, nessuna modifica a sistemi condivisi mentre altre sentinelle lavoravano in parallelo).

### Personal Brand & Infobusiness
- [[sources/Source_MiK_Cosentino_Micro_Personal_Brand|MiK Cosentino — Come creare un MICRO-PERSONAL BRAND da milioni di euro (batch max17, v16)]] — Video gUnQK6bWHkI, 57m10s, intervento dal vivo a un evento (non uno screen-share come gli altri video del lotto). **20/858 frame guardati nativamente (2,3%, campionamento mirato dichiarato)**, **trascrizione audio 100% letta**, 29 KA, NO-FINTO PASS con copertura frame parziale dichiarata. Tesi centrale: non serve un pubblico gigantesco, serve **tagliare volontariamente la supply** (front-end ~€500, mastermind a due livelli €30k/€46k l'anno, cap 50 persone) per far salire la domanda percepita, con un ciclo mensile a 4 settimane (3 di contenuto, 1 di apertura/chiusura cancelli con lista d'attesa vera e dichiarazione pubblica di sold-out). Rilettura aggiornata di **"1000 True Fans" di Kevin Kelly**: studio proprietario su 1.300 infobusiness (media ~2.000 follower Instagram tra chi supera €10.000/mese). Contributo più riproducibile: le **Storie in Evidenza come sales letter a 14 card** (dolore→scoperta→risultato→CTA), testo integrale letto a schermo in `frame-663.png`. **0 patch applicate** (perimetro `EMP-QQ2R`, fase di studio) — 5 gap verificati via grep in `cro-strategy-social-(ig-tiktok)`, `social`, `icp-radar`, `market-launch`, `pricing`, proposte non costruite.

### Agency Operations & Scaling
- [[sources/Source_Giovanni_Beggiato_Guida_Agenzia_AI|Giovanni Beggiato — Come Avviare Un'Agenzia AI da 10.000€/Mese, Guida COMPLETA (batch max17, v17)]] — Video rvpRQD43wdY, **4h17m00s, il più lungo del lotto max17**. Formato misto: talking-head+whiteboard per ~2h53m (disegni fissi a schermo fino a 31,5 minuti consecutivi), poi screen-share denso (GoHighLevel/Meta Ads/Whimsical) negli ultimi ~80 minuti. **24/1.928 frame guardati nativamente (1,2%), concentrati nelle sezioni a screen-share reale**, **trascrizione audio 100% letta** (5.550 righe uniche da 44.552 grezze, 12 blocchi), 31 KA, NO-FINTO PASS con copertura frame parziale dichiarata. Guida end-to-end: Blue/Red Ocean per la nicchia, matrice di pricing **DIY/DWY/DFY × Tempo/Unità/Risultato** (verificata a schermo), golden rule del **close rate al 30%**, **6 metodologie di acquisizione clienti ranked** (Warm Network con catena testimonial+2 referral, Upwork, Strategie Cold con funnel a cascata e tecnica "cold reading", Ads, Fiverr, Organico), flowchart Whimsical reale di fulfillment (kickoff call, under-promise-overdeliver, scope creep, policy credenziali sempre del cliente), demo GoHighLevel (custom values vs custom fields, workflow con tecnica "taking in charge"), regola di hiring "CTO prima del commerciale, per bisogno non per crescita". **0 patch applicate** (perimetro `EMP-QQ2R`, fase di studio) — 5 gap verificati via grep in `agency-scalping`, `client-handover`, `delivery-playbook`, `pricing` (nessuna skill DE oggi menziona GoHighLevel), proposte non costruite. **Aggiornata 2026-09-04** con il materiale nuovo dello stesso file consegnato da Max (dati Eurostat 8%/6,4%/12,8%, mondo 84% mai usato l'AI e 0,04% su strumenti agentici, **released capacity** 500 ore→€5.000/settimana, le 7 fasi, i 3 pilastri, meccanica esatta dello speed to lead) e con la **correzione delle soglie del close rate** (60% troppo basso / 30% golden rule / 20% troppo alto — erano schiacciate in una sola soglia).
...
- [[Tool_Higgsfield_ElevenLabs]] — valutazione d'acquisto dei due strumenti: listini veri estratti dal DOM, tabella crediti per modello, e i tre vincoli che cambiano il piano (Fliki non si sostituisce, l'unlimited non si automatizza, le chiamate a freddo in Italia sono bloccate)
