---
Type: PROJECT
Status: Active
Tags: #youtube #lead-generation #cro #acquisizione-clienti #canale-youtube #formazione
Created: 2026-07-19
Last updated: 2026-07-19
---

# Project: YouTube Lead Machine — Canale Digital Empire

## Overview
Progetto di lancio del canale YouTube di Digital Empire come **sistema di acquisizione clienti** (non di views)
per l'agenzia CRO. Basato sul metodo Media Profit (5 video), sui principi lead-magnet di Alex Hormozi e sul
framework Think Media. Materiale sorgente: `Formazzione/Youtube/` (lista video + 2 infografiche NotebookLM).

## Documenti operativi
- **Strategia completa:** `Formazzione/Youtube/STRATEGIA-YOUTUBE-LEAD-MAGNET.md` ⭐ (punto di partenza)
- **Note per video:** `Formazzione/Youtube/note-video/01-07`
- **KIT DI LANCIO (2026-07-20):** `Formazzione/Youtube/CLIENTE-DORO.md` (scheda ICP) · `SETUP-CANALE.md`
  (copy pronto da incollare: nome/descrizione/banner/link) · `LEAD-MAGNET-01-analisi-gratuita.md`
  (magnet + gate qualifica + 5 messaggi automazione speed-to-lead pronti) · `batch-01/` (piano 6 video
  + script completi hook/scaletta/CTA + 2 concept copertina in `batch-01/copertine/`)
- **Fonti:** [[Source_MediaProfit_YouTube_Lead_Machine]] · [[Source_Hormozi_Lead_Magnet]] · [[Source_ThinkMedia_YouTube_Lead_Framework]]
- **Concept:** [[Concept_YouTube_Funnel_TOFU_MOFU_BOFU]] · [[Concept_Lead_Magnet_Hormozi]] · [[Concept_Speed_To_Lead]]

## Il sistema in una riga
1-2 video evergreen a settimana (4 ore, batch) → SEO/audit/case study → CTA 1-click Calendly/WhatsApp
→ risposta in 1 minuto (automatizzata) → call già calda. Views ≠ clienti: meglio 200 iscritti e €20k che 1M views.

## Stato e prossimi passi
- [x] **Kit strategico-creativo COMPLETO (2026-07-20, CP-20260720-001):** cliente d'oro, setup canale
  (copy pronta), lead magnet con automazioni, batch #1: 6 script pronti da registrare.
- [ ] Eseguire setup canale con la copy di SETUP-CANALE.md (nome frontman da decidere: Max o Gael)
- [ ] Creare Calendly "Analisi gratuita 15 min" + gate 3 domande + attivare i 5 messaggi automazione
- [ ] Preparare setup registrazione fisso (telefono + lavalier + finestra) + checklist PDF "21 punti"
- [ ] Registrare batch #1 (ordine: V03 → V01 → V02 → V04 → V06 → V05) in una sessione da 4 ore
- [ ] Pubblicare 1-2 video/settimana + review dati settimanale (retention, CTR, lead per video)

## Posizionamento canale
Frontman = persona, non logo. Promessa: trasformare traffico esistente in fatturato con sprint CRO productized —
coerente col brand "l'agenzia progettata per essere licenziata". Target: decision-maker con traffico e conversioni basse (livelli Schwartz 3-4).

## Connessioni
- [[Piano_Maestro_EMPIRE_OS]] (ecosistema 04 MARKETING / 05 MULTI-BUSINESS — YouTube Automation)
- [[Project_Prof_Autocad_PreventivoForge]] (stesso principio delivery: sistema > sforzo manuale)
- [[Concept_CLV_Customer_Lifetime_Value]] (CAC tollerabile alto su ticket CRO high-ticket)
