...
Genera i file in questo ordine — **TUTTI E 4 GLI STEP SONO OBBLIGATORI, NESSUNO È OPZIONALE:**

```
1. [Nome Skill].skill\[Nome Skill].md          ← file principale skill
2. [Nome Skill].skill\references\*.md           ← file di riferimento dettagliati (min 3)
3. [Nome Skill].skill\scripts-py\*.py           ← script Python (min 2) — NON SALTARE MAI
4. [Nome Skill].skill\[Nome Skill] - COMPLETO.md ← file unico con tutto dentro (OBBLIGATORIO)
```

> ⚠️ **REGOLA NON NEGOZIABILE — SCRIPTS-PY:**
> Lo step 3 (scripts-py) NON può essere saltato per nessun motivo.
> Non esistono skill "troppo semplici" per avere script Python.
> Minimo 2 script per skill. Se non hai idee, crea sempre:
> - uno script `[nome]_generator.py` che produce output strutturato
> - uno script `[nome]_checker.py` o `[nome]_validator.py` che valida input
>
> **Prima di generare il file COMPLETO, verifica con questo controllo:**
> ```
> CHECKLIST PRE-COMPLETO:
> □ skill.md creato
> □ references/ ha almeno 3 file .md
> □ scripts-py/ ha almeno 2 file .py ← SE NON C'È, FERMATI E CREA GLI SCRIPT PRIMA
> □ Solo dopo aver verificato i 3 punti sopra: genera il COMPLETO
> ```
...
- YAML frontmatter con `name:` e `description:` (multi-riga, con USE THIS SKILL e trigger phrases)
...
- Non usare librerie esterne oltre la stdlib Python
