...
### FONTE 9 — `PIANO-MAESTRO/29-ECOSISTEMA-LANCI/07-REPARTI-E-GERARCHIA.md` + `08-WORKFLOW.md`

**QUESTA FONTE ROMPE LA REGOLA DI TUTTE LE ALTRE.** Verifica su disco:
`PIANO-MAESTRO/29-ECOSISTEMA-LANCI/dati/registro.yaml:1741` apre una sezione `passaggi:` che contiene
**15 passaggi come dati leggibili da un programma** (13 interni + 2 esterni), e
`dati/valida_registro.py:479-501` esegue **INV-20**, che boccia il registro se un passaggio non cita
due reparti esistenti, un artefatto esistente, un criterio di accettazione **e cosa succede se e'
rifiutato**. Sono gli **unici passaggi di consegne dell'Impero che esistano come contratto su disco e
siano verificati da una macchina** — tutti gli altri 298 vivono solo in prosa. Il dossier lo dice da
solo: *"Un passaggio senza criterio di accettazione e' una speranza: chi riceve scopre che manca
qualcosa quando e' gia' al lavoro"* (`07-REPARTI-E-GERARCHIA.md:407-409`). Restano MAI PERCORSI —
nessun lancio e' mai stato eseguito — ma la differenza e' netta: **qui manca il percorso, non la
strada.**

Due invarianti governano il resto: **INV-01** `giudice != produttore` (il gate lo esegue `lan-gate`
di `LAN-QLT`, che non ha `Write` ne' `Edit`, INV-09) e la regola *"nessun workflow contiene il proprio
gate fra le fasi"* (`08-WORKFLOW.md:57-60`), nata da un difetto misurato: nella v3 **cinque controlli
su sette del flusso Prodotto erano eseguiti dallo stesso agente che aveva prodotto la cosa da
controllare** (`08-WORKFLOW.md:61-67`).

| # | DA | A | COSA PASSA | CRITERIO DI ACCETTAZIONE | FONTE | CONTRATTO ESISTE? | PERCORSO? |
|---|---|---|---|---|---|---|---|
| 299 | `LAN-MER` (Mercato) | `LAN-STR` (Strategia) | `ART-PUB` — il pubblico verificato | il totale verificato e' un numero, e **ogni canale che contribuisce ha una prova non piu' vecchia di 30 giorni** · rifiuto: torna a `LAN-MER`, il lancio resta in `IDEA` | `29-ECOSISTEMA-LANCI/07-REPARTI-E-GERARCHIA.md:415` + `dati/registro.yaml:1743-1747` | **SI — dato in `registro.yaml`, verificato da INV-20** | INTRA · MAI (nessun lancio eseguito) |
| 300 | `LAN-MER` | `LAN-OFF` (Offerta) | `ART-RIC` — la ricerca | almeno **15 frasi con fonte apribile**, e il campione riaperto dal gate risulta raggiungibile · rifiuto: torna a `LAN-MER`, **le frasi con fonte valida si conservano** | `07-REPARTI-E-GERARCHIA.md:416` + `registro.yaml` | **SI** | INTRA · MAI |
| 301 | `LAN-STR` | `LAN-PRD` (Prodotto) | `ART-DEC` — la decisione | **cinque risposte, nessuna negativa, ognuna con l'artefatto che la sostiene** · rifiuto: `ARCHIVIATO` con la ragione, riproponibile con un elemento nuovo | `07-REPARTI-E-GERARCHIA.md:417` + `registro.yaml` | **SI** | INTRA · MAI |
| 302 | `LAN-PRD` | `LAN-OFF` | `ART-CRT` — il certificato di consegnabilita' | nessuna bandiera rossa, **link testati dal gate** (*"la verifica la rifa' il gate, non il produttore"*), debito di collaudo dichiarato in modalita' retroattiva · rifiuto: torna a `VALUTATO` con l'elenco di cosa manca | `07-REPARTI-E-GERARCHIA.md:418`, `08-WORKFLOW.md:228` + `registro.yaml` | **SI** | INTRA · MAI |
| 303 | `LAN-OFF` | `LAN-CPY` (Parola) | `ART-OFF` — l'OFFERTA (prezzo + data) | prezzo e data esistono, la firma ha canale ammesso e **l'impronta corrisponde alla proposta corrente** · rifiuto: torna a `LAN-OFF`, la proposta si conserva e la fase `O5` si riapre | `07-REPARTI-E-GERARCHIA.md:419` + `registro.yaml` | **SI** | INTRA · MAI (**e' "il cuore": il flusso per cui l'intero ecosistema esiste**) |
| 304 | `LAN-OFF` | `LAN-TSR` (Tesoro) | `ART-PRV` — la previsione | i tre scenari esistono e **ogni assunzione dichiara stato `misurato` oppure `assunto`** · rifiuto: torna a `LAN-OFF`, la previsione si conserva marcata non valida | `07-REPARTI-E-GERARCHIA.md:420` + `registro.yaml` | **SI** | INTRA · MAI |
| 305 | `LAN-CPY` | `LAN-FNL` (Vendita) | `ART-CPY` — i testi | **punteggio totale ≥80, nessun blocco sotto meta' dei propri punti** (il totale alto non salva il blocco debole), ogni prova risolvibile · rifiuto: **si rifanno solo i blocchi bocciati** | `07-REPARTI-E-GERARCHIA.md:421`, `08-WORKFLOW.md:302` + `registro.yaml` | **SI** | INTRA · MAI |
| 306 | `LAN-CPY` | `LAN-EDT` (Editoriale) | `ART-CPY` — i pezzi editoriali | ogni pezzo destinato a un contenuto editoriale **esiste come file non vuoto** · rifiuto: torna con l'elenco dei pezzi mancanti | `07-REPARTI-E-GERARCHIA.md:422` + `registro.yaml` | **SI** | INTRA · MAI |
| 307 | `LAN-FNL` | `LAN-REG` (Regia) | `ART-FNL` — pagine + cassa | ogni pagina risponde, l'evento e' arrivato **dalla piattaforma** (*"non si accetta una schermata"*), e la prova di cassa e' **incassata e rimborsata** · rifiuto: le pagine conformi restano | `07-REPARTI-E-GERARCHIA.md:423`, `08-WORKFLOW.md:335` + `registro.yaml` | **SI** | INTRA · MAI (**l'unico criterio di accettazione dell'Impero che esiga un euro realmente incassato**) |
| 308 | `LAN-EDT` | `LAN-REG` | `ART-EDT` — il piano editoriale | nessuna riga incompleta, ogni contenuto punta a una pagina esistente, **ogni giorno del carrello ha un contenuto** · rifiuto: righe incomplete evidenziate | `07-REPARTI-E-GERARCHIA.md:424` + `registro.yaml` | **SI** | INTRA · MAI |
| 309 | `LAN-TSR` | `LAN-REG` | `ART-BDG` — il budget | costo totale sotto il tetto, **pareggio calcolato dalla previsione**, costo macchina presente · rifiuto: budget conservato marcato oltre tetto | `07-REPARTI-E-GERARCHIA.md:425` + `registro.yaml` | **SI** | INTRA · MAI |
| 310 | `LAN-REG` | `LAN-MEM` (Memoria) | `ART-CNS` — il consuntivo | **il ricavo ha origine verificabile** e il periodo copre tutte le date del carrello · rifiuto: il lancio resta `APERTO` | `07-REPARTI-E-GERARCHIA.md:426`, `08-WORKFLOW.md:433` + `registro.yaml` | **SI** | INTRA · MAI |
| 311 | `LAN-MEM` | `LAN-DIR` (Direzione) | `ART-DBR` — il debrief | ogni scarto oltre il 10% ha una causa, **≥3 schemi con `si_applica_quando`** (*"uno schema che si applica sempre non si applica mai"*), e **la controfirma della Regia** · rifiuto: il lancio resta `CHIUSO` e non diventa `APPRESO` | `07-REPARTI-E-GERARCHIA.md:427`, `08-WORKFLOW.md:459` + `registro.yaml` | **SI** | INTRA · MAI |
| 312 | **ULTIMO METRO** (ADR-016) | `LAN-STR` | **la coda in ingresso dell'ecosistema**: l'elenco di cio' che e' pronto e non e' uscito — 25 pezzi finiti mai pubblicati, il piu' vecchio da 135 giorni | il prodotto **esiste come file ed e' dichiarato pubblicabile** | `07-REPARTI-E-GERARCHIA.md:436`, `:441-442` + `registro.yaml:1822` | **SI** | **INTER · MAI** (**e' il collegamento piu' vicino a un incasso di tutta la mappa: la fila da cui i lanci nascono, gia' piena**) |
| 313 | `LAN-REG` | **TESORERIA** (ecosistema 14) | ricavi e costi reali | **ogni numero ha un'origine leggibile in un pannello**; regola propria: *"**ogni euro nasce qui e sale in Tesoreria, non scende mai**. Se un numero compare in tutti e due i posti ed e' diverso, ha ragione la Tesoreria"* | `07-REPARTI-E-GERARCHIA.md:437`, `:444-447` + `registro.yaml:1830` | **SI** | **INTER · MAI** (**l'unico passaggio dell'Impero che porti denaro reale da un ecosistema a un altro**) |
| 314 | `LAN-MEM` (`MM-4`) | `LAN-REG` (`lan-reg-calendarista`) | **controfirma** del debrief da parte di un agente di un altro reparto (`reparto_ospite: LAN-REG`) | la Regia ha controfirmato, **oppure ha lasciato scritta l'obiezione accanto** — *"nessuno dei due scrive da solo la storia di com'e' andata"* | `08-WORKFLOW.md:458`, `:475-477` | **SI** (fase nel registro, deroga dichiarata, INV-19 la verifica) | INTRA · MAI (**unica fase dell'Impero eseguita da un agente di un altro reparto, e l'unica deroga scritta invece che presa**) |
| 315 | macchina | **Max** (6 punti umani) | `PU-RUOLO` (7gg, predefinito `vendita`) · `PU-PREZZO` (14gg, **nessun predefinito** → `SOSPESO`) · `PU-COPERTINA` (3gg, *"la copertina la fa Max"*) · `PU-INVIO` · `PU-APERTURA` · `PU-SPESA` (questi tre **senza scadenza e senza predefinito**) | **il cronometro sta su OGNI punto umano aperto, non sulla firma finale** — verificato da `INV-06` | `08-WORKFLOW.md:483-490`, `:494-505` | **SI** (nel registro, con scadenza/predefinito/comportamento) | INTER (macchina→umano) · MAI (**il prodotto pilota si e' fermato 6 mesi su `PU-RUOLO`, e "il fallimento era invisibile alla sua stessa misura"**) |

### FONTI 10-12 — `company/Ecosistemi/*/ECOSISTEMA.md` + `*/BACKBONE.md` + `company/Backbone/` + `company/GRUPPO.md`

Verifica su disco, ecosistema per ecosistema (righe di `ECOSISTEMA.md` / `BACKBONE.md`):
01-AGENCY 186/118 · 02-INFO-BUSINESS 51/46 · 03-CONTENT-FACTORY 209/144 · 04-MARKETING 58/46 ·
05-MULTI-BUSINESS 234/46 · 06-PLATFORM 46/46 · 07-FORGE 193/116 · 08-INTELLIGENCE 51/46 ·
09-OPERATIONS 167/97 · 10-MEMORY 57/46 · 11-APEX-7-CORE 8/189 · 12-STREAM-S7-BOT 6/8 ·
13-ARENA-APEX 7/7 · **14-TESORERIA: nessun `ECOSISTEMA.md`, nessun `BACKBONE.md`** ·
**08-STREAM-S7-BOT: idem**. Cinque ecosistemi hanno un `BACKBONE.md` di 46 righe identiche —
uno scheletro, non un backbone. Il risultato: **i passaggi che questi file dovrebbero dichiarare
in gran parte ripetono i dossier gia' spogliati**. Qui entrano solo le righe che i dossier non
avevano.

Nota decisiva su `company/Backbone/Bus/README.md:110`, ultima riga del file:
**"Stato: DA COSTRUIRE (F2, task 2.3)"** — `bus.sh`, `gbus.sh`, `validate-handoff.sh` e
`contracts/registry.yaml` **non esistono**; esistono solo `HC-template.json` e la cartella
`handoffs/` vuota. **La strada su cui dovrebbero viaggiare tutti i 314 passaggi di questa mappa
non e' mai stata asfaltata.**

| # | DA | A | COSA PASSA | CRITERIO DI ACCETTAZIONE | FONTE | CONTRATTO ESISTE? | PERCORSO? |
|---|---|---|---|---|---|---|---|
| 316 | 07-FORGE | 09-OPERATIONS | **validazione del cost model di OGNI nuovo agente della holding prima che la FORGE lo deployi** — *"OPERATIONS ha un ruolo speciale"* | agente censito in `Backbone/Identity-HR/registro-agenti.yaml` con tier e costo stimato/run; assunzioni/ritiri **solo** via 07-FORGE → Chief-Forge → registro | `company/Ecosistemi/09-OPERATIONS/BACKBONE.md:85-89` | NO | INTER · MAI (**il veto economico sulla popolazione dell'Impero: ~248 agenti progettati sono passati da qui zero volte**) |
| 317 | 09-OPERATIONS | tutte le business unit | *"OPERATIONS e' l'OPERATORE del componente Observability: metrics, cost-attribution e dashboard sono i suoi deliverable. **Le altre business unit consumano; OPERATIONS produce**"* | `verify-empire.sh` categoria OPERATIONS: **ledger coerente, 0 run orfane, 0 schedule senza runbook** | `company/Ecosistemi/09-OPERATIONS/BACKBONE.md:80-82`, `:91-95` | NO (`verify-empire.sh` da creare in F2) | INTER (1→9) · MAI |
| 318 | coordinator ricevente | mittente | **rifiuto formale sul Bus**: `status: rejected` **DEVE includere `note_correttive` dettagliate**; 2 reject consecutivi dallo stesso team → escalation automatica via gbus | KPI: **handoff rejected senza note correttive = 0**; escalation da 2 reject **tracciate 100%** | `company/Backbone/Bus/README.md:52-57`, `:100-104` | **NO — `bus.sh`/`gbus.sh`/`validate-handoff.sh` non esistono ("DA COSTRUIRE, F2 task 2.3")** | INTRA+INTER · MAI |
| 319 | qualsiasi mittente | qualsiasi destinatario | **priorita' con SLA di risposta**: `CRITICAL` <5 min (Sonnet) · `HIGH` <1h (Sonnet) · `NORMAL` <4h (Haiku) · `LOW` <24h (Haiku) — *"il coordinator valuta la priorita' e istanzia il tier modello appropriato"* | backlog bus (messaggi pending >24h) = 0 | `company/Backbone/Bus/README.md:86-93`, `:99` | NO | INTER · MAI (**l'unica clausola di tempo di risposta scritta in tutto l'Impero, e nessun messaggio l'ha mai messa alla prova**) |
| 320 | mittente inter-ecosistema | destinatario | **`brand_kit` obbligatorio nel payload inter-ecosistema** (pattern #11 multi-tenant): lo stesso bus serve DE, clienti agency, canali YT, libri KDP | handoff inter senza `brand_kit` = invalido | `company/Backbone/Bus/README.md:57`, `:96` | NO (nessun `brand-kit.json` esiste su disco, cfr. #93/#146) | INTER · MAI |
| 321 | `Bus/contracts/registry.yaml` | tutti | **registro dei contratti attivi** + `HC-template.json` per ogni coppia di ecosistemi | nessuno | `company/Backbone/Bus/README.md:74-77`, `:110` | **template presente, registry "da creare F2"** | INTER · MAI (**il catalogo dei contratti dell'Impero e' un file mai creato**) |
| 322 | publisher (agente APEX-7) | subscriber (agente APEX-7) | **Event Bus publish/subscribe**: *"Nessun agente chiama un altro direttamente. Publisher NON SA chi riceve. Subscriber NON SA chi ha inviato → zero coupling"* | nessuno dichiarato | `company/Ecosistemi/11-APEX-7-CORE/BACKBONE.md:168-189` | NO | INTRA (11-APEX-7) · **decimo schema di comunicazione dell'Impero, incompatibile con gli altri nove (nessun `from`/`to`)** |
| 323 | 01-AGENCY (`agency/kpi`) | dashboard KPI 6 reparti (via 06-PLATFORM) | metriche per reparto per ciclo | **KPI "alimentati da dati reali"** (gate B5) | `company/Ecosistemi/01-AGENCY/ECOSISTEMA.md:28`, `:151`, `:166` + `BACKBONE.md:22` | NO | INTER · MAI (gemello di #50) |
| 324 | 01-AGENCY (`agency/reasoning`) | ReasoningBank corporate | pattern distillati da ogni fallimento (bounce, preventivo perso, delivery in ritardo) | entry in `agency/reasoning` | `company/Ecosistemi/01-AGENCY/ECOSISTEMA.md:29` + `BACKBONE.md:23` | NO | INTER · MAI (gemello di #28) |
| 325 | ogni gate AGENCY | `company/metrics/runs.jsonl` | eventi `gate_passed`/`gate_failed` (Bibbia, Preventivo, Delivery), `handoff_rejected`, **`sale_closed`** | nessuno | `company/Ecosistemi/01-AGENCY/BACKBONE.md:110` | NO | INTER · MAI (**`sale_closed` compare in due file dell'Impero e non e' mai stato emesso, cfr. #283**) |
| 326 | 03-CF (conductor) | `wiki/log.md` | *"ogni ordine chiuso logga in `wiki/log.md`; il log lo scrive il conductor"* | ordine chiuso | `company/Ecosistemi/03-CONTENT-FACTORY/BACKBONE.md:82` | NO (`orders/` non esiste) | INTER · MAI (gemello di #101) |
| 327 | Board / Guild | tutti i team | *"una Guild e' un gruppo di agenti con expertise comune che **serve tutti i team**"* (Copy-APSOC, Cost, Design, Prompt, Quality) | nessuno | `company/GRUPPO.md:59` | NO | INTER · **VAGO** (rapporto di servizio, non passaggio) · MAI |
| 328 | 5 Sentinel (LX) | ogni output della holding | Cost · Quality · Drift · Security · Brand-Voice, **always-on trasversale** | ogni sentinella ha soglie proprie nei dossier; **blocco non derogabile per Brand-Voice (solo LX sblocca)** | `company/GRUPPO.md:16`, `company/Ecosistemi/01-AGENCY/ECOSISTEMA.md:140` | NO | INTER (LX→9) · MAI |
