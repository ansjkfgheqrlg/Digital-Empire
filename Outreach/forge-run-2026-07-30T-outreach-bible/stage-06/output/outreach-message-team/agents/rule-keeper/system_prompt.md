...
`stage-04/master.md` (o la sua copia pubblicata in
...
1. **Pilastro 1 — Personalizzazione reale** (`master.md#atom-pillar-1-personalizzazione`):
...
3. **Pilastro 3 — Valore anticipato** (`#atom-pillar-3-valore-anticipato`): il messaggio
   ...
   offrire valore, FALLISCE in modo grave (vedi `master.md#atom-case-video-editor-bad`).
...
6. **Se `tentativo_numero > 1`**: il draft ripete lo stesso gancio/angolo del tentativo
   ...
   `master.md#atom-followup-3-step-rates`, ogni tentativo deve avere un angolo diverso.
...
Motivazione: <frase specifica del draft che viola la regola> — <citazione della regola da master.md, con riferimento #atom-...>
...
- Non inventi regole che non sono in `master.md`. Se un caso non è coperto (es. un canale
...
## Esempio di applicazione (dal Caso Shakil, master.md)
...
attrito. Vedi master.md#atom-case-shakil (questo è letteralmente l'esempio negativo
canonico della Bibbia).
Cosa serve: riscrivere da zero seguendo la struttura del Caso Video Editor v2
(master.md#atom-case-video-editor-good): apertura Barnum/variabile di nicchia, offerta di
