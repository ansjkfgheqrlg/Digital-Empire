...
function CourseCTA({ variant = "dark" }: { variant?: "dark" | "light" }) {
  const styles = variant === "dark"
    ? "bg-white/5 text-white border border-white/25 hover:bg-white/10 hover:border-white/40"
    : "bg-ink/5 text-ink border border-ink/25 hover:bg-ink/10 hover:border-ink/45";
  const subStyles = variant === "dark" ? "text-white/45" : "text-ink/50";

  return (
    <a
      href={COURSE_URL}
      className={`group inline-flex items-center gap-2.5 px-6 py-[14px] rounded-[12px] font-semibold text-[15px] transition-all duration-300 ${styles}`}
    >
      <ArrowRight className="h-4 w-4 opacity-70 group-hover:opacity-100 group-hover:translate-x-0.5 transition-transform" strokeWidth={2} />
      <span className="flex flex-col items-start leading-tight text-left">
        <span>Scopri il corso completo</span>
        <span className={`text-[10px] uppercase tracking-[0.18em] font-bold mt-0.5 ${subStyles}`}>Claude Code Mastery · €397</span>
      </span>
    </a>
  );
}
  ...
  return (
    <main className="relative">
      ...
      <section className="bg-ink relative overflow-hidden">
        <div
          className="border-b border-white/20 overflow-hidden py-3 relative"
          style={{
            backgroundImage: "linear-gradient(135deg, #ffffff 0%, #e8e3ef 25%, #d9d4e1 45%, #ff8a4a 80%, #fb4604 100%)",
            boxShadow: "0 2px 0 rgba(255,255,255,0.6) inset, 0 -2px 0 rgba(201,55,10,0.25) inset",
          }}
        >
          <div className="marquee flex gap-10 whitespace-nowrap text-xs uppercase tracking-[0.25em] font-bold text-[#1c1c1c]" style={{ width: "max-content" }}>
        ...
        <div className="max-w-5xl mx-auto px-6 py-24 md:py-36 text-center relative">
          ...
          <Reveal delay={0.05}>
            <div className="pre-headline mt-4 mb-5">Per chi sta imparando l'AI · E vuole fare il salto</div>
          </Reveal>
          <Reveal delay={0.1}>
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <Reveal delay={0.1}>
            <div className="space-y-5 text-lg md:text-xl text-white/80 leading-relaxed">
              <p>Che tu sia un ragazzo che sta imparando adesso, un freelancer che vuole scalare, o uno che lavora in azienda — se sei qui è perché l'AI ti ha già preso.</p>
              <p>Stai già usando Claude. ChatGPT. Forse Perplexity. E ti piacciono pure.</p>
              <p>
      ...
      <section className="bg-ink section section-border-t">
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <Reveal delay={0.1}>
              <div className="stat-card-silver h-full !text-left !p-10">
                <div className="bubble-orange mb-5"><Check className="h-3.5 w-3.5" /> È per te se</div>
                <ul className="space-y-3.5 text-[#1c1c1c]">
                  {[
                    "Sei un ragazzo/a che sta imparando l'AI e vuoi fare il SALTO — da \"la uso a caso\" a \"ho un sistema\"",
                    "Stai già usando Claude o ChatGPT ogni giorno e senti di grattare la superficie",
                    "Hai progetti tuoi (studio, side project, idee) o lavori (freelance, agency, azienda) e vuoi andare più veloce — senza scrivere una riga di codice",
                    "Ti interessa il METODO e il sistema, non il tip random da Twitter",
                    "Sei disposto ad aprire screen share e mostrarmi cosa stai facendo davvero",
                  ].map((s, i) => (
      ...
      <section className="bg-ink-2 section section-border-t">
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <Reveal delay={0.2}>
            <div className="flex flex-col sm:flex-row gap-3 justify-center items-center">
              <CTA large label="Prendi il Tuo Slot" />
              <CourseCTA />
            </div>
          </Reveal>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <Reveal delay={0.1}>
            <h2 className="text-[36px] md:text-[48px] font-bold leading-tight mb-8 mt-6">
              <span className="text-silver-white">"Ok ma tu chi sei?"</span>
            </h2>
          </Reveal>
          <Reveal delay={0.15}>
      ...
      <section className="bg-ink section section-border-t">
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { n: "Claude Code", d: "Il core. Dove costruisco e refactoro progetti reali ogni giorno. Senza scrivere codice." },
              { n: "Claude Chat", d: "Per pensare ad alta voce, brainstorming, prompt crafting prima di passare a Claude Code." },
              { n: "Claude Projects", d: "Dove metto contesto, documenti e istruzioni persistenti. Il cervello lungo dell'AI." },
              { n: "Claude Agent SDK", d: "Subagenti, skill, hooks. Il layer che trasforma Claude in un team." },
              { n: "Perplexity", d: "Ricerca vera, con fonti. Dove vado quando devo capire qualcosa prima di costruirla." },
              { n: "Manus", d: "Agente autonomo che esegue task complessi end-to-end mentre faccio altro." },
              { n: "Cursor + VS Code", d: "Quando e perché li alterno a Claude Code. E quando NON mi servono proprio." },
              { n: "Playwright", d: "Per automazioni browser che funzionano al primo colpo — generate dall'AI." },
              { n: "Memory & MCP", d: "Como faccio ricordare al modello ciò che conta tra una sessione e l'altra." },
            ].map((t, i) => (
              <Reveal key={i} delay={0.1 + i * 0.05}>
                <div className="card-silver-orange h-full">
                  <div className="text-lg font-bold text-[#1c1c1c] mb-2">{t.n}</div>
                  <div className="text-sm text-[#2a2a2a] leading-relaxed">{t.d}</div>
                </div>
              </Reveal>
            ))}
          </div>
