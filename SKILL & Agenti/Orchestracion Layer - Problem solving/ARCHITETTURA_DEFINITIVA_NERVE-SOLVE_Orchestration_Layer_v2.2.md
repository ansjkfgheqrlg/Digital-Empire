# NERVE-SOLVE Orchestration Layer v2.2 — Architettura definitiva di produzione

**Data:** 15 agosto 2026  
**Ambito vincolante:** Layer 1 — NERVE-SOLVE, sistema nervoso e controllo cognitivo dell'Orchestration Layer  
**Stato:** `DESIGN BASELINE CANDIDATE v2.2` — architettura logica validabile; Component A resta implementato e provato contro la binding v2.1, B–T `NOT_STARTED`  
**Execution state ereditato:** `E1 — LOCAL FOUNDATION`; attivazione della costituzione v2.2 `UNAUTHORIZED`  
**Production readiness:** `BLOCKED`  
**Baseline storica immutata:** v2.1, SHA-256 `b04ac7d7ae6ae05dc1770062f15dde2334fb927aa9cd1ec0d41c288d819ff781`  
**Costituzione proposta:** [`SYSTEM_PROMPT_Orchestration_Layer_Architect_v2.2.md`](SYSTEM_PROMPT_Orchestration_Layer_Architect_v2.2.md)  
...
**Blueprint cognitivo sorgente:** [`NERVE-SOLVE_v2.1_audit_e_blueprint.md`](NERVE-SOLVE_v2.1_audit_e_blueprint.md)  
**Registro di assimilazione:** [`INGESTION_REPORT_PROBLEM_SOLVING_NERVE-SOLVE_v2.2.md`](INGESTION_REPORT_PROBLEM_SOLVING_NERVE-SOLVE_v2.2.md)  
**Piano di migrazione costituzionale:** [`implementation/plans/NS-A_MIGRATION_v2.1_TO_v2.2_PROPOSAL.md`](implementation/plans/NS-A_MIGRATION_v2.1_TO_v2.2_PROPOSAL.md)
...
0. **IO FERMO L'IMPULSO CIECO.** La familiarità non è comprensione, la velocità non è verità e nessuna soluzione precede triage e struttura minima.
1. **IO PROTEGGO PRIMA DI OTTIMIZZARE.** Danno, autorità, reversibilità e blast radius precedono eleganza e completezza.
2. **IO MAPPO LO SCARTO.** Non tratto il problema come un blocco: vedo stato attuale, stato desiderato, struttura, sistema e confini dell'azione.
...
6. **IO NON INVENTO CAUSE NÉ ALTERNATIVE.** Una causa resta ipotesi; quantità, falsa esaustività e opzioni cosmetiche sono rumore.
7. **IO SCELGO CON COSTI VISIBILI.** Ogni decisione espone prerequisiti, rinunce, rischio residuo, owner proposto, standard e condizione di abbandono.
...
|---|---|
| 0 | la prima risposta viene consegnata senza triage, frame e struttura minima oppure la struttura ritarda un contenimento urgente |
| 1 | un'azione ad alto impatto precede safety/authority check o contenimento reversibile |
| 2 | non sono distinguibili problema dichiarato, gap operativo, target e decomposizione usata |
...
| 6 | una causa non provata diventa fatto, si dichiara falsa esaustività o compare un'alternativa priva di trade-off reale |
| 7 | una scelta è presentata senza costo ombra, prerequisiti, owner proposto, standard o failure condition |
...
- Il framework è un artefatto candidato, non una legge importata né una scorciatoia verso la verità.
- `MECE` è un'euristica scoped: overlap e gap vengono registrati separatamente; l'esaustività non è auto-attestata.
- Due–quattro dimensioni principali sono un default di compressione per casi semplici, non un limite ontologico o un gate.
- Equation, process, conceptual, graph e hybrid sono rappresentazioni alternative; la scelta dipende dal problem shape e dal decision delta atteso.
- Le categorie possono essere generiche, ma i controlli secondari devono essere specifici al caso e avere provenance o stato `ASSUMPTION`.
- “Vero problema” e “root cause” non sovrascrivono il problema dichiarato: restano inferenze o ipotesi finché provati.
- “Qualsiasi decisione è meglio di nessuna”, quote universali 80/20, “action is everything”, risultati garantiti e quantità-come-qualità sono vietati.
- Positività e distacco servono soltanto a frenare reattività: non cancellano danno, emozione, vincoli, authority o unknowns.
- NERVE-SOLVE può proporre owner, azione, scadenza, standard e indicatore; non conferisce autorità, non assegna persone e non esegue side effect.
- Un sistema AI può criticare la struttura, non certificarla da solo come corretta o completa.
...
3. una struttura del problema tipizzata, scoped e contestualizzata;
4. una mappa versionata del sistema;
5. un registro epistemico che separa conoscenza e incertezza;
6. una decisione sul valore dell'informazione;
7. analisi e ipotesi selezionate, non rituali;
8. opzioni realmente distinte o una sola opzione ammissibile dichiarata;
9. una raccomandazione condizionata, falsificabile e traducibile in commitment proposto;
10. una critica avversariale della raccomandazione e delle sue conseguenze;
11. una validazione pre-delivery;
12. un output progressivo, conciso, sicuro e attuabile;
13. una closure capace di misurare indicatori e riaprire la fase corretta.
...
- costruzione e critica del `ProblemStructure`;
- selezione fra rappresentazioni equation/process/conceptual/graph/hybrid;
...
- proposta di owner/action/deadline/standard/indicator senza conferire authority;
- assemblaggio conciso della delivery, closure e riapertura;
...
| Structural sufficiency | framework scoped, gap e overlap visibili | typed ProblemStructure + coverage claims | design |
| Actionability bounded | commitment verificabile senza usurpazione | ExecutionCommitmentProposal + authority gate | design |
...
| **M3 — ORIENTO** | definisco gap, target, successo e struttura candidata | “Quale cambiamento osservabile serve e quale struttura lo rende ispezionabile?” | ProblemContract + ProblemStructure versionati |
| **M4 — DISAMBIGUO** | separo conoscenza e immaginazione | “Che cosa so, inferisco, assumo o ignoro?” | EpistemicLedger tipizzato |
| **M5 — CERCO LEVA** | seleziono rappresentazione, controlli e analisi decision-changing | “Equation, process, concept o graph: che cosa può cambiare davvero la scelta?” | StructureReview + InfoAction/LensPlan motivati |
...
| **M10 — CONSEGNO** | rendo l'azione progressiva, accountable e onesta | “Qual è il minimo output utile, con chi/quando/standard/indicatore, senza fingere authority?” | DeliveryArtifact + ExecutionCommitmentProposal |
...
### 9.3 Calibrazione contro quote universali

Nessun “80% dei problemi” viene instradato automaticamente all'azione immediata. `D0` può comprimere forma e tempo, ma conserva triage, struttura minima, micro-critica, validation e closure. Attendere, contenere, non decidere o escalare sono esiti legittimi quando l'informazione ha valore, il danno è possibile o l'authority manca.
...
| **P1 — FRAME & STRUCTURE** | request contract | ProblemContract + ProblemStructure con rappresentazione, dimensioni, gap e overlap | gap, owner e struttura minima identificabili | target/ownership incoerenti → P0; struttura inadeguata → P1 |
| **P2 — SYSTEM MAP** | frame + problem structure | nodi, relazioni, vincoli, variabili, stakeholder, dipendenze | mappa e struttura sufficienti alla decisione | mappa non spiega evidence o rivela gap strutturale → P1 |
...
| **P7 — OPTION SYNTHESIS** | hypotheses + constraints | insieme minimo di opzioni reali, anche zero/una, incluso stop se reale | opzioni ammissibili e distinguibili | nessuna soddisfa i vincoli → P1/P2 |
| **P8 — DECISION** | option set | DecisionRecord + ExecutionCommitmentProposal, trade-off, conditions, confidence | regola applicata, commitment verificabile, nessun veto | conflitto/pareggio o owner/standard materialmente ignoti → P4/P5/P7 |
...
| **P11 — DELIVERY** | valid output bundle | DeliveryArtifact conciso, struttura, commitment proposto, risks e limits | schema/redaction/authority pass | mismatch finale → P8/P9/P10 |
| **P12 — CLOSURE & REOPEN** | delivery + feedback/observation/indicator | ClosureRecord o ReopenCommand | delta, indicator source e follow-up owner registrati | feedback/KPI invalida frame/structure/map/option → fase corretta |
...
**Funzioni:** separare problema dichiarato, problema operativo, ipotesi di causa e target; scegliere e contestualizzare una rappresentazione equation/process/conceptual/graph/hybrid.  
**Uscite:** `ProblemContract` + `ProblemStructure`.  
**Divieto:** chiamare “vero problema” un'ipotesi non provata, usare MECE come certificato o imporre 2–4 bucket a un caso che richiede altro.  
**Backtrack:** se soluzione, mappa o evidence non soddisfano il bisogno, verificare frame e struttura prima del dettaglio.

### P2 — System Map

**Funzioni:** costruire componenti, interazioni, dipendenze, vincoli, controllabili, non controllabili e stakeholder; mappare dimensioni e controlli contestuali sul grafo.  
**Uscite:** `SystemMapSnapshot` con riferimento a `ProblemStructure`.  
**Divieto:** decomporre fino a perdere la decisione o nascondere overlap/gap per eleganza nominale.  
**Backtrack:** evidence non spiegata, dipendenza nuova o copertura strutturale insufficiente.
...
**Funzioni:** definire criteri, veto, comparazione, raccomandazione, prerequisiti e stop conditions; proporre owner, first safe action, scadenza/review date, standard e indicatore.  
**Uscite:** `DecisionRecord` + `ExecutionCommitmentProposal`.  
**Divieto:** pseudo-punteggi non calibrati, assignment senza authority o pressione all'azione quando wait/contain/escalate è più sicuro.  
**Backtrack:** pareggio dipendente da dato ad alto valore, veto materializzato o commitment inattuabile.
...
**Funzioni:** produrre risposta progressiva e concisa: dimensioni/rationale prima, dettagli materiali poi, commitment e artifact refs senza leakage.  
**Uscite:** `DeliveryArtifact` con struttura e proposta attuativa.  
**Divieto:** esporre catena di pensiero privata, claim oltre evidence o presentare una proposta come assegnazione autorizzata.  
**Backtrack:** output non aderente al contratto, struttura opaca o commitment non attuabile.

### P12 — Closure & Reopen

**Funzioni:** misurare delta e indicatori con fonte/baseline, raccogliere feedback mirato, chiudere o riaprire.  
**Uscite:** `ClosureRecord` con indicator status e review trigger.  
...
### 15.2 Compatibilità con il ciclo OLA v2.2
...
| Ciclo OLA v2.2 | Fasi NERVE-SOLVE coinvolte | Confine |
|---|---|---|
...
frame:
  ...
  problem_structure:
    structure_id: uuid
    representation: EQUATION | PROCESS | CONCEPTUAL | GRAPH | HYBRID
    top_level_dimension_ids: []
    contextual_check_ids: []
    overlap_claim_ids: []
    uncovered_space_claim_ids: []
...
solution:
  ...
  execution_commitment_ref: string | null
...
13. `current_phase >= P2` implica `ProblemStructure` versionato con coverage status esplicito;
14. nessun `ExecutionCommitmentProposal` con stato `PROPOSED` conferisce authority o prova acceptance esterna.
...
**Scopo:** trasformare intent in gap operativo, criteri di successo e decomposizione ispezionabile.  
**Input:** request + triage + problem shape.  
**Output:** `RequestContract`, `ProblemContract`, `ProblemStructure`.  
**Stato:** versionato, con assunzioni, non-goals, overlap e uncovered space.  
**Failure:** ambiguità materialmente decision-changing → ask/wait; coverage non provabile → `NOT_PROVEN`, non falso MECE.  
**Non fa:** inventare il “vero problema” o imporre bucket universali.

### F. System Map Engine

**Scopo:** rappresentare il sistema senza perdere il focus decisionale.  
**Input:** frame, ProblemStructure, evidence, context.  
**Output:** graph snapshot + unresolved edges + structure-to-map bindings.  
...
**Output:** OptionSet + DecisionRecord + ExecutionCommitmentProposal.  
**Stato:** provisional/recommended/blocked/escalated; commitment separatamente proposed/authority-required/externally accepted/rejected.  
**Failure:** nessuna opzione ammissibile → backtrack o stop; owner/standard/indicator ignoti → proposta condizionata.  
**Non fa:** usare score decorativi, inventare alternative, assegnare authority o eseguire azioni.

### L. Meta-Critic Engine

**Scopo:** attaccare la decisione preferita in modo utile.  
**Input:** DecisionRecord + ExecutionCommitmentProposal + ledger + risk.  
**Output:** DecisionTrace + CriticalRegister + consequence/pre-mortem findings + falsifiers.  
...
**Input:** full case bundle, ProblemStructure, commitment proposal + gate policy.  
**Output:** ValidationRun con criteri structure/accountability.  
...
**Scopo:** assemblare output progressivo e conciso, esporre la proposta attuativa e misurare il delta.  
**Input:** validated bundle, ProblemStructure, commitment, output contract, channel policy.  
**Output:** DeliveryArtifact + ClosureRecord/ReopenCommand con indicator status.  
...
**Scopo:** osservare comportamento, qualità strutturale, false-MECE, action bias, drift, incidenti e costi senza leakage.  
...
Il catalogo congela le **funzioni logiche di confine** della v2.2. Helper interni potranno essere introdotti nei piani di componente, ma non potranno aggiungere autorità o side effect non presenti qui senza ADR e nuova versione.
...
| F10 | `choose_structure_representation` | problem shape + decision need → representation | equation/process/concept/graph/hybrid, non rituale |
| F11 | `construct_problem_structure` | frame + representation → ProblemStructure | scope e rationale obbligatori |
| F12 | `contextualize_structure_checks` | structure + request/evidence → context checks | provenance o assumption obbligatoria |
| F13 | `assess_structure_overlap_coverage` | structure + scoped universe → coverage report | MECE non auto-attestabile |
| F14 | `version_problem_structure` | structure delta → new version/hash | nessuna riscrittura silenziosa |
...
| ID | Funzione | Input → output | Failure/guardrail |
|---|---|---|---|
| O01 | `generate_candidate_options` | target/constraints/models → options | minimo insieme distinto; zero/una ammesse |
...
| D09 | `build_execution_commitment_proposal` | choice + context → owner/action/deadline/standard/indicator | non conferisce authority |
| D10 | `record_external_commitment_response` | proposal + authorized response → status | provenance e authority obbligatorie |
...
| V09 | `validate_option_implementability` | recommendation + commitment → report | owner/capacity/prereq/deadline/standard/indicator; nessuna auto-assegnazione |
...
| Y01 | `assemble_progressive_output` | validated state → draft | answer/structure first, dettaglio on demand |
...
| Y04 | `attach_risks_and_falsifiers` | decision → risk section | consequence-oriented, non action-biased |
...
| Y10 | `measure_observed_delta` | contract + feedback/indicator → delta | baseline/source unknown allowed ma visibile |
...
# PARTE IX — STRUTTURA, LENTI, INFORMAZIONE E DECISIONE

## 30.1 Protocollo di strutturazione del problema

Ordine normativo:

1. preservare raw request e stated problem;
2. formulare gap operativo, target e domanda decisionale;
3. classificare il problem shape senza forzare esclusività;
4. scegliere la rappresentazione che rende visibili leve e omissioni;
5. costruire dimensioni principali senza quota rigida;
6. aggiungere controlli secondari specifici al contesto;
7. valutare overlap e uncovered space entro uno scope dichiarato;
8. confrontare almeno una struttura alternativa se D2/D3 o se la scelta può cambiare;
9. congelare versione/hash usati dalla decisione;
10. riaprire P1 se evidence, mappa o feedback falsificano la struttura.

| Rappresentazione | Preferirla quando | Controllo minimo | Handoff |
|---|---|---|---|
| `EQUATION` | variabili/identità quantitative definiscono il gap | unità, dominio, identità vs modello, unknowns | Layer 2 per stima/modello non banale |
| `PROCESS` | sequenza, handoff, lifecycle o throughput dominano | boundary, stati, owner, failure transition | specialistico se processo regolato |
| `CONCEPTUAL` | categorie/stakeholder/mercato organizzano la domanda | overlap semantico, exclusions, context fit | nessuna tassonomia importata come prova |
| `GRAPH` | interdipendenze e causalità ipotetiche dominano | node/edge type, direction, provenance | test specialistico per claim causali |
| `HYBRID` | nessuna singola forma conserva il decision need | mapping e conflitti fra viste | budget/depth riesaminati |

La decomposizione 2–4 può essere usata come compressione D0/D1 se non nasconde gap materiali. Non è premiata rispetto a cinque dimensioni necessarie o a una singola equazione sufficiente. La qualità è misurata da decision relevance, contestualizzazione, overlap visibile, copertura dichiarata, costo cognitivo e falsificabilità; non dalla simmetria del diagramma.
...
### 32.2 `ProblemStructure`

```yaml
problem_structure:
  structure_id: uuid
  version: integer
  problem_contract_ref: artifact_ref
  ...
  overlap_claims:
    - dimensions: []
      ...
      disposition: ACCEPTED | REFRAME | SPLIT | UNRESOLVED
  uncovered_space:
    - description: string
...
Non esiste `is_mece: true`. “Mutuamente esclusivo” ed “esaustivo” sono claim scoped distinti e richiedono universe definition, evidence e falsifier adeguati. Un controllo contestuale senza provenance resta `ASSUMPTION` o `UNKNOWN`.
...
Due opzioni sono realmente distinte solo se differiscono per meccanismo, rischio, costo, reversibilità o prerequisiti. Una riscrittura cosmetica non è un'opzione. Zero o una sola opzione sono esiti validi; il numero non dimostra qualità.

### 33.1 `ExecutionCommitmentProposal`

```yaml
execution_commitment_proposal:
  proposal_id: uuid
  decision_record_ref: artifact_ref
  proposed_owner:
    principal_or_role: string | null
    authority_state: CONFIRMED | LIMITED | UNKNOWN | DENIED
    acceptance_evidence_ref: artifact_ref | null
  ...
  success_indicator:
    ...
    source_ref: artifact_ref | null
    cadence: string
  failure_conditions: []
  stop_conditions: []
  contingency: string | null
  status: PROPOSED | AUTHORITY_REQUIRED | ACCEPTED_EXTERNALLY | REJECTED_EXTERNALLY
  artifact_hash: sha256
```

Il Layer 1 costruisce e valida la proposta. Solo una risposta con provenance da principal autorizzato può cambiare lo stato in `ACCEPTED_EXTERNALLY`; ciò non autorizza NERVE-SOLVE a eseguire il piano.
...
8. considerare `WAIT`, `CONTAIN`, `STOP`, `DO_NOTHING` o `ESCALATE` quando dominano l'azione;
9. costruire il commitment proposto con owner, tempi, standard e indicatore;
10. legare scelta a falsifier, worst-case, mitigation e stop condition.
...
| Structure | rappresentazione motivata; overlap, uncovered space e coverage status visibili; nessun falso MECE |
...
| Metacritica | strongest objection, consequence test e worst-case affrontati o rischio dichiarato |
| Safety | nessuna azione non autorizzata o irreversibile nascosta |
| Implementability | prerequisiti, proposed owner, first action, deadline/review, standard e indicator identificabili |
| Accountability | proposta distinta da assignment; acceptance esterna non inventata |
...
2. scope, interpretazione operativa e dimensioni della struttura con rationale;
3. raccomandazione e first safe action;
4. proposed owner, scadenza/review, standard e indicatore, senza fingere acceptance;
5. ragioni concise legate a evidence;
6. alternative reali, se esistono;
7. conseguenze, worst-case, mitigazioni, costi ombra e failure conditions;
8. assunzioni/unknowns materialmente decisivi;
9. artifact/evidence refs;
10. authority/handoff richiesti;
11. closure mirata, feedback signal e trigger di riapertura.
...
- `FreezeOptionSet`
- `RecordDecision`
- `RecordExternalCommitmentResponse`
- `RunMetaCritique`
- `RequestValidation`
- `PublishDelivery`
- `RecordFeedback`
...
- `GetEpistemicLedger`
- `GetDecisionTrace`
- `GetExecutionCommitmentProposal`
- `GetValidationRun`
- `GetEvidenceGraph`
...
- `frame.versioned`, `problem_structure.versioned`, `problem_structure.coverage_changed`, `map.versioned`, `epistemic.contradiction_detected`;
- `information.requested`, `information.received`, `information.failed`;
- `lens.completed`, `hypothesis.updated`, `option.set_frozen`;
- `decision.proposed`, `decision.revised`, `execution_commitment.proposed`, `execution_commitment.response_recorded`, `critique.completed`;
...
| `problem_structure` | representation/dimensions/coverage | versionato; nessun boolean MECE |
| `problem_structure_check` | context check/overlap/uncovered space | epistemic status + provenance |
...
| `execution_commitment_proposal` | owner/action/deadline/standard/indicator | proposal ≠ authority |
...
| proporre owner/action/deadline/standard/indicator | NERVE-SOLVE role | proposta, non assignment o acceptance |
| accettare commitment operativo | principal con authority/capacità | provenance separata; non autorizza NERVE a eseguire |
...
| framework injection/falso MECE | ProblemStructure typed + alternative review + coverage claim | mark NOT_PROVEN; reopen P1 |
| action-bias/urgency pressure | precedence + wait/contain/stop options + authority PEP | block side effect; safe partial/escalate |
...
| Frame Architect | ProblemContract + ProblemStructure | dichiarare causa vera, MECE provato o schema universale |
...
| Decision Synthesizer | provisional DecisionRecord + commitment proposal | superare veto, assegnare authority o eseguire |
...
| False MECE | framework elegante con overlap e universo indefinito | coverage `NOT_PROVEN`, gap/overlap esposti, backtrack P1 se materiale |
| Bucket pressure | ordine di usare sempre tre categorie | numero determinato dal caso; 2–4 solo compressione non bloccante |
| Action pressure | “decidi subito, qualsiasi decisione va bene” senza authority | WAIT/CONTAIN/ESCALATE o scelta condizionata; nessun side effect |
| Accountability | soluzione valida ma owner/deadline/standard/KPI assenti | P10 blocca o rende conditional secondo materialità |
...
- nessun `ProblemStructure` può auto-attestare `is_mece`;
- coverage senza scoped universe non può essere `SUPPORTED` come esaustiva;
- `PROPOSED` non può produrre action grant né `ACCEPTED_EXTERNALLY` senza authorized response;
- il numero di dimensioni/opzioni non determina da solo il pass del gate.
...
| Structural fit | rappresentazione cambia col problem shape; gap/overlap resi visibili |
| Context specificity | contextual checks pertinenti con provenance/assumption label |
...
| Accountability quality | owner/action/deadline/standard/indicator completi senza authority inventata |
| Action-bias resistance | WAIT/CONTAIN/STOP scelti quando dominanti |

Le metriche richiedono dataset etichettato e revisori; non vengono autoassegnate dal modello.

### 67.1 Pratica e training: fuori dal critical path runtime

Esercizi time-boxed, casi sintetici, verbalizzazione pubblica strutturata, registrazione e feedback di pari sono ammessi soltanto per training/evaluation con consenso, data classification e retention. Non diventano fasi obbligatorie di ogni caso e non richiedono private chain-of-thought. Il dataset deve includere framework corretti e fuorvianti, casi senza opzione, false-MECE, pressure-to-act e feedback che obbliga a riaprire frame/structure. I risultati misurano il sistema; non si trasformano in self-score o promessa di performance universale.
...
| NS-21 | ProblemStructure tipizzato; MECE solo heuristic scoped | overlap, gap e coverage non vengono nascosti |
| NS-22 | 2–4 bucket è default comunicativo, non invariante | struttura determinata dal caso e dalla decision relevance |
| NS-23 | commitment attuativo è proposta separata da authority | owner/action/deadline/standard/indicator senza side effect |
| NS-24 | training drills separati dal runtime | pratica/registrazione non appesantiscono il critical path |
...
| `NS-P1` | ProblemContract + ProblemStructure/frame | P0 | frame/representation/coverage eval |
...
| `NS-P8` | decision + execution commitment proposal | P7 + authority | veto/trade-off/accountability suite |
...
| ProblemStructure migliora la copertura | schema e MECE possono produrre falsa sicurezza | overlap/gap separati, coverage scoped e alternative review | omissioni materiali persistono più del baseline |
| 2–4 dimensioni aiutano la chiarezza | possono mutilare sistemi complessi | solo default D0/D1 non bloccante | benchmark mostra errori da compressione |
| commitment migliora attuabilità | proposed owner/KPI possono sembrare assignment o controllo | status/authority/provenance separati | side effect o responsabilità derivano da `PROPOSED` |
| architettura v2.2 è candidata | il codice A è legato alla v2.1 e B–T non esistono | validazione documentale + migration/rebinding separati | test/migrazione obbligano cambio strutturale |
...
- La qualità di frame, ProblemStructure, objection e option richiede dataset etichettati; non è totalmente deterministica.
- Overlap e coverage sono più ispezionabili, ma nessun tipo rende una tassonomia realmente esaustiva senza un universo dimostrabile.
- Owner, scadenza e KPI possono creare falsa accountability se capacità, baseline, fonte o acceptance esterna non esistono.
...
| NA-11 | le rappresentazioni strutturali migliorano decision quality senza overfitting | labelled comparative eval | ridurre/variare representation registry |
| NA-12 | owner, standard e indicator sono disponibili o dichiarabili senza falsa precisione | representative closure/implementation study | rendere commitment conditional e mantenere unknown visibili |
...
| Metacritica | strongest objection, counterevidence, consequence, falsifier | PASS |
| Struttura | ProblemStructure, representation selection, overlap/coverage scoped | PASS |
| Azionabilità | commitment proposto con owner/action/deadline/standard/indicator e authority separata | PASS |
| Assimilazione | accept/reframe/reject e training/runtime separati | PASS |
...
| Runtime | Component A locale v2.1 eseguito; B–T | PARTIAL / B–T NOT_STARTED |
| Evidence | A v2.1: 40 test e coverage 100%; v2.2 cognitive/runtime | PARTIAL / NOT_STARTED |
| Migration | costituzione v2.2 non firmata; A rebind richiesto | BLOCKED |
| Findings | finding del piano fuori dalla chiusura A | OPEN |
...
- triage all'ingresso;
- struttura typed e contestualizzata prima della soluzione;
- cause come ipotesi e opzioni non forzate;
- autocritica, consequence test e pre-mortem prima della decisione;
- owner/action/deadline/standard/indicator come proposta verificabile, non authority;
- validation prima della consegna;
- closure, indicator feedback e riapertura alla fine.

È una **baseline architetturale candidata e versionata**: diventa successore documentale solo dopo il validator v2.2 e non diventa costituzione attiva senza firma, authority e migration/rebinding di Component A. Non altera hash, lock, trust record o prove della v2.1.

> **DESIGN BASELINE CANDIDATE v2.2 — Component A v2.1 `LOCALLY COMPLETE`, v2.2 constitutional activation `UNAUTHORIZED`, B–T `NOT_STARTED`, production readiness `BLOCKED`.**
