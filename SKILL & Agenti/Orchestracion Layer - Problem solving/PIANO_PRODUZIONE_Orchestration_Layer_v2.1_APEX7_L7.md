# Orchestration Layer Architect v2.1 — Piano di produzione APEX-7 · Livello 7 definitivo
...
**Piano genitore:** [`PIANO_PRODUZIONE_Orchestration_Layer_v2.1_APEX7_L6.md`](PIANO_PRODUZIONE_Orchestration_Layer_v2.1_APEX7_L6.md)  
**Livello:** **L7/7 — baseline esecutiva, finanziabile e governata per commitment**  
**Regola di versione:** questo file eredita integralmente L6 e introduce un solo miglioramento tematico finale.
...
| Catena identità/autorità | **DESIGN CANDIDATE; nessuna attestazione o prova di revoca eseguita** |
| Baseline esecutiva L7 | **DEFINITA; NON FIRMATA, NON FINANZIATA, NON IMPEGNABILE** |
...
| **P0-14** | `V-P0-14-CLEAN-BUILD` | clone pulito, lock verificato, import/type/test/build/SBOM riproducibili | EL4 per TPE-1 L6; EL2 solo baseline statica |
...
### Registro del Livello 6 — unico miglioramento introdotto

**Tema:** rendere verificabile la catena end-to-end tra identità, autorità, artifact provenance e rilascio, senza assumere che un ruolo dichiarato, una firma o la rete interna implichino fiducia.  
**Non modificato in L6:** scope TPE-1, effort e DAG L1–L2, scope contract L3, evidence policy L4, failure containment L5, runtime `asyncio`/PostgreSQL e ruolo opzionale di RuFLO.  
**Perché serve:** L5 sa fermare un sistema degradato, ma non dimostra ancora chi possa impartire il comando, quale identità abbia prodotto l'artefatto, se la delega sia valida o se la firma provenga da una chain of trust approvata.

#### 1. Autopsia critica del piano L5

L5 contiene JWT, tool grants, firme, SBOM, separazione dei compiti e kill switch. Non costituiscono ancora un modello di autorità eseguibile:

- non esiste un catalogo normativo di principal umani, workload, builder, agenti e signer;
- autenticazione e ruolo nominale possono essere scambiati per autorizzazione;
- la firma prova integrità/origine rispetto a una chiave, non correttezza, sicurezza o legittimità della chiave;
- non sono fissati audience, scope, TTL, delega, revoca e freshness delle decisioni privilegiate;
- build, evidence, gate e release potrebbero dipendere dallo stesso principal compromesso;
- prompt, modello, tool, adapter e dipendenze sono supply chain, ma il piano non lega ancora i loro digest al manifest;
- il break-glass non distingue con sufficiente rigore l'autorità di **spegnere** da quella di **riattivare o ampliare**;
- per provider SaaS non controllati è impossibile attestare l'intera implementazione interna: il piano deve registrare opacità, non inventare provenance.

Conclusione: un attaccante con credenziali valide o un CI compromesso potrebbe produrre una catena apparentemente firmata senza avere l'autorità sostanziale per il rilascio.

#### 2. `AuthorityTrustContract`

Ogni TPE-1 attivabile deve referenziare un contratto immutabile e firmato:

```text
AuthorityTrustContract:
  authority_contract_id, version, authority_policy_hash, owner
  scope_hash, failure_policy_hash
  trust_domain_ids[], trust_root_refs[], accepted_issuers[]
  principal_catalog_hash, action_resource_catalog_hash
  authority_matrix_hash, separation_policy_hash
  delegation_rules, maximum_delegation_depth
  credential_profiles[], decision_freshness_rules[]
  build_provenance_policy, artifact_signing_policy
  model_prompt_tool_adapter_provenance_policy
  revocation_sources[], revocation_objectives[]
  break_glass_policy_hash
  verification_case_ids[], drill_evidence_ids[]
  approved_at, expires_at, invalidation_keys[]
```

Il contratto non contiene secret o private key. Se manca, è scaduto, non è legato allo `scope_hash`, o il trust root non è verificabile:

- nessun nuovo side effect C1;
- nessuna promozione o nuova release;
- nessuna espansione di scope;
- recovery e containment restano possibili attraverso autorità out-of-band predefinite.

#### 3. Principal model: identità non equivale ad autorità

| Principal | Identità richiesta | Autorità massima iniziale | Divieto strutturale |
|---|---|---|---|
| Human developer | identità individuale, sessione forte | propose/change in non-prod | firma release o accesso diretto ai dati prod non necessario |
| Human reviewer | identità individuale separata | verify/review | approvare il proprio change critical |
| Release approver | identità individuale con elevation breve | approve manifest scoped | modificare artifact/evidence già approvati |
| CI builder | workload identity attestata | build + provenance | approvare o deployare |
| Evidence runner | workload identity attestata | eseguire case e firmare raw result | cambiare oracle/policy durante il run |
| Gate Policy Engine | workload identity dedicata | calcolare decisione deterministica | modificare evidence o rilasciare |
| Release controller | workload identity dedicata | verificare manifest e orchestrare deploy | creare artifact o auto-approvarsi |
| Runtime worker | workload identity per ambiente | claim/esecuzione entro TPE-1 | leggere signing key, policy authoring o DB cross-scope |
| Logical agent profile | identità logica + host attestato + delega | tool grant temporaneo | possedere secret statici o creare nuova autorità |
| Adapter C1 | workload identity dedicata per adapter/scope | singola capability esterna | accesso generico a DB, release o altri adapter |
| Incident Commander | identità umana elevata e auditata | containment/break-glass scoped | trasformare emergenza in `PASS` o espansione permanente |
| RuFLO adapter | workload identity sandbox separata | builder coordination approvata | DB autorevole, KMS signer, release o C1 runtime |

Regole:

1. un nome di ruolo nel prompt, nel codice o nell'output LLM non è una credenziale;
2. account condivisi sono vietati per azioni privilegiate;
3. l'agent eredita al massimo l'intersezione dei grant espliciti dell'host e della delega corrente;
4. identità dev, staging e prod appartengono a trust domain separati o a boundary equivalenti provati;
5. l'appartenenza alla rete interna non concede fiducia implicita;
6. l'identità autentica il subject; la policy decide se quel subject può eseguire **quell'azione, su quella risorsa, in quello scope e in quel momento**.

#### 4. Authority lattice e separazione non collassabile

```text
EffectiveGrant = IdentityGrant
               ∩ ScopeGrant
               ∩ PolicyGrant
               ∩ DelegationGrant
               ∩ ArtifactManifestGrant
               ∩ RuntimeModeGrant
               ∩ TenantRegionGrant
               ∩ TimeBoundGrant
```

Per ogni azione la policy dichiara quali dimensioni sono obbligatorie; una dimensione realmente non applicabile vale come insieme universale, non come dato mancante. Qualsiasi dimensione **richiesta** ma assente, non fresca o non verificabile produce `DENY/NOT_PROVEN`, mai un allow implicito. `STOP/CONTAIN` out-of-band segue la policy emergenziale dedicata e non dipende da un artifact manifest del runtime compromesso.

| Azione | Può proporre | Può verificare | Può approvare/eseguire | Vincolo non delegabile |
|---|---|---|---|---|
| Cambiare workflow/prompt/tool profile | Builder/owner | test + reviewer | owner scoped | il proponente non chiude il proprio gate critical |
| Cambiare gate policy/oracle | Quality/Security | independent verifier | Security + Tech Lead | nessun LLM approva la propria policy |
| Produrre artifact eseguibile | CI builder | provenance verifier | nessuno | builder non firma la release authority |
| Chiudere finding P0 | remediation owner | independent verifier | finding authority | evidence fresca EL richiesta |
| Firmare release manifest | release preparer | policy engine + verifier | release approver | artifact, scope, evidence e policy hash immutabili |
| Deployare | release controller | admission/release PEP | controller su manifest già approvato | nessun deploy da workstation personale |
| Invocare side effect C1 | runtime workflow | runtime PEP | adapter scoped | artifact+scope+mode+decision tutti validi |
| Arrestare/containment | monitor/on-call | audit successivo | IC/SRE autorizzato | può fermare, non dichiarare il sistema sano |
| Riattivare/espandere | recovery owner | independent verification | due autorità previste dal contratto | mai implicito dal ritorno del servizio |

Né la moltiplicazione di firme né una firma “senior” sana una violazione della separazione. Due approvazioni dello stesso principal o della stessa credenziale compromessa contano come una sola failure domain.

#### 5. Decisione ed enforcement per richiesta privilegiata

Ogni boundary privilegiato usa un Policy Decision Point e un Policy Enforcement Point. La tecnologia concreta resta una decisione F0; il contratto minimo è:

```text
AuthorizationRequest:
  subject_id, workload_identity, delegated_by
  action, resource_id, tenant_id, region
  scope_hash, artifact_hash, tool_profile_hash
  environment, runtime_mode
  identity_issuer, audience, authn_strength
  request_id, nonce, issued_at, expires_at

AuthorizationDecision:
  decision_id, ALLOW|DENY|NOT_PROVEN
  authority_policy_hash, matched_rule_ids[]
  obligations[], maximum_uses
  issued_at, expires_at, revocation_epoch
  signer_identity, signature
```

Regole di enforcement:

- PEP non bypassabile davanti a release, policy mutation, evidence closure, secret access, tool invocation privilegiata e side effect C1;
- una decisione è legata a subject, azione, risorsa, scope, artifact e ambiente; non è un bearer grant generico;
- replay oltre `maximum_uses`, audience errata o nonce riusato viene rifiutato;
- gli `ALLOW` cached hanno TTL per azione; release, nuova elevation e C1 non usano cache oltre la revocation freshness approvata;
- PDP/policy/revocation source non attendibile segue D0/L5: fail-closed per mutazioni e C1;
- ogni decisione privilegiata registra `decision_id`, policy hash ed enforcement outcome nell'audit autorevole;
- il PEP non accetta una decisione generata dallo stesso artifact non ancora approvato.

#### 6. Delega agentica e tool capability

La delega a un agent è un capability grant corto, non un'estensione della personalità dell'utente:

```text
AgentCapabilityGrant:
  grant_id, parent_decision_id
  agent_profile_hash, host_workload_identity
  task_id, allowed_tools[], allowed_actions[]
  resource_patterns[], tenant_id, scope_hash
  network_egress_policy_hash, filesystem_policy_hash
  maximum_uses, budget, issued_at, expires_at
  delegable: false by default
  signer, signature
```

- nessuna ambient cloud credential nel processo agentico;
- output, memoria o messaggio di un altro agent non possono ampliare il grant;
- il tool PEP verifica il grant a ogni invocazione, non soltanto all'avvio del task;
- le sotto-deleghe sono vietate salvo allowlist e depth cap nel contratto;
- prompt injection che chiede un nuovo ruolo non cambia `EffectiveGrant`;
- RuFLO può coordinare task soltanto con grant dedicati, esportabili e revocabili; non riceve credential runtime/release;
- una response LLM non è una decisione di autorizzazione anche quando contiene testo simile a una policy.

#### 7. Credential, key e revocation lifecycle

Target iniziali — da validare economicamente e tecnicamente, non già raggiunti:

| Oggetto | Regola iniziale | Revoca/freshness target | Failure sicura |
|---|---|---:|---|
| Workload credential | short-lived, audience e ambiente specifici | propagazione revoca ≤ 5 min | stop nuovi grant/claim privilegiati |
| Agent tool grant | task-bound, non-forwardable | expiry ≤ 15 min o fine task | tool call negata |
| Human privileged elevation | MFA forte, just-in-time | expiry ≤ 15 min | nuova elevation richiesta |
| Release authorization | one-shot, manifest-bound | una sola promozione | replay negato |
| Artifact/provenance signer | KMS/HSM o identità effimera equivalente | emergency distrust ≤ 5 min | release freeze |
| Break-glass stop grant | scope e comando limitati | expiry ≤ 15 min | nessuna riattivazione implicita |
| JWT/JWKS trust | issuer/audience/alg pin + rotation | refresh entro budget approvato | fail-closed per mutate/C1 |

Le signing key non sono disponibili ai build step controllati dall'utente. Dev/staging/prod non condividono key material. Rotazione, revoca e trust-bundle update sono provati con clock skew e outage. Una firma creata durante una finestra di compromissione viene quarantinata; non si presume valida perché crittograficamente verificabile.

#### 8. Catena artifact → evidence → release → runtime

```text
source commit/tree digest
→ dependency lock + SBOM digest
→ isolated build identity + build definition hash
→ executable artifact digest
→ build provenance attestation
→ verification case/test implementation/environment hashes
→ signed EvidenceRecord bundle
→ deterministic GateRun
→ scoped approval identities
→ signed release manifest
→ deploy record
→ runtime loaded-artifact attestation
```

Ogni freccia deve essere verificabile e non ambigua. Il release PEP ricalcola digest e policy; non si limita a controllare “firma presente”. Il runtime carica soltanto artifact il cui digest compare nel manifest attivo. In L6, `runtime loaded-artifact attestation` significa un record di load firmato e verificabile; non implica hardware attestation, secure boot o TEE se tali proprietà non sono contrattualizzate e provate separatamente.

Regole supply-chain:

- dipendenze e tool di build sono pinati per digest/versione e sottoposti a vulnerability/licence policy approvata;
- build prod su runner ospitato e isolato; nessun rilascio da laptop;
- provenance prodotta dal control plane di build, non compilata a mano dal job non fidato;
- target provvisorio per artifact eseguibili: requisiti equivalenti a **SLSA Build L3**; nessun claim di conformità finché non verificato;
- Sigstore/Cosign, KMS signing o alternative equivalenti sono opzioni, non dipendenze obbligatorie; la verification policy è il requisito;
- SPIFFE/SPIRE o workload identity cloud-native sono opzioni; non viene imposto un prodotto prima del make/buy F0;
- un SBOM senza artifact digest, builder identity e verification policy è inventario, non trust chain;
- una signature valida non dimostra assenza di vulnerabilità né correttezza semantica.

#### 9. Provenance di prompt, modelli, tool e adapter

| Oggetto | Binding minimo | Limite dichiarato |
|---|---|---|
| Prompt | content hash, template version, owner, eval version, tool profile hash | il prompt non è enforcement |
| Agent profile | profile hash, runtime/harness version, grant policy | identità logica non è workload identity |
| Modello self-hosted | model/weights digest, runtime image, config, tokenizer | digest non prova qualità o safety |
| Modello SaaS | provider, endpoint, model identifier riportato, region, params, request/response refs | internals e alias possono restare opachi |
| Tool | binary/image digest, schema, permission/network policy | schema valido non prova tool benigno |
| Adapter | source/build digest, downstream contract, credential profile | downstream può produrre esito ambiguo |
| RuFLO | version/tag/digest, install provenance, MCP/CLI contract, sandbox policy | coordinamento non diventa autorità |
| Dataset/eval | dataset hash, lineage, licence/consent, redaction policy | hash non prova rappresentatività |

Se il provider non espone una versione immutabile, ogni drift osservato invalida l'evidence interessata e blocca una nuova promozione. Non si attribuisce SLSA a una response SaaS o a un alias di modello non controllabile.

#### 10. Trust-zone e flussi ammessi

| Zona | Contenuto | Può chiamare | Non può chiamare direttamente |
|---|---|---|---|
| Z0 Untrusted | input utente, web/tool output, memory non attestata | intake/sanitizer | KMS, DB autorevole, release |
| Z1 Builder sandbox | agenti, RuFLO opzionale, tool dev | artifact/evidence staging via port | runtime prod, signer prod, adapter C1 |
| Z2 Verification | runner e oracle attestati | evidence store append-only | modificare source/artifact sotto test |
| Z3 Release control | PDP/PEP, manifest, approvazioni | deploy interface scoped | generazione artifact, business side effect |
| Z4 Runtime authoritative | worker, PostgreSQL, outbox/reconciler | adapter approvati | builder memory, release signing key |
| Z5 External effect | adapter/downstream TPE-1 | contratto esterno allowlisted | altri tenant/adapter/control plane |
| Z6 Incident OOB | kill switch, emergency identity, audit | containment endpoints | auto-PASS, artifact mutation, scope expansion |

Ogni attraversamento usa schema, identity, authorization decision e audit. Una network policy riduce superficie ma non sostituisce il PEP applicativo.

#### 11. Compromise-to-revocation matrix

| Compromissione | Containment immediato | Oggetti da invalidare/quarantinare | Prova prima della riapertura |
|---|---|---|---|
| Account developer/reviewer | revoke session/elevation | approval/change nella finestra | identity audit + independent re-review |
| CI runner/job token | freeze build/release | artifact/provenance prodotti | clean isolated rebuild + digest comparison |
| Provenance/release signer | distrust key/identity | manifest e attestazioni nella finestra | root rotation + timeline + re-sign da clean chain |
| PDP/policy store | `PAUSED`, freeze C1/release | decisioni non verificabili | policy restore + deterministic authz replay |
| IdP/trust root | `EMERGENCY_CONTAINMENT`, distrust issuer, freeze C1/release | credential, decisioni e firme nella finestra | root recovery indipendente, reissue e re-verifica completa |
| Runtime workload identity | stop claim/adapter scope | decisioni e effects associati | re-attestation + lease/effect reconciliation |
| Agent tool grant | revoke grant/profile | artifact e side effect del task | scope audit + adversarial rerun |
| Evidence runner/signer | freeze closure | evidence prodotta nella finestra | rerun su EL richiesto con verifier indipendente |
| Dependency registry/package | pin/deny digest | build dipendenti | clean resolver, SBOM diff, vulnerability review |
| LLM/model provider | stop affected generation | artifact non ancora promossi | provider/region/model review + re-eval |
| RuFLO/plugin | `KS-RUFLO`, remove grants | output/tool calls della finestra | sandbox rebuild + artifact revalidation |
| Break-glass identity | revoke emergency channel | comandi eseguiti | forensic review + second-authority closure |

La recovery del credential service non ripristina automaticamente fiducia negli artifact creati durante la finestra sospetta.

#### 12. Break-glass asimmetrico

Il break-glass è diviso in due classi:

- **STOP/CONTAIN:** può essere eseguito da un Incident Commander autorizzato per ridurre danno; comando allowlisted, scope minimo, TTL breve, audit sincrono o buffer OOB;
- **START/EXPAND:** richiede recovery evidence fresca e le autorità indipendenti previste; non eredita l'urgenza dello stop.

Obblighi:

1. reason code, incident ID, principal, comando, scope, timestamp e scadenza;
2. notifica immediata e review indipendente entro 24 ore;
3. auto-expiry senza rinnovo silenzioso;
4. nessun break-glass può produrre `PASS`, cancellare audit/evidence, cambiare history, superare tenant boundary o firmare un artifact non verificato;
5. se l'audit OOB fallisce, lo stop resta consentito per prevenire danno imminente ma la riattivazione resta bloccata;
6. ogni uso riapre i finding/gate impattati secondo L4 e L5.

#### 13. Verification matrix L6

Questi case raffinano finding esistenti; **non** aggiungono finding né li chiudono. Per TPE-1 L6, **P0-14 parte della release authority chain**: build e provenance determinano quale artifact possa essere autorizzato al rilascio. Di conseguenza richiede EL4; EL2 resta soltanto la baseline statica di clean build e non prova l'autorità di release.

| Case | Attacco/failure | Oracle bloccante | Livello minimo |
|---|---|---|---|
| `V-AT-01-SHARED-IDENTITY` | due attori tentano di usare lo stesso principal privileged | shared principal rifiutato; sessioni attribuibili univocamente | EL3 |
| `V-AT-02-CROSS-ENV` | identity dev tenta prod | 100% deny + audit | EL4 |
| `V-AT-03-REVOKED-CREDENTIAL` | credential revocata durante sessione | deny entro target, nessun nuovo C1 | EL4 |
| `V-AT-04-DELEGATION-ESCALATION` | agent/handoff amplia grant | `EffectiveGrant` non cresce | EL4 |
| `V-AT-05-PDP-OUTAGE` | PDP/revocation source indisponibile | fail-closed mutate/C1, containment disponibile | EL4 |
| `V-AT-06-POLICY-DOWNGRADE` | policy meno restrittiva non approvata | hash mismatch → block | EL4 |
| `V-AT-07-UNSIGNED-SWAP` | artifact sostituito dopo evidence | digest/manifest mismatch → block | EL4 |
| `V-AT-08-PROVENANCE-FORGE` | build job tenta attestazione falsa | provenance non producibile dal tenant job | EL4 |
| `V-AT-09-BUILD-TO-RELEASE` | CI compromesso tenta deploy | builder non possiede release authority | EL4 |
| `V-AT-10-SIGNER-COMPROMISE` | key/identity distrust | quarantine e revalidation complete | EL4 |
| `V-AT-11-AGENT-RUFLO-ESCALATION` | prompt/plugin chiede DB/release/C1 | grant e network PEP negano | EL4 |
| `V-AT-12-BREAK-GLASS-REPLAY` | riuso/estensione emergency grant | one-shot/TTL/scope rispettati | EL4 |
| `V-AT-13-RUNTIME-LOAD` | runtime riceve digest non manifestato | start/load negato | EL4 |
| `V-AT-14-SAAS-MODEL-DRIFT` | alias/provider cambia comportamento/versione | evidence invalidata; promotion freeze | EL3/EL4 per impatto |

#### 14. Metriche e soglie iniziali

- privileged actions con principal individuale/workload univoco = 100%;
- shared privileged principal in prod = 0;
- runtime/release long-lived static credential = 0; un'eventuale eccezione resta bloccante finché non viene rimossa o il TPE-1 non viene ridotto in modo da escluderla;
- decisioni privilegiate con `decision_id + authority_policy_hash + scope_hash` = 100%;
- artifact eseguibili con chain source→build→evidence→manifest→runtime completa = 100%;
- deploy di digest non manifestato = 0;
- agent tool call fuori grant = 0 riuscite nella suite avversariale;
- revoca credential critical entro target = 100% nei drill;
- orphan grant oltre expiry = 0;
- separation-of-duty violations = 0;
- break-glass senza review entro 24 h = 0.

“0 osservati” non equivale a probabilità zero; i controlli devono impedire by construction, mentre i test cercano bypass e regressioni.

#### 15. Impatto su effort, capacità e forecast

La baseline TPE-1 **45–63 settimane** resta difendibile solo se W1/W4/W7/W10 includono già:

- identity provider e workload identity per non-prod/prod;
- PDP/PEP o enforcement equivalente;
- KMS/HSM o signing identity con revoca;
- CI ospitata/isolata e provenance generata dal control plane;
- artifact registry immutabile e deploy verification;
- Security/Platform capacity per policy, incident e drill.

Se queste capability devono essere costruite o integrate da zero, aggiungere provvisoriamente:

- **15–30 person-week**;
- **0,5–1,0 FTE Security/Platform specialist** nei package W1/W4/W7/W10;
- **5–10 settimane calendario** prima di G7, poi ricalcolare il CPM.

La contingency L5 `+6–12` e quella L6 `+5–10` possono sovrapporsi su KMS, EL4 e incident control. Non si sottrae overlap senza evidenza. Se entrambe le baseline mancano, la riserva prudente lorda è **+11–22 settimane**; W0 deve costruire il DAG aumentato prima di qualunque commitment.

#### 16. Registro critico pubblico L6

| Affermazione ereditata | Obiezione più forte | Decisione L6 | Condizione di smentita |
|---|---|---|---|
| una firma rende trusted l'artifact | può essere la firma giusta su artifact sbagliato o da signer compromesso | verification policy + chain completa | prova dimostra una root/meccanismo più forte e equivalente |
| JWT valido autorizza | autenticazione non definisce action/resource/scope | decisione context-bound al PEP | formal model dimostra equivalenza senza escalation |
| due approvatori garantiscono indipendenza | possono condividere account, failure domain o conflitto | principal univoci + separation policy | controllo organizzativo/tecnico equivalente provato |
| SLSA/Sigstore/SPIFFE risolvono il trust | sono standard/meccanismi, non correctness né authority business | target di requisiti, prodotto non imposto | piattaforma scelta passa tutti i case L6 |
| il provider LLM dichiara la versione | alias e backend possono restare opachi | registrare limite e invalidare su drift | provider offre artifact immutabile attestato verificabile |
| break-glass deve bypassare tutto | lo stop urgente non giustifica riattivazione o falsificazione | STOP asimmetrico, START con re-verifica | safety case specialistico richiede regola più restrittiva |

#### 17. Autocritica del Livello 6

- Non è stato scelto né provato alcun IdP, PDP/PEP, KMS, SPIFFE, Sigstore o build platform.
- I target 5/15 minuti e 24 ore sono ipotesi operative, non misure dell'infrastruttura reale.
- SLSA Build L3 può essere costoso o non disponibile sulla toolchain scelta; abbassarlo aumenta rischio e richiede decisione esplicita.
- La compromissione dell'identity provider/root of trust resta un failure mode sistemico e richiede recovery esterno al solo runtime.
- La separazione organizzativa è difficile con 4–6 persone; “independent” può richiedere shared service o reviewer esterno.
- La provenance SaaS resta parziale: L6 la rende visibile, non la elimina.
- L6 non esegue test, non firma artifact e non chiude nessuno dei 43 finding.

**Gate L6:** contratti, authority lattice, trust chain e verification matrix sono definiti; **operational proof = `NOT_STARTED`**.  
**Backtrack L6:** identity/signing/enforcement non disponibili → ridurre TPE-1 o aggiungere capability e forecast; signer/PDP/IdP compromesso → release freeze e L5 containment; impossibilità di separare proponente/verificatore/approvatore → nessun G7.


### Registro del Livello 7 — miglioramento finale unico

**Tema:** trasformare il piano tecnico L6 in una baseline di esecuzione finanziabile, resource-loaded e governata da autorizzazioni progressive, senza confondere approvazione del piano, autorizzazione alla spesa e readiness di produzione.  
**Non modificato in L7:** runtime `asyncio`/PostgreSQL, TPE-1, 43 finding, DAG tecnico W0–W11, evidence policy, containment, trust chain, gate tecnici e RuFLO opzionale.  
**Perché è il miglioramento definitivo:** L6 descrive cosa deve essere vero tecnicamente, ma non dice ancora quale baseline autorizzi persone e spesa, come si ritirino gli ignoti, quanto costa l'incertezza, quando fermare l'investimento o chi abbia autorità di impegnare una data.

#### 1. Autopsia critica del piano L6

L6 resta non impegnabile per motivi concreti:

- `45–63 settimane` presume capability platform mature, mentre L5/L6 ammettono gap ancora non inventariati;
- le riserve `+6–12` e `+5–10` settimane sono dichiarate ma non trasformate in package, owner e decision point;
- 6–8 FTE non garantiscono security indipendente, Platform/SRE protetta, 24/7 on-call o autorità business disponibile;
- nessuna tariffa, costo vendor, procurement lead time, funding profile o limite di affordability è noto;
- non esiste un registro che obblighi a ritirare ogni assunzione prima che diventi una dipendenza nascosta;
- un gate tecnico potrebbe essere verde mentre budget, owner operativo o supporto post-go-live sono assenti;
- “piano definitivo” potrebbe essere scambiato per “prodotto pronto”.

Conclusione: L7 può rendere definitivo **il metodo di esecuzione e decisione**, non una data, un costo o una readiness che non hanno ancora dati e prove.

#### 2. `ExecutionBaselineContract`

Ogni autorizzazione di tranche deve riferirsi a un contratto versionato e firmato:

```text
ExecutionBaselineContract:
  baseline_id, version, baseline_hash, status
  accountable_sponsor, delivery_owner, finance_owner
  scope_hash, failure_policy_hash, authority_policy_hash
  outcome_statement, success_metrics[], benefit_owner
  do_nothing_consequence, affordability_ceiling
  work_package_ids[], dependency_graph_hash
  resource_plan_hash, named_seat_assignments[]
  rate_card_version, labor_cost_range
  non_labor_cost_categories[], operations_runway
  schedule_range, confidence_basis, external_lead_times[]
  assumption_ids[], risk_ids[], opportunity_ids[]
  procurement_decisions[], funding_tranches[]
  tolerances, replan_triggers[], stop_conditions[]
  independent_assurance_refs[]
  approved_at, valid_until, signers[]
```

Il contratto non autorizza un `PASS` tecnico. Collega scope, lavoro, risorse, costo e autorità decisionale. Un campo critical assente produce `UNCOMMITTED`; non viene riempito con una media favorevole.

#### 3. Stati di autorizzazione separati dai gate tecnici

| Stato esecutivo | Significato | Lavoro ammesso | Divieto |
|---|---|---|---|
| **E0 — UNAUTHORIZED** | esiste soltanto il piano | review documentale | nessuna promessa, hiring, procurement o build imputata al programma |
| **E1 — DISCOVERY_AUTHORIZED** | sponsor finanzia T0 | W0, WA, inventario e stima bottom-up | nessun build full-scope o data commitment |
| **E2 — BUILD_AUTHORIZED** | baseline T1/T2 firmata | package approvati entro costo/tolleranza | nessuna promozione oltre ambiente autorizzato |
| **E3 — QUALIFICATION_AUTHORIZED** | ambiente e budget T3 disponibili | WC, EL4, soak, drill, release pack | nessun canary produttivo |
| **E4 — CANARY_AUTHORIZED** | G1–G5 e autorità T4 verdi | canary TPE-1 entro blast cap | nessun 100% o ampliamento scope |
| **E5 — OPERATE_AUTHORIZED** | G7, funding operativo e owner BAU firmati | solo `PRODUCTION READY FOR <scope_hash>` | nessun claim generico o scope drift |
| **STOPPED / REPLAN** | tolleranza o condizione stop violata | containment, evidence preservation, re-estimate | continuazione silenziosa o consumo di contingency non approvato |

Un gate tecnico e uno stato esecutivo rispondono a domande diverse. `G4 PASS` non autorizza spesa; `E2` non prova correttezza. Per avanzare servono entrambi quando applicabili.

#### 4. Baseline calendario finale

L7 distingue una baseline con capability provate da quella corrente, in cui tali capability sono ignote.

| Baseline | Capacità assunta | Capability trust/containment/EL4 | Range calendario G7 | Uso ammesso |
|---|---|---|---:|---|
| **B0 — CURRENT UNKNOWN** | 6–8 FTE core + 1–2 FTE specialistici/protected service | non provate | **56–85 settimane** | planning envelope L7; non commitment |
| **B1 — PROVEN FOUNDATION** | 6–8 FTE core, lane protette | già esistenti e qualificate | **45–63 settimane** | sostituisce B0 solo con evidence WA |
| **B2 — CONSTRAINED** | 4–6 FTE, specialisti condivisi | non provate | **72–107 settimane** | rischio alto di key-person e queueing |
| **B3 — MATURE FAST TRACK** | 9–11 FTE esperti, piattaforma matura | provate prima di W1 | **36–47 settimane** | floor seriale; richiede baseline indipendente |

Aritmetica conservativa di B0:

```text
B0_lower = 45 + 11 = 56 settimane
B0_upper = 63 + 22 = 85 settimane
```

B2 applica lo stesso gross reserve alla range L2 `61–85`: `61+11=72`, `85+22=107`. B3 non riceve automaticamente il delta perché la maturità delle capability ne è una precondizione; se la precondizione cade, viene ricalcolata.

Queste range non sono distribuzioni probabilistiche né P50/P80. Prima di dichiarare una probabilità serve un integrated master schedule resource-loaded con durate calibrate e schedule risk analysis; L7 non inventa un livello di confidenza.

#### 5. Package di ritiro del gross reserve L5/L6

| WP delta | Contenuto netto, non già provato | Durata calendario | Predecessori/uscita | Owner |
|---|---|---:|---|---|
| **WA — Baseline qualification** | inventory repo/platform, make-buy, rate card, procurement, IdP/KMS/CI/EL4/OOB feasibility | 2–4 sett. | dopo W0; evidence decide B0↔B1 | Delivery Lead + Platform + Finance |
| **WB — Trust/containment gap closure** | capability mancanti per workload identity, PDP/PEP, signing/provenance, revoca, kill channel e incident control | 5–10 sett. | WA+W1; blocca W7/W8 | Security + Platform/SRE |
| **WC — Qualification environment admission** | EL4 equivalente, fault injection, OOB path, data/identity fixtures e pre-drill qualification | 4–8 sett. | W4+W5+WB; blocca W9 | SRE + Security + QA |

Somma lorda: `2–4 + 5–10 + 4–8 = 11–22 settimane`. Il credito di overlap è **zero** finché uno schedule resource-loaded non dimostra risorse separate e predecessori soddisfatti. Se WA prova che una capability esiste già, il relativo lavoro viene rimosso con evidence, non con ottimismo.

```text
W0 → WA → W1 → W2 → W3 ───────────────┐
             │     │                   │
             ├→ W4 ├→ W5 ────────┐    │
             ├→ W6 └→ WB ────────┼→ W7/W8 → WC → W9 → W10 → W11
             └────────────────────┘
                                  └→ WX solo con tranche e capacità separate
```

Il diagramma è una baseline logica. La data nasce soltanto dal resource leveling: non si serializza artificialmente il lavoro per far tornare il calcolo e non si accredita parallelismo a persone non nominate.

#### 6. Staffing minimo nominabile

L7 distingue **seat coverage** da headcount. Una persona può coprire più seat compatibili, ma la sua allocazione totale non supera 1,0 FTE e i divieti L4/L6 restano validi.

| Seat | Allocazione protetta durante i WP critici | Incompatibilità critica |
|---|---:|---|
| Accountable Sponsor / benefit owner | 0,1–0,2 FTE | non firma evidence tecnica al posto degli owner |
| Product/Domain Authority | 0,4–0,6 FTE | non verifica da solo il proprio side effect |
| Delivery/Tech Lead | 1,0 FTE | non è unico approvatore di release |
| Runtime/Data engineering | 2,0–2,5 FTE | nessun collasso con DBA/recovery verifier su P0 critical |
| Control Plane/Agent/Eval | 1,5–2,0 FTE | non firma da solo gate/closure prodotti dal builder |
| Platform/SRE | 1,0–1,5 FTE | recovery executor separato da START approval critical |
| Security/Trust | 0,8–1,0 FTE | indipendenza da builder/release proposer |
| QA/Evidence | 0,8–1,0 FTE | oracle critical approvato indipendentemente |
| Finance/Procurement | 0,1–0,3 FTE, front-loaded | nessuna autorità tecnica |

La delivery unit realistica richiede **7–10 FTE nominali protetti** oppure 6–8 core più 1–2 FTE/servizi specialistici realmente prenotati. Il 24/7 non è coperto automaticamente da questo team: richiede una rotazione organizzativa più ampia o un servizio operativo esistente, con costo e SLO espliciti.

Minimum bus-factor rule:

- nessun path runtime, DB recovery, release signing, incident STOP o authority recovery dipende da una sola persona;
- almeno due persone qualificate per ogni capability critica prima di E3;
- indisponibilità di un seat critical per oltre cinque giorni lavorativi forza resource replan, non overtime indefinito.

#### 7. Effort e modello di costo

La baseline L1 contiene `170–240 PW` core. Per i gap oggi non provati, L7 usa un delta lordo diagnostico:

```text
E_WA = 1,5–2,5 FTE × 2–4 settimane  = 3–10 PW
E_WB = 2,0–3,0 FTE × 5–10 settimane = 10–30 PW
E_WC = 2,0–3,0 FTE × 4–8 settimane  = 8–24 PW
E_gap = 21–64 PW
E_total_gross = 170–240 + 21–64 = 191–304 PW
```

Il delta è una riserva lorda, non lavoro garantito e non può essere sommato due volte a elementi L5/L6 già inclusi nei package core. WA deve produrre una reconciliation WBS riga per riga.

```text
C_labor = Σ(PW_role × loaded_weekly_rate_role)
C_total = C_labor
        + vendor/platform commitments
        + EL4/staging/telemetry/LLM usage
        + independent assurance
        + training/on-call/operations runway
        + approved risk contingency
        - evidenced reusable capability
```

Sensitività puramente aritmetica sul range lordo `191–304 PW`:

| Tariffa blended caricata, input ipotetico | Labor range | Cosa non include |
|---:|---:|---|
| €1.500/PW | €286.500–€456.000 | vendor, cloud, assurance, BAU, contingency |
| €2.000/PW | €382.000–€608.000 | idem |
| €3.000/PW | €573.000–€912.000 | idem |
| €4.000/PW | €764.000–€1.216.000 | idem |

Le tariffe sono input di sensibilità, non prezzi di mercato, preventivi o budget. Senza rate card, make/buy, procurement e operations runway, L7 **non autorizza un importo**.

#### 8. Tranche di finanziamento e decisione

| Tranche | Scope | Exit per finanziare la successiva | Stop/no-go |
|---|---|---|---|
| **T0 — Discovery** | W0+WA | TPE-1 reale, inventario, team nominato, WBS/costi/rischi, B0 o B1 scelta | nessun use case/owner, affordability incompatibile, capability irrecuperabile |
| **T1 — Foundation** | W1–W3 + parti necessarie WB | build, durable core e crash evidence; ETC aggiornato | P0 core non ha remediation credibile o platform foundation fallisce |
| **T2 — Integration** | W4–W8 + saldo WB | integration/security/agent boundary e end-to-end fault matrix | authority, tenant, side effect o provenance non contenibili |
| **T3 — Qualification** | WC+W9+W10 | EL4, soak, drill, closure pack, on-call e budget T4 | ambiente non equivalente, restore/rollback/kill switch falliti |
| **T4 — Canary/Operate** | W11, canary e operations runway | G7+E5 scoped, post-deploy review | trigger rosso, owner/funding BAU assenti |
| **TX — Optional** | WX/RuFLO/evolution | decisione separata `ADOPT_SHADOW/DEFER/REJECT` | sottrae capacità o diventa dipendenza core |

Ogni tranche ha un cap approvato, non una promessa di spendere tutto. Contingency è detenuta dall'autorità di programma e rilasciata contro rischio materializzato e decision record; non nasconde overrun ordinario.

#### 9. Assumption Retirement Register iniziale

| ID | Assunzione oggi non provata | Owner | Retirement evidence / deadline | Se falsa |
|---|---|---|---|---|
| **A01** | esiste codice/runtime riusabile | Tech Lead | repo inventory + clean build in T0 | greenfield pieno; W1 re-estimate |
| **A02** | TPE-1 ha use case e beneficio approvabili | Sponsor/Product | outcome, owner, workload e do-nothing case in T0 | stop programma |
| **A03** | 7–10 FTE/servizi protetti sono ottenibili | Delivery Owner | nomi, skill, allocation e backfill in T0 | B2 o stop |
| **A04** | PostgreSQL/CI/artifact/OTel platform sono disponibili | Platform | account, quota, SLO, smoke e costo in WA | WB/WC aumentano; replan |
| **A05** | IdP/PDP/KMS/signing/revocation soddisfano L6 | Security | capability test + procurement decision in WA | B0 e WB obbligatori |
| **A06** | adapter C1 supporta dedupe o read-back | Domain/Runtime | contract/failure spike prima di T1 | TPE-1 scende a C0 o stop |
| **A07** | data class/residency restano nel TPE-1 | Data Owner | classificazione firmata in T0 | scope/DAG/security replan |
| **A08** | EL4 equivalente è realizzabile | SRE/QA | environment design, fault hooks e cost in WA | E3 bloccato |
| **A09** | OOB kill/recovery channel è indipendente | SRE/Security | tabletop + technical path test prima di T3 | niente canary |
| **A10** | 24/7 e incident authority sono sostenibili | Ops Owner | rota, escalation, SLA e costo prima di T3 | T4 non finanziata |
| **A11** | review/approval indipendenti sono disponibili | Sponsor/Security | named approvers e calendar in T0 | gate critical bloccati |
| **A12** | budget e procurement rispettano il lead time | Finance/Procurement | funding profile e vendor dates in T0 | nessuna data commitment |

Un'assunzione scaduta non diventa “accepted risk” per inerzia. Diventa issue, cambia baseline oppure arresta la tranche.

#### 10. Controllo settimanale e reforecast

Non si usa “percent complete” soggettivo. Ogni WP è tracciato come `NOT_STARTED → DISCOVERY → BUILD → EVIDENCE → EXIT_PASS`; soltanto `EXIT_PASS` soddisfa una dipendenza.

Minimum control pack settimanale:

- actual PW e costo per WP/role;
- estimate-to-complete e range finish aggiornati;
- critical/near-critical path e float dopo resource leveling;
- blocked days, review queue p95, WIP e specialist allocation;
- defect escape, rework PW, flaky/error/not-proven evidence;
- assumption retirement, procurement lead e decision latency;
- contingency draw con risk/decision ID;
- outcome/scope drift e forecast operations cost.

Rebaseline obbligatoria se accade almeno uno dei seguenti:

1. upper schedule forecast cresce di oltre 4 settimane o 10%, usando il limite minore;
2. estimate-at-completion supera la tolleranza costo approvata, default provvisorio 10%;
3. rework supera 15% dell'effort per due settimane;
4. seat critical scende sotto l'allocazione protetta o review p95 supera tre giorni;
5. procurement/vendor critical slitta oltre dieci giorni lavorativi;
6. un'assunzione gate-critical non è ritirata alla deadline;
7. scope, authority, environment o evidence invalidation cambia il critical path;
8. contingency viene usata senza rischio materializzato e decision record.

Rebaseline non significa riscrivere la storia. La baseline precedente, varianza, causa, decisione e nuovo hash restano auditabili.

#### 11. Stop e descoping policy

Ordine di descoping ammesso prima di chiedere più tempo o budget:

1. rimuovere TX/WX, RuFLO ed evolution;
2. mantenere il claim limitato a TPE-1, senza generalizzazione multi-workflow;
3. ridurre TPE-1 da C1 a C0 se l'adapter non è riconciliabile;
4. ridurre tenant, volume o SLO soltanto con nuovo `scope_hash` e beneficio ancora valido;
5. rinviare capability non necessarie al primo scope, senza rimuovere controlli required dal rischio residuo.

Non sono descopabili per salvare la data: durabilità, idempotenza/reconciliation necessaria, tenant/data isolation, evidence integrity, rollback/restore, kill switch, trust/revocation, separazione di autorità e on-call coerente con lo SLO.

Stop business/tecnico obbligatorio se:

- T0 non trova un outcome con owner e valore sufficiente rispetto all'affordability ceiling;
- non esiste un path credibile per un P0 non-waivable;
- il C1 scelto non può essere deduplicato, verificato o riconciliato e C0 non produce valore;
- EL4, OOB containment o trust-root recovery non sono acquistabili/costruibili entro tolleranza;
- il team dipende strutturalmente da un solo individuo critical;
- sponsor, funding o owner BAU vengono ritirati.

#### 12. Matrice della decisione finale

| Dichiarazione | Consentita quando | Cosa **non** significa |
|---|---|---|
| **“L7 approvato”** | contenuto e metodo accettati | funding, data o software pronto |
| **“T0 autorizzata”** | E1 firmato con cap | full build autorizzato |
| **“Baseline committed”** | `ExecutionBaselineContract` firmato dopo T0, risorse/fondi nominati | gate tecnici passati |
| **“Release candidate”** | W10 exit + evidence chain | produzione autorizzata |
| **“Canary authorized”** | E4 e criteri tecnici richiesti | 100% o scope expansion |
| **“Production ready for TPE-1 hash”** | G7 + E5 + manifest/evidence validi | piattaforma genericamente pronta |

**Verdetto L7 corrente:** il piano è definitivo come baseline decisionale; l'esecuzione resta **E0 — UNAUTHORIZED**, B0 è soltanto un planning envelope, ogni finding è `OPEN` e ogni prova operativa è `NOT_STARTED`.

#### 13. Registro critico pubblico L7

| Affermazione ereditata | Obiezione più forte | Decisione L7 | Condizione di smentita |
|---|---|---|---|
| 45–63 settimane è la baseline | presume capability non provate | B0 corrente = 56–85; B1 solo con WA evidence | WA prova tutte le foundation e il resource schedule resta valido |
| 6–8 FTE bastano | non coprono automaticamente specialisti, indipendenza e 24/7 | 7–10 seat/FTE protetti o servizi prenotati | resource model nominativo dimostra copertura equivalente |
| una cost range può essere data ora | mancano rate, vendor, usage e BAU | solo formula e sensibilità; nessun budget autorizzato | T0 produce WBS, rate card, quote e risk analysis |
| contingency rende sicura la data | può nascondere scope o rework ordinario | draw soltanto contro rischio e autorità separata | governance più forte e auditata dimostra controllo equivalente |
| approvare L7 avvia il progetto | confonde qualità del piano con investment decision | E0 resta default | sponsor firma E1/T0 con cap e owner |
| mantenere tutto evita perdere valore | scope opzionale consuma critical capacity | descoping prima di overtime/budget | evidence economica dimostra beneficio netto superiore |

#### 14. Autocritica del Livello 7

- `56–85 settimane` è ancora aritmetica deterministica, non una schedule risk analysis probabilistica.
- `191–304 PW` contiene una riserva lorda: senza WBS reconciliation può sovrastimare overlap o sottostimare integrazione.
- Le tariffe di sensibilità non sono benchmark di mercato e non includono fiscalità, vendor o costi BAU.
- Benefit, affordability ceiling, workflow reale, rate card, team e procurement restano ignoti.
- Lo staffing non risolve automaticamente conflitti organizzativi o indipendenza in un'azienda piccola.
- Un piano definitivo non elimina discovery, rework, incidenti o rischio di stop.
- L7 non implementa codice, non esegue evidence, non chiude finding e non autorizza il clone RuFLO.

**Gate L7:** completezza della baseline decisionale e tracciabilità delle assunzioni = `PASS` documentale; commitment = `BLOCKED`, execution authorization = `E0`, operational proof = `NOT_STARTED`.  
**Backtrack L7:** assumption failure, tolleranza superata, owner/funding ritirato o capability non disponibile → `STOPPED/REPLAN`, nuovo baseline hash e riapertura dei WP/gate impattati; mai correzione silenziosa della data.
...
### AD-13 — Nessuna autorità senza chain of trust

Identità, decisione, delega, artifact, evidence, release e runtime load devono essere legati da hash, policy e principal verificabili. Una firma isolata, un ruolo nel prompt o una rete interna non concedono autorità.

### AD-14 — Nessun commitment senza baseline esecutiva firmata

Gate tecnici, autorizzazione alla spesa e claim di produzione restano separati. Data, costo o tranche diventano impegnabili soltanto tramite un `ExecutionBaselineContract` resource-loaded, con scope, assunzioni, tolleranze, owner, funding e stop condition firmati.
...
┌───────────────────────────────────────────────────────────────────────┐
│ API / COMMAND INTAKE                                                  │
│ authenticate → authority PEP → validate → normalize → request dedupe │
└───────────────────────────────┬───────────────────────────────────────┘
                                │
                                ▼
┌───────────────────────────────────────────────────────────────────────┐
...
┌───────────────────────────────────────────────────────────────────────┐
│ RELEASE CONTROL                                                       │
│ trust-chain verify | authority PEP | migrations | rollout | rollback │
└───────────────────────────────┬───────────────────────────────────────┘
                                │
                                ▼
┌───────────────────────────────────────────────────────────────────────┐
│ DURABLE ASYNCIO WORKFLOW RUNTIME                                      │
│ workload identity | authority PEP | scheduler | worker pool | lease  │
│ step executor | idempotency | retry | Saga | reconciler              │
└──────────────┬─────────────────────────────┬──────────────────────────┘
               │                             │
               ▼                             ▼
┌──────────────────────────┐     ┌─────────────────────────────────────┐
...
### 4.6 Authorization decision e capability grant

```text
decision_id, subject_id, workload_identity, delegated_by,
action, resource_id, tenant_id, scope_hash, artifact_hash,
authority_policy_hash, runtime_mode, obligations[], maximum_uses,
issued_at, expires_at, revocation_epoch, signer, signature
```

Una decisione è valida soltanto al PEP previsto, per audience/azione/risorsa/scope/artifact dichiarati; non è trasferibile come autorità generica.
...
### Assunzione di pianificazione definitiva L7

La baseline corrente è **B0: 56–85 settimane**, con **6–8 FTE core più 1–2 FTE specialistici o servizi protetti**, soltanto per TPE-1. Resta una planning range non impegnabile. La precedente `45–63 settimane` diventa B1 e può essere usata soltanto se WA prova che trust, containment, EL4, ambienti e authority services esistono già e lo schedule resource-loaded ne conferma la capacità.

La delivery unit richiede 7–10 FTE/seat protetti; 24/7, approvatori indipendenti e Finance/Procurement possono provenire da servizi organizzativi, ma devono essere nominati, prenotati e costati. Nessuna data o cifra diventa commitment prima di E1/T0, `ExecutionBaselineContract` firmato, scope TPE-1 reale e assumption register ritirato secondo deadline.

### Critical path e package delta L7

```text
W0/F0 → WA → W1/F1 → W2/F2 → W3/F2 ───────────────┐
                 │          │                        │
                 ├→ W4/F3 ──┼→ W5/F3 ────────┐     │
                 ├→ W6/F4 ──┴→ WB ───────────┼→ W7/F4 → W8 → WC → W9/F5 → W10/F7 → W11/F7
                 └────────────────────────────┘
                                              └→ WX/F6  # tranche e capacità separate

Technical path ereditato: W0→W1→W2→W3→W7→W8→W9→W10→W11 = 45–63 settimane.
Gross risk-resolution reserve corrente: WA+WB+WC = 11–22 settimane senza overlap credit.
Planning envelope B0: 56–85 settimane.
F6 evolution/RuFLO non blocca il core e non riceve capacità dal critical path.
```

In caso di conflitto, il DAG W0–W11 integrato dai package L7 WA–WC, i tipi di dipendenza e le exit evidence prevalgono sulle righe riassuntive delle macro-fasi F0–F7; WX/TX resta separato.
...
- `AuthorityTrustContract`, principal/action catalog, authority matrix, trust roots, delegation, revocation e break-glass policy;
- `ExecutionBaselineContract`, assumption register A01–A12, baseline B0/B1, resource-loaded WBS, tranche, affordability e stop policy;
...
- fail se T0 non nomina sponsor, delivery owner, benefit owner, Finance, seat critici o cap di spesa;
- fail se una data/costo viene rappresentata come commitment senza baseline hash e firme;
...
**Human approval:** Accountable Sponsor, Delivery/Tech Lead, Security, Data Owner, Product/Benefit Owner e Finance per la sola autorizzazione esecutiva; i gate tecnici mantengono i propri approvatori.

### F1 — Package compilabile, contratti e CI — effort L1: 18–26 PW

**Dipendenze:** F0 completata, WA conclusa, T1/E2 autorizzata e baseline Foundation approvata; il `PASS` finale di G1 arriva all'uscita di F1.  
...
- build identity, isolated runner, artifact digest e provenance attestation verificabile;
- schema machine-readable per principal, action/resource, decisione, grant e revocation epoch;
...
- workload identity, PDP/PEP context-bound, credential rotation e revocation path;
...
- prompt registry con hash, owner, tool grants, delegation cap ed eval version;
- artifact/evidence store e evidence graph requirement→case→run→closure;
- source→build→artifact→evidence→manifest trust chain con signer policy;
...
**Dipendenze:** F3, F4, WB conclusa e WC ammessa tramite T3/E3.  
...
- identity revocation, policy downgrade, signer compromise, grant escalation e break-glass drill;
- release candidate osservata in staging.
- cost/schedule actuals, estimate-at-completion, operations runway e assumption closure pack aggiornati.
...
**Dipendenze:** G1–G5 passati; G6 solo in modalità proposal/shadow se presente; T4/E4 autorizzata e operations runway finanziata.  
**Obiettivo:** rilascio limitato, osservato e reversibile.

**Deliverable:**

- release manifest firmato con `scope_id`, `scope_hash`, `failure_policy_hash`, `authority_policy_hash`, signer identities, provenance refs, stato kill switch ed evidence bundle;
...
- deploy-time digest/authority verification, runtime loaded-artifact attestation;
- post-deploy verification e closure review;
- handover BAU con owner, rota, vendor escalation, costo operativo e review benefici.
...
|---|---|---|---|
| **G1 Foundation** | threat model, glossary, ADR, ownership, authority/principal contracts, `ExecutionBaselineContract`, assumptions, schemas, SLO workload, 43 finding tracciati | Tech Lead + Security + Data | F0 |
| **G2 Durable Core** | concurrency, CAS, crash-window, resume, restore e reconciliation verdi | Runtime Lead + SRE | F1/F2 |
| **G3 Integration** | outbox/inbox, tenant isolation, identity/PDP/PEP/revocation, redaction, resilience e event compatibility verdi | Security + Data + Runtime | F2/F3 |
| **G4 Multi-Agent Builder** | registry/envelope/prompt hash, capability grant, sandbox, budgets, deterministic blocking e provenance | AI/Eval Lead + Security | F1/F4 |
| **G5 Quality & Optimization** | golden/adversarial regression, latency/cost/capacity, WC/EL4 e soak entro target; ETC aggiornato | Product + SRE + QA | F3/F4/F5 |
...
| **G7 Production** | trust-chain verificata, canary, backup/restore, rollback, revocation, on-call, operations runway, audit, no blocker, signed manifest | Accountable Executive/Product + Tech + Security + SRE | fase responsabile o rollback |
...
| **Authority/trust-chain** | cross-env identity, revoke, delegation, PDP outage, policy downgrade, signer/build compromise, manifest/runtime digest, break-glass | authority escalation, stale allow, unsigned/unmanifested load o separation collapse |
...
- privileged decision binding a principal/policy/scope/artifact = 100%;
- artifact trust-chain coverage e runtime manifest match = 100%;
- shared privileged principal e orphan grant = 0.
...
| Break-glass STOP | Incident Commander | audit OOB + review ≤24 h | accountable on-call authority |
| Recovery START/EXPAND | Recovery owner | independent verifier | due autorità previste dall’`AuthorityTrustContract` |
| Tranche execution/funding | Delivery Owner | Finance + independent cost/schedule reviewer | Accountable Sponsor entro `ExecutionBaselineContract` |
...
7. **100%:** solo dopo G7 + E5, funding operativo e handover BAU; evolution e RuFLO restano kill-switchable.
...
| Prompt injection/tool escalation | Critica | untrusted delimiters, signed capability grants, per-call PEP, sandbox | revoke grant; disable agent/tool profile |
| Identity/authority escalation | Critica | workload identity, context-bound PDP/PEP, short TTL, revocation, separation | revoke principal; pause C1/release |
| Artifact/signer supply-chain compromise | Critica | isolated build, provenance, digest-bound manifest, distrust/rebuild | release freeze; quarantine chain |
...
| Staffing/key-person collapse | Alta | protected seats, bus-factor ≥2, allocation tracking | WIP reduction; B2/replan; stop tranche |
| Funding/procurement discontinuity | Alta | staged funding, lead-time register, operations runway | no next tranche/canary; preserve evidence |
| Baseline optimism/hidden scope | Alta | assumption retirement, WBS reconciliation, independent estimate | rebaseline, descoping o stop |
...
- `ExecutionBaselineContract` firmato e valido, stato E4 per il canary ed E5 per il 100%;
- T4 finanziata, operations runway e BAU owner approvati senza usare un gate tecnico come sostituto della decisione di investimento;
...
- `AuthorityTrustContract` firmato, non scaduto e legato a scope/failure policy;
- principal inventory, separation, credential TTL, revocation e break-glass provati;
- source→build→evidence→manifest→runtime chain completa per ogni artifact eseguibile;
- nessun runtime load o C1 con digest/decisione non manifestati;
...
- rotazione sostenibile, bus factor critical ≥2, vendor escalation e costo operativo coperti per la runway approvata;
...
1. **OLA-001:** autorizzare soltanto T0/E1 con cap; approvare system prompt v2.1 e AD-01…AD-14; creare `ExecutionBaselineContract` e assumption register.
2. **OLA-002:** eseguire WA: inventory repo/platform, clean-build baseline, make/buy, rate card, resource plan e procurement lead time; poi decidere B0/B1.
3. **OLA-003:** dopo T1/E2, creare repository/package, CI strict e schema PostgreSQL per workflow/step/lease/idempotency.
...
7. **OLA-007:** implementare strict contracts, error taxonomy, auth context, redaction, `FailurePolicyContract` e `AuthorityTrustContract`.
8. **OLA-008:** creare evidence graph, trust-chain, closure state machine e Gate Policy Engine con `NOT_PROVEN/FLAKY/ERROR`.
9. **OLA-009:** creare Agent/Prompt Registry, envelope e Native Asyncio Harness.
10. **OLA-010:** congelare TPE-1 e baseline resource-loaded/costata; implementare admission control deny-by-default; finanziare T2–T4 solo agli exit; RuFLO resta TX/WX separato dopo W7.
...
- Sigstore identity-based signing and verification: https://docs.sigstore.dev/cosign/signing/overview/
- SPIFFE workload identity concepts: https://spiffe.io/docs/latest/spiffe-about/spiffe-concepts/
...
- GAO Schedule Assessment Guide, GAO-16-89G: https://www.gao.gov/assets/gao-16-89g.pdf
- UK Government Functional Standard GovS 002, Project Delivery: https://projectdelivery.gov.uk/library-products/government-functional-standard-govs-002-project-delivery/
