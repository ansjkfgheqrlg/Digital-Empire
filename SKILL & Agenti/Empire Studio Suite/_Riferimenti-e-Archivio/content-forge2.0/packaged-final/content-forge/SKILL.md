...
| Sei a | Vai a |
|---|---|
| Capire il pipeline complessivo | `references/stages/01..08-*.md` |
...
[Stage 4] 🌟 MASTER KNOWLEDGE   → A5 mkd-builder-agent  (SEMPRE)
          DOCUMENT (MKD)          Output: master.md + glossary.md + faq.md + schemas.md
                                  (il "documento perfetto" ampliato)
...
[Stage 6] Interactive Build     → D1 question-designer + Bx target-builder
                                  ...
                                  Output: stage-06/output/<artifact-slug>/
[Stage 7] External QA           → C1 coverage-verifier + C3 schema-validator (parallel)
                                  Output: qa-report.md
[Stage 8] Packaging             → scripts/package_target.py
                                  Output: stage-08/packaged/
...
## 🤖 Inventario agenti (12 specialisti + Conductor)
...
├── stage-07/        (coverage-report.{json,md}, schema-report.{json,md}, qa-report.md)
├── stage-08/        (packaged/<artifact-slug>/ + master-knowledge-document/ bonus)
