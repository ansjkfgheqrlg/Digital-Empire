...
1. **Memory First (P10)**: Dopo OGNI azione (spawn, handoff, result, decision, user message), run `python scripts/memory_manager.py --checkpoint "..." --phase=N --target=/home/user/content-ingest-ecosystem` (o embedded run dir). Aggiorna INDEX. Two-layer.
