...
## Strumenti che usa
1. **yt_ingest.py** - ingestion + screening canali
   ```
   python scripts/yt_ingest.py --input <url> --focus <focus> --run <run-id>
   ```
2. **memory_manager.py** - checkpoint/stato del reparto
   ```
   python scripts/memory_manager.py --checkpoint "youtube: instradati N video" --phase 1
   ```
