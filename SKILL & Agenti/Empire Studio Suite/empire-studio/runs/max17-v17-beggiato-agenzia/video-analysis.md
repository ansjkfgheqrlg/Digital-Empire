# Video Analysis — rvpRQD43wdY
## "Come Avviare Un'Agenzia AI da 10.000€/Mese (Guida COMPLETA)" — Giovanni Beggiato (Gentes AI)

**Fonte:** trascrizione audio integrale (100%, 5.550 righe uniche deduplicate da 44.552 righe grezze
del `.vtt` a caption-a-scorrimento, lette per intero in 12 blocchi da questa sessione) + campione
visivo mirato (24 frame guardati nativamente su 1.928 densi, 1,2% — campionamento dichiarato e
motivato in `coverage.md`, concentrato sulle sezioni a schermo condiviso).
**Durata:** 4h17m00s (15.420s) · **Uploader:** Giovanni Beggiato · **Lingua:** italiano
**Run:** `empire-studio/runs/max17-v17-beggiato-agenzia` (batch max17, video più lungo del lotto)
**Ingested:** 2026-09-04

> **Formato del video (verificato, non presunto):** misto. Circa i primi ~2/3 (0:00–2:53:14, capitoli
> Intro → Acquisizione clienti) sono prevalentemente **talking-head + lavagna Excalidraw** (webcam
> in basso a sinistra, disegni a mano fatti in diretta su excalidraw.com che restano fissi a schermo
> per lunghi tratti — fino a 29+ minuti in un caso verificato, vedi `coverage.md`), con brevi incursioni
> di **screen-share reale** di piattaforme esterne (Upwork, Fiverr, LinkedIn analytics). Dagli ultimi
> ~80 minuti (Fulfillment → Hiring & Scaling, capitoli 7-14) il video diventa **screen-share denso**
> — demo dal vivo di un documento Whimsical, di GoHighLevel (CRM) e di Meta Ads Manager. Questo
> motiva l'`--interval 8` scelto (vedi `coverage.md`) e la concentrazione del campione di frame in
> questa seconda metà.

## Capitoli (da `ingest.json`, confermati a video)
...
Regola esplicita anti-**"nich(e) hopping"**: permanenza minima consigliata di **6 mesi** su una
...
(impiegato/imprenditore/studente). Script di contatto testato dall'autore (mostrato a voce con
Whisper come dettatura): *"Ho aperto da poco un'azienda di AI... sto cercando due clienti a cui
implementare questo servizio gratuitamente. Conosci qualcuno a cui potrebbe interessare? [...]
l'unica cosa che chiedo in cambio è un testimonial e almeno due referral"* — **verificato**,
`frame-0751.png` (1:40:08), mostra lo stesso schema di risposta attesa per amico/studente
("si, conosco genitori di X, ti do il contatto") vs dipendente. Meccanica di crescita a catena:
1 cliente gratuito → 2 referral → 2 nuovi clienti (a pagamento crescente, es. 250€→500€→1.000€) →
altri referral. Osservazione comportamentale: le persone aiutano molto di più a inizio carriera
(quando non c'è ancora "invidia sociale") che a metà crescita — quindi è il momento migliore per
chiedere aiuto, non quello peggiore come si crede.
...
canale LinkedIn, 54.000 follower in un anno, "50K" (probabile riferimento a fatturato/euro,
ambiguo nella trascrizione) generati, poi passaggio full-time a YouTube.
...
corso ads. **Questi contenuti non sono stati visti né ingeriti** — sono dietro paywall, riportati
qui solo come riferimento a ciò che l'autore dichiara di offrire, non come conoscenza acquisita da
questa sessione.
