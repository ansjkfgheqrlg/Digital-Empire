...
# Contenuto Integrale — JdAQzAcWR6k
## "How to Create VIRAL Carousels in ChatGPT (No Coding)" — Artem Novitckii

**Fonte:** analisi visiva nativa completa (117/117 frame unici su 230 densi, coverage 100%, NO-FINTO PASS) + sottotitoli inglesi integrali (`JdAQzAcWR6k.en.vtt`, 1865 righe) letti per intero in 2 blocchi. Dettaglio completo del metodo di lettura in `runs/max17-v01-artem/coverage.md`.
**Durata:** 7m40s (460s) · **Canale:** Artem Novitckii — Co-founder @ Aha!, "Teaching how to build and scale AI Systems", Auckland (Nuova Zelanda) · **Lingua:** inglese
**Run:** `empire-studio/runs/max17-v01-artem` (batch max17, v01)
**Ingested:** 2026-09-02T12:39:44 · **Archiviato (Memory Empire Stage C):** 2026-09-02

> **Regola applicata:** trascrizione e trascrizione visiva INTEGRALI, mai riassunte. I 4 prompt master recuperati parola per parola sono riportati qui sotto in blocchi di codice, testo esatto. Il quinto prompt del video (LinkedIn "recreate this infographic") è recuperato solo in forma di frammenti leggibili — **non è integrale**, ed è dichiarato esplicitamente come tale in ogni punto in cui compare in questo documento, mai completato a intuito.
...
## PARTE 1 — LA TESI DEL VIDEO (perché gli AI carousel "one-shot" falliscono)
...
Narrazione integrale della prima parte, in inglese, dal VTT (00:00:41–00:01:16, ricostruita sui sottotitoli a scorrimento): *"the reason why most AI carousels fail is because these image generation models like ChatGPT image, Nana Banana, they can only generate one image at a time. So the moment you ask it to create a full six slide carousel in one go, every single slide is being created from scratch without any awareness of the other slides. And that's why you end up with a carousel that looks generic, inconsistent and honestly it just doesn't have any taste to it."*

---

## PARTE 2 — IL METODO — 4 passi (slide di chiusura, frame-207/216/217/219/221/228)
...
⚠️ Onestà sulla trascrizione: `frame-072`/`frame-074` mostrano questa pagina a leggera sfocatura da scroll (motion blur); i numeri e le citazioni sopra sono confermati leggibili con sicurezza medio-alta, ma il titolo esatto della sezione "1.2 [...] Work: The Psychology" ha una parola tagliata dal blur (probabile "Why Carousels Work" o simile — non ricostruibile con certezza al 100%). Nessuna di queste statistiche di terzi (tabella performance benchmarks) è verificata indipendentemente da questa sessione: sono riportate come "citate nel documento fonte del video", non come dato confermato da Digital Empire.

Narrato a voce (02:02–02:11): *"before you even think about generating any images, you need to nail your copy first. This is the step that most people skip, but you shouldn't, because it doesn't matter how pretty your carousel is if your copy is weak."*

---

## PARTE 4 — IL PROMPT DI COPY (uso reale) — integrale
...
## PARTE 5 — LA RICERCA DEL VISUAL ANCHOR — Pinterest
...
## PARTE 6 — I DUE PROMPT MASTER (i pezzi più preziosi del video) — integrali, parola per parola
...
### PROMPT META (bonus, non uno dei due master ma prompt integrale a sé, usato in una chat separata di pianificazione contenuti)

```
can you generate a 5-7 slide carousel about how gpt image 2 can be used to
fully automate instagram carousel and generate me those 5-7 slides, think
through each slide, this carousel has to go mega viral on each word and
visual asset must it earn its place I want this carousel to showcase the
full capability of its model and how it can easily be integrated or to
automate Instagram Carousels. The last slide must be a CTA like Comment the
word image and I'll send you the exact guide to set this up or something
along the lines of that. I will send them instructions and a prompt to
automate this.
```

(frame-023, chat "GPT Stage 2 Carousel", usato per generare un carosello dimostrativo *sul metodo stesso*, non sulla morning routine — è il quarto dei quattro prompt recuperati parola per parola in questo ingest).

---

## PARTE 7 — GENERAZIONE — cosa è uscito, slide per slide (carosello "Morning Routine")
...
## PARTE 8 — STRUMENTO DI ANTEPRIMA — Publer (free tool)
...
## PARTE 9 — CANVA MAGIC LAYERS — rifinitura (Part 3, cap. 5:25)
...
Struttura cartelle: le 8 slide generate vengono scaricate in una singola cartella locale nominata dal topic (es. file `slide-1.png` ... `slide-8.png` visti nel file picker di Canva), poi caricate tutte insieme sia in Publer (per l'anteprima) sia in Canva (per eventuali micro-tweak con Magic Layers).

---

## PARTE 10 — PARTE 4 DEL VIDEO: Stesso sistema per LinkedIn (cap. 6:14)
...
### Il quinto prompt (LinkedIn infographic) — NON INTEGRALE, dichiarato esplicitamente

Prompt dichiarato a voce: *"I gave it a prompt and I just said recreate this infographic"* (06:34–06:38). Il testo digitato in chat è visibile su schermo ma **su più righe piccole e in parte sfocate** (frame-196/199/200). ⚠️ **Questo non è un recupero integrale** — non deve essere trattato come tale in nessun uso futuro di questo documento. I frammenti leggibili con sicurezza sono i seguenti, riportati come frammenti e non ricomposti in un prompt continuo:

> *"...Google just killed SEO. [...] website will become invisible. They [...] the new Google Search, and it's the first time it's been updated in 25 years. [...] it's not really search anymore, it's [AI] looking for the answer [...] Generative Engine Optimization. Which means people need to structure content on your website in a way that AI engines like Gemini and ChatGPT actually cite your website as an answer. For example, turn your page headers into actual questions. Instead of 'Our Services,' write 'What does [your brand] do?' AI looks for question-answer pairs, so give it exactly that. [...] Comment SEARCH and I'll send you the skill + my full SEO playbook"*

Il senso generale (script per un'infografica virale su "SEO è morto, arriva GEO") è chiaro e confermato dal risultato generato, ma **le parole/frasi di collegamento mancanti non sono ricostruibili con certezza dalla risoluzione dello screenshot** e non sono state completate a intuito in nessun punto di questo documento o dei suoi derivati (`atoms.json`, `ingest-manifest.json`).
...
## PARTE 11 — PREVIEW INIZIALE (0:00, prima del tutorial vero e proprio)
...
Un secondo elemento della stessa lavagna Excalidraw (frame-051, circa 1:00), mostrato come esempio di "metodo" da comunicare al pubblico (non il workflow realmente eseguito nel resto del video): un framework a 5 step (01 Pick an angle, 02 Build the slide outline, 03 Lock the visual style, 04 Generate each slide, 05 Critique and refine) più un prompt stack a 4 elementi (01 STRATEGY PROMPT, 02 STYLE PROMPT, 03 SLIDE PROMPT, 04 CRITIQUE PROMPT), con lo slogan: **"The unlock is not the image. It's the system."**

---

## PARTE 12 — RISULTATI REALI MOSTRATI — Instagram Insights del carosello già pubblicato
...
⚠️ Onestà sui numeri: solo Interactions (12.911), Comments (6.003) e Shares (1.743) sono trascritti con confidenza alta. Il numero di Views esatto non è leggibile con certezza nei frame disponibili — si riporta quindi la dichiarazione verbale ("nearly 100,000 views") come fonte primaria per quella cifra, non un numero letto.

---

## PARTE 13 — LA COMUNITÀ / CTA FINALE
...
## PARTE 14 — CIÒ CHE IL VIDEO NON MOSTRA

- **Il prompt esatto e integrale usato per "recreate this infographic"** (LinkedIn) — solo frammenti leggibili tra motion blur e testo minuscolo (vedi Parte 10).
...
## PARTE 15 — CONFRONTO CON DIGITAL EMPIRE E CONSIGLI (integrale, come già in `video-analysis.md`)
...
## PARTE 16 — ENRICHMENT APPLICATO IN QUESTA SESSIONE (Stage D-F)

Il brief di questa sessione limitava esplicitamente il perimetro di patch a `carousel-empire` e `image` e a due concetti precisi: **slide-per-slide invece di carosello intero**, e **visual anchor**.

### Verifica del gap (prima di scrivere)

- `carousel-empire/SKILL.md` letto integralmente (251 righe prima della patch): Step 2-4 generano sempre e solo le 7 slide in un colpo solo via `generate_carousel.py`, con template HTML fisso — nessun concetto di "slide 1 come reference per le successive", nessuna menzione di generazione iterativa slide-per-slide. **Gap confermato.**
- `image/SKILL.md` letto integralmente (340 righe prima della patch): la tabella "Model Comparison" e la sezione "When to Use Which" menzionano "multi-image reference" come capacità di Gemini/Flux, e "Common Mistakes" #7 dice genericamente "use Flux multi-reference... for consistency" — ma **nessun punto della skill descrive la tecnica operativa specifica** "genera la prima immagine della serie, poi usala come reference per tutte le successive" (il pattern "visual anchor" applicato a una serie, non solo "consistenza di brand" generica). **Gap confermato, più stretto del previsto**: la capacità tecnica di base (multi-image reference) era già nota alla skill, mancava solo il pattern operativo nominato esplicitamente.

### Patch applicate (additive, 0 cancellazioni)

- `carousel-empire/SKILL.md`: **+120 righe**, nuova sezione "## Modalità Alternativa — Stile AI-Generativo con Visual Anchor" inserita dopo lo Step 7 "Report Finale" e prima di "## Esempi Contenuto per Prodotto". Contiene: il principio slide-per-slide, la definizione di visual anchor, i due prompt master **integrali** (Prompt Master 1 "Slide 1" e Prompt Master 2 "Slide [X]"), e una sezione "Quando usarlo e come" con le regole operative (pick-best-of-N, disciplina anti-plagio "Do not copy", applicare comunque il Self-Check Visivo, riferimento incrociato alla skill `image` per il modello da usare).
- `image/SKILL.md`: **+6 righe**, nuova sottosezione "### Visual Anchor — Style Consistency Across a Series" inserita dopo "When to Use Which" e prima di "### Prompting Basics". Descrive il pattern in inglese (lingua della skill), con fonte citata in linea e riferimento incrociato a `carousel-empire/SKILL.md` per i prompt completi.
- **Nessuna terza skill toccata.** Nessuna cancellazione in nessuno dei due file (`git diff --numstat` → `+120/-0` e `+6/-0`).
- **Line endings**: entrambi i file erano già LF puro (0 CRLF) prima della patch e sono rimasti LF puro dopo — verificato con conteggio binario `\r\n` vs `\n`-only.

Dettaglio completo in `enrichment-report.md` nella stessa cartella.
