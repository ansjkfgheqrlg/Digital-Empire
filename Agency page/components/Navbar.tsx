...
const navItems = [
  ...
  { id: 'philosophy', label: 'visione', path: '/#philosophy' },
...
export const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (location.pathname === '/') {
        ...
        // Se siamo nel blog, attiva la sezione blog
  ...
  const handleNavClick = async (e: React.MouseEvent | undefined, item: typeof navItems[0]) => {
    ...
    if (location.pathname !== '/') {
      ...
      await navigate('/');
      setTimeout(() => {
        const targetId = item.id === 'hero' ? 'hero' : item.id;
          ...
          >
            ...
            {navItems.map((item) => (
              <a
                ...
                href={item.path}
                onClick={(e) => handleNavClick(e, item)}
                className={`font-serif text-lg transition-all w-full text-center py-4 rounded-xl lowercase ${
