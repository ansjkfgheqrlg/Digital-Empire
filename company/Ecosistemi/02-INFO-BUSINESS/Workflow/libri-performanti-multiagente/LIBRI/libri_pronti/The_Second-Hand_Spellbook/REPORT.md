# Report di consegna — The Second-Hand Spellbook

_Generato il 23/08/2026 alle 12:09_

---

## Il libro

- **Titolo**: The Second-Hand Spellbook
- **Autore**: Maren Ashcroft
- **Nicchia**: cozy fantasy bookshop
- **Capitoli**: 24
- **Parole**: 38.114
- **Pagine reali** (contate sul PDF impaginato): **118**
- **Formato**: 6.0x9.0 pollici

## File consegnati

- **PDF (da leggere)**: `The_Second-Hand_Spellbook.pdf`
- **Copertina**: `Cover_The_Second-Hand_Spellbook.png`
- **Manoscritto Word**: `The_Second-Hand_Spellbook.docx`
- **Metadati KDP**: `KDP_METADATA.txt`

## Su cosa si basa questo libro

Il libro nasce da un'analisi di mercato su Amazon: quali libri vendono in questa nicchia, a che prezzo, con quanta concorrenza. **Il testo e' originale**: i concorrenti servono a capire cosa cerca il lettore, mai a riprendere contenuti (KDP verifica i manoscritti e chiude gli account per contenuti derivati).

### Perche' questa nicchia (punteggio 83.1/100)

recensioni mediane 33 -> 50.0/50 | 7 concorrenti sotto 100 recensioni su 16 -> 13.1/30 | prezzo medio $21.19 -> 20.0/20

- Concorrenti esaminati in prima pagina: **16**
- Recensioni mediane dei concorrenti: **33** (piu' sono basse, piu' e' aggredibile)
- Concorrenti deboli (sotto 100 recensioni): **7**
- Prezzo medio della nicchia: **$21.19**
- Valutazione media: **4.25/5**

### Libri di riferimento analizzati

_Concorrenti in prima pagina su Amazon per questa ricerca. Usati per capire posizionamento e prezzo, non come fonte di testo._

- The Bookshop and the Barbarian
- The Last Spell At The Used Bookshop: A Cozy Fantasy of Magic, Grief, and the Books That Find Us (Tales of Eldermere Book 1)
- The Bookshop at the Edge of the Enchanted Forest: A Cozy Fantasy Novel About Found Family, Magic, and the Courage to Begin Again (The Enchanted Forest Chronicles Book 1)
- The Mudpuddle Manual of Natural Magic: A cozy witchy fantasy set in a magical bookshop
- Cloud Knight Bookshop: A Cozy Slice of Life LitRPG Fantasy

_Dati grezzi dell'analisi: `LIBRI/_ricerca_nicchie/nicchie_20260820_135214.json`_

---

## Controlli automatici eseguiti

- ✅ **Capitoli interrotti a meta'**: nessun problema
- ✅ **Lineette lunghe (non devono esserci)**: nessun problema
- ✅ **Capitoli che si ripetono**: nessun problema
- ✅ **Capitoli simili (da rileggere)**: nessun problema
- ✅ **Copy KDP (lo legge chi compra)**: nessun problema
- ⚠️ **Trattini nel testo (da rivedere a occhio)**: 15 da verificare
    - riga 5: trattino in 'rust-coloured' — contesto: 'She recognised it from the harbour at Fenn, before she had even read t'
    - riga 52: trattino in 'coal-tar' — contesto: 'That was the thing that did it, in the end. Not the board and not the '
    - riga 80: trattino in 'water-damaged' — contesto: 'It was a stock book, not accounts. Ilse's hand, small and upright and '
    - riga 191: trattino in 'blind-stamped' — contesto: 'It came away easily. It was heavier than it looked, the way books alwa'
    - riga 550: trattino in 'blind-stamped' — contesto: 'She stopped about two thirds of the way along, at a squat brown one wi'
    - _...e altri 10_
- ✅ **Prezzo rispetto alla nicchia**: nessun problema
- ✅ **Titolo sulla copertina**: nessun problema
- ✅ **EPUB (ebook)**: nessun problema
- ✅ **Numerazione pagine**: nessun problema
- ✅ **Parole spezzate a fine riga**: nessun problema

---

## Prima di pubblicare

1. Apri il PDF e leggi almeno i primi capitoli: e' il tuo nome sulla copertina.
2. Controlla che il titolo sulla copertina sia scritto correttamente.
3. Su KDP: formato 6x9, carica il PDF come interno e il PNG come copertina.
4. A pubblicazione avvenuta, sposta questa cartella in `LIBRI/libri_pubblicati/`.
