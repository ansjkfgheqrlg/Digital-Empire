# Report di consegna — The Ninth Winter

_Generato il 18/08/2026 alle 10:55_

---

## Il libro

- **Titolo**: The Ninth Winter
- **Autore**: Rebecca Miller
- **Nicchia**: amish romance suspense
- **Capitoli**: 24
- **Parole**: 36.853
- **Pagine reali** (contate sul PDF impaginato): **116**
- **Formato**: 6.0x9.0 pollici

## File consegnati

- **PDF (da leggere)**: `The_Ninth_Winter.pdf`
- **Copertina**: `Cover_The_Ninth_Winter.png`
- **Manoscritto Word**: `The_Ninth_Winter.docx`
- **Metadati KDP**: `KDP_METADATA.txt`

## Su cosa si basa questo libro

Il libro nasce da un'analisi di mercato su Amazon: quali libri vendono in questa nicchia, a che prezzo, con quanta concorrenza. **Il testo e' originale**: i concorrenti servono a capire cosa cerca il lettore, mai a riprendere contenuti (KDP verifica i manoscritti e chiude gli account per contenuti derivati).

### Perche' questa nicchia (punteggio 73.1/100)

recensioni mediane 180 -> 50.0/50 | 6 concorrenti sotto 100 recensioni su 16 -> 11.2/30 | prezzo medio $5.95 -> 11.9/20

- Concorrenti esaminati in prima pagina: **16**
- Recensioni mediane dei concorrenti: **180.0** (piu' sono basse, piu' e' aggredibile)
- Concorrenti deboli (sotto 100 recensioni): **6**
- Prezzo medio della nicchia: **$5.95**
- Valutazione media: **4.5/5**

### Libri di riferimento analizzati

_Concorrenti in prima pagina su Amazon per questa ricerca. Usati per capire posizionamento e prezzo, non come fonte di testo._

- Framed in Amish Country: An Amish Suspense Novel – A Reunion Contemporary Romance (Love Inspired Suspense)
- Plain Obsession: An Amish Romantic Suspense Novel (Hunters Ridge Book 1)
- The Amish Book Cellar: Second Change Love & Small-Town Secrets (A Willow Springs Amish Mystery Romance 1)
- Unshaken: Contemporary Amish Romantic Suspense with a Protective Hero Set in Southern Ohio
- The Case of the Amish Winter Vandal: Amish Mystery and Romance (Cedar Hollow Mysteries Book 1)

_Dati grezzi dell'analisi: `LIBRI/_ricerca_nicchie/nicchie_20260813_175812.json`_

---

## Controlli automatici eseguiti

- ✅ **Lineette lunghe (non devono esserci)**: nessun problema
- ⚠️ **Trattini nel testo (da rivedere a occhio)**: 14 da verificare
    - riga 142: trattino in 'not-talking' — contesto: 'That was not unusual. Miriam Yoder had never been a talker, and in the'
    - riga 290: trattino in 'chow-chow' — contesto: 'The first four were recipes. Actual recipes, in Sarah's hand: shoofly '
    - riga 940: trattino in 'wet-looking' — contesto: 'Graber Aggregate & Stone was not a large operation. Rebecca was not su'
    - riga 942: trattino in 'hand-lettered' — contesto: 'There was a bell on a post by the office with a hand-lettered sign: DE'
    - riga 1004: trattino in 'chow-chow' — contesto: '"Sarah." He said it the way a man says a name he is being handed. He t'
    - _...e altri 9_
- ✅ **Titolo sulla copertina**: nessun problema
- ✅ **Numerazione pagine**: nessun problema
- ✅ **Parole spezzate a fine riga**: nessun problema

---

## Prima di pubblicare

1. Apri il PDF e leggi almeno i primi capitoli: e' il tuo nome sulla copertina.
2. Controlla che il titolo sulla copertina sia scritto correttamente.
3. Su KDP: formato 6x9, carica il PDF come interno e il PNG come copertina.
4. A pubblicazione avvenuta, sposta questa cartella in `LIBRI/libri_pubblicati/`.
