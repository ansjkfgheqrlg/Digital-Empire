...
PlaywrightRotationManagerAgent = Agent(
    ...
    role="Gestisce rotazione Playwright per evitare blocchi user_agent timeout - operational - L5 Team SearchOptimizationTeam Ecosistema ResearchEcosystem - perfetta sincronia armonia",
    hierarchy_level=5,
    team="SearchOptimizationTeam",
    ecosystem="ResearchEcosystem",
    sub_ecosystem="OptimizationSub",
    inputs=["Playwright_logs", "rotation_requests"],
    outputs=["rotation_confirmation"],
    decision_logic="""Come agente PlaywrightRotationManagerAgent L5 team SearchOptimizationTeam ResearchEcosystem: Gestisce rotazione Playwright per evitare blocchi user_agent timeout - operational - Riceve HarmonySignal ready da TeamSynchronyProtocol leader SearchOptimizationLeader - Legge memoria via MemoryReaderAgent - Esegue task core con Playwright tool se necessario - Valida output con validator - Emite checkpoint condiviso broadcast ALL_TEAM - Handoff interno ack obbligatorio - Self-healing DetectionTeam se fail - Verifica harmony_status synchronized - Scrive memoria via MemoryWriterAgent + checkpoint via CheckpointManagerAgent - Logga decisione via DecisionLoggerAgent - Sincronia perfetta armonia intra-team InterTeamHarmonyProtocol esterno 8-step""",
    connections={"reports_to": ["SearchOptimizationLeader"], "manages": [], "collaborates_with": ["MemoryManagerLeader","MemoryReaderAgent","CheckpointManagerAgent"]},
    ...
    playwright_usage="real operational tool" if "Playwright" in "PlaywrightRotationManagerAgent" or "Search" in "PlaywrightRotationManagerAgent" or "Extractor" in "PlaywrightRotationManagerAgent" else None,
    skill_usage=["SelfHealingSkill","MemoryReadWriteSkill","CheckpointManagementSkill"],
    level_name="L5"
)

class PlaywrightRotationManagerAgent_SynchronizedWrapper:
...
PlaywrightRotationManagerAgent_sync = PlaywrightRotationManagerAgent_SynchronizedWrapper(PlaywrightRotationManagerAgent)
def get_agent():
    return PlaywrightRotationManagerAgent
def get_synchronized_wrapper():
    return PlaywrightRotationManagerAgent_sync
print(f"Agent dedicated PlaywrightRotationManagerAgent L5 Team SearchOptimizationTeam - perfect synchrony harmony")
