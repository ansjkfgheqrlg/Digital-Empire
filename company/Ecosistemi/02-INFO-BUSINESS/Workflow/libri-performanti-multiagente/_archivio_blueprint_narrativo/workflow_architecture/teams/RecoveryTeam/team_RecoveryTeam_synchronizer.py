...
# Team definition - non è più un unico file per tutti i team, ma file dedicato per RecoveryTeam
# Ogni agente ha suo file dedicato sotto questa cartella e lavora in sincronia perfetta

TEAM_NAME = "RecoveryTeam"
ECOSYSTEM = "SelfHealingEcosystem"
SUB_ECOSYSTEM = "RecoverySub"
LEADER = "RecoveryLeader"
MEMBERS = ['RecoveryLeader', 'RetryExecutorAgent', 'RollbackExecutorAgent', 'AlternativePathAgent', 'RecoveryValidatorAgent', 'EscalationManagerAgent']
...
TEAM_DEFINITION = Team(
    ...
    responsibilities=["Responsabilita principale RecoveryTeam in SelfHealingEcosystem", "Gestisce flusso interno con TeamSynchronyProtocol", "Valida output con validator agent", "Crea checkpoint condiviso via CheckpointManagerAgent", "Gestisce self-healing sincronizzato intra-team", "Esegue handoff esterno 8-step sincronizzato via InterTeamHarmonyProtocol"],
    ...
    external_handoff_protocol={
        ...
        "steps": [
            "1. RecoveryLeader (LEADER L3) crea handoff package structured output decisions risks checkpoint ref + harmony_status synchronized",
