...
"""
AGENTE DEDICATO - File singolo per GraphicPromptMicroAgent
Team: GraphicDesignTeam
Ecosistema: VisualEcosystem / GraphicSub
...
GraphicPromptMicroAgent = Agent(
    ...
    role="Atomic creazione singolo prompt grafica - micro - L7 - Team GraphicDesignTeam - Ecosistema VisualEcosystem - Lavora in perfetta sincronia e armonia con altri agenti via TeamSynchronyProtocol HarmonySignal",
    hierarchy_level=7,
    team="GraphicDesignTeam",
    ecosystem="VisualEcosystem",
    sub_ecosystem="GraphicSub",
    inputs=["prompt_request_single"],
    outputs=["single_prompt_created"],
    decision_logic="""Come agente GraphicPromptMicroAgent L7 in team GraphicDesignTeam ecosistema VisualEcosystem sub GraphicSub:
    Atomic creazione singolo prompt grafica - micro
    LOGICA DECISIONALE ESATTA:
    - Riceve HarmonySignal ready da TeamSynchronyProtocol leader GraphicDesignLeader
    ...
    - Emite HarmonySignal checkpoint con checkpoint_id condiviso via CheckpointManagerAgent broadcast a ALL_TEAM team GraphicDesignTeam
    ...
    SINCRONIA E ARMONIA: Lavora in perfetta sincronia e armonia con altri agenti del team GraphicDesignTeam - ogni agente invia ready signal, checkpoint condiviso, handoff con ack, validazione, harmony_status synchronized - InterTeamHarmonyProtocol per handoff esterno 8-step sincronizzato via Memory broker
    """,
    connections={"reports_to": ["GraphicDesignLeader"], "manages": [], "collaborates_with": ["MemoryManagerLeader","MemoryReaderAgent","CheckpointManagerAgent","TeamSynchronizer_GraphicDesignTeam"]},
...
class GraphicPromptMicroAgent_SynchronizedWrapper:
    ...
    def emit_ready(self):
        signal = HarmonySignal(
            signal_id=f"GraphicDesignTeam_{self.agent.name}_ready_{int(time.time()*1000)}",
            sender_agent=self.agent.name,
            receiver_agent="GraphicDesignLeader",
            team="GraphicDesignTeam",
            ecosystem="VisualEcosystem",
            signal_type="ready",
    ...
    def sync_checkpoint(self, checkpoint_id: str):
        ...
        signal = HarmonySignal(
            signal_id=f"GraphicDesignTeam_{self.agent.name}_checkpoint_{checkpoint_id}",
            sender_agent=self.agent.name,
            receiver_agent="ALL_TEAM_GraphicDesignTeam",
            team="GraphicDesignTeam",
            ecosystem="VisualEcosystem",
            signal_type="checkpoint",
            payload={"checkpoint_id": checkpoint_id, "shared": True, "agent": self.agent.name, "team": "GraphicDesignTeam"},
            timestamp=time.strftime("%Y-%m-%dT%H:%M:%SZ"),
            requires_ack=False
        )
        self.harmony_signals.append(signal)
    ...
    def communicate(self, target_agent: str, payload: dict, signal_type: str = "handoff"):
        signal = HarmonySignal(
            signal_id=f"GraphicDesignTeam_{self.agent.name}_to_{target_agent}_{signal_type}",
            sender_agent=self.agent.name,
    ...
    def validate_harmony(self):
        return {"agent": self.agent.name, "team": "GraphicDesignTeam", "status": self.status, "checkpoint_shared": self.checkpoint_shared, "signals_count": len(self.harmony_signals), "harmony": "synchronized"}

    def self_heal_synchronized(self, error_type: str, failed_op: str):
        # Self-healing in armonia con team - rollback sincronizzato
        return {
            "phase": "GraphicDesignTeam",
            ...
            "team_synchrony": "all members rollback to shared checkpoint GraphicDesignTeam"
        }

# Istanza wrapper sincronizzata
GraphicPromptMicroAgent_sync = GraphicPromptMicroAgent_SynchronizedWrapper(GraphicPromptMicroAgent)

def get_agent():
    return GraphicPromptMicroAgent

def get_synchronized_wrapper():
    return GraphicPromptMicroAgent_sync

print(f"Agent file dedicated GraphicPromptMicroAgent L7 Team GraphicDesignTeam Ecosistema VisualEcosystem - perfect synchrony harmony initialized")
