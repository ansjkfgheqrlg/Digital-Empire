...
# Team definition - non è più un unico file per tutti i team, ma file dedicato per DetectionTeam
# Ogni agente ha suo file dedicato sotto questa cartella e lavora in sincronia perfetta

TEAM_NAME = "DetectionTeam"
ECOSYSTEM = "SelfHealingEcosystem"
SUB_ECOSYSTEM = "DetectionSub"
LEADER = "DetectionLeader"
MEMBERS = ['DetectionLeader', 'OutputMonitorAgent', 'ErrorDetectorAgent', 'AnomalyDetectorAgent', 'StallDetectorAgent', 'PlaywrightFailureDetectorAgent', 'MemoryFailureDetectorAgent', 'ValidationCheckMicroAgent']
...
TEAM_DEFINITION = Team(
    ...
    responsibilities=["Responsabilita principale DetectionTeam in SelfHealingEcosystem", "Gestisce flusso interno con TeamSynchronyProtocol", "Valida output con validator agent", "Crea checkpoint condiviso via CheckpointManagerAgent", "Gestisce self-healing sincronizzato intra-team", "Esegue handoff esterno 8-step sincronizzato via InterTeamHarmonyProtocol"],
    ...
    external_handoff_protocol={
        ...
        "steps": [
            "1. DetectionLeader (LEADER L3) crea handoff package structured output decisions risks checkpoint ref + harmony_status synchronized",
