...
"""
AGENTE DEDICATO - File singolo per SearchQualityValidatorAgent
...
SearchQualityValidatorAgent = Agent(
    ...
    role="Valida qualita risultati search Amazon - support - L6 - Team AmazonKeywordResearchTeam - Ecosistema ResearchEcosystem - Lavora in perfetta sincronia e armonia con altri agenti via TeamSynchronyProtocol HarmonySignal",
    hierarchy_level=6,
    team="AmazonKeywordResearchTeam",
    ecosystem="ResearchEcosystem",
    sub_ecosystem="PlaywrightOps",
    inputs=["search_results_raw", "extracted_metadata"],
    outputs=["search_quality_validation", "quality_flag"],
    decision_logic="""Come agente SearchQualityValidatorAgent L6 in team AmazonKeywordResearchTeam ecosistema ResearchEcosystem sub PlaywrightOps:
    Valida qualita risultati search Amazon - support
...
class SearchQualityValidatorAgent_SynchronizedWrapper:
...
SearchQualityValidatorAgent_sync = SearchQualityValidatorAgent_SynchronizedWrapper(SearchQualityValidatorAgent)

def get_agent():
    return SearchQualityValidatorAgent

def get_synchronized_wrapper():
    return SearchQualityValidatorAgent_sync

print(f"Agent file dedicated SearchQualityValidatorAgent L6 Team AmazonKeywordResearchTeam Ecosistema ResearchEcosystem - perfect synchrony harmony initialized")
