...
# Team definition - non è più un unico file per tutti i team, ma file dedicato per QualificationDecisionTeam
# Ogni agente ha suo file dedicato sotto questa cartella e lavora in sincronia perfetta

TEAM_NAME = "QualificationDecisionTeam"
ECOSYSTEM = "QualificationEcosystem"
SUB_ECOSYSTEM = "DecisionSub"
LEADER = "QualificationDecisionLeader"
MEMBERS = ['QualificationDecisionLeader', 'DecisionAggregatorAgent', 'RiskFlagManagerAgent', 'QualificationReportWriterAgent', 'DecisionQualityCheckerAgent']
...
TEAM_DEFINITION = Team(
    ...
    responsibilities=["Responsabilita principale QualificationDecisionTeam in QualificationEcosystem", "Gestisce flusso interno con TeamSynchronyProtocol", "Valida output con validator agent", "Crea checkpoint condiviso via CheckpointManagerAgent", "Gestisce self-healing sincronizzato intra-team", "Esegue handoff esterno 8-step sincronizzato via InterTeamHarmonyProtocol"],
    ...
    external_handoff_protocol={
        ...
        "steps": [
            "1. QualificationDecisionLeader (LEADER L3) crea handoff package structured output decisions risks checkpoint ref + harmony_status synchronized",
