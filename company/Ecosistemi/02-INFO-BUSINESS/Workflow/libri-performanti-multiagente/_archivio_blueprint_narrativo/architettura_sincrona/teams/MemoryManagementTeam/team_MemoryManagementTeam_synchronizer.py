...
# Team definition - non è più un unico file per tutti i team, ma file dedicato per MemoryManagementTeam
# Ogni agente ha suo file dedicato sotto questa cartella e lavora in sincronia perfetta

TEAM_NAME = "MemoryManagementTeam"
ECOSYSTEM = "MemoryEcosystem"
SUB_ECOSYSTEM = "CoreMemorySub"
LEADER = "MemoryManagerLeader"
MEMBERS = ['MemoryManagerLeader', 'MemoryWriterAgent', 'MemoryReaderAgent', 'MemoryValidatorAgent', 'CheckpointManagerAgent', 'DecisionLoggerAgent', 'PlanStorageAgent', 'HierarchyManagerAgent', 'ImportantNotesAgent', 'MemoryReadMicroAgent', 'MemoryWriteMicroAgent']
...
TEAM_DEFINITION = Team(
    ...
    responsibilities=["Responsabilita principale MemoryManagementTeam in MemoryEcosystem", "Gestisce flusso interno con TeamSynchronyProtocol", "Valida output con validator agent", "Crea checkpoint condiviso via CheckpointManagerAgent", "Gestisce self-healing sincronizzato intra-team", "Esegue handoff esterno 8-step sincronizzato via InterTeamHarmonyProtocol"],
    input_source="Handoff package da ecosistema precedente + memory MemoryEcosystem + sync signals",
    output_target="Prossimo team/ecosistema + memory MemoryEcosystem + checkpoint condiviso + sync ack",
    ...
    external_handoff_protocol={
        ...
        "steps": [
            "1. MemoryManagerLeader (LEADER L3) crea handoff package structured output decisions risks checkpoint ref + harmony_status synchronized",
