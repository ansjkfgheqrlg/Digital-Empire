...
CheckpointPrunerAgent = Agent(
    ...
    role="Gestisce pruning checkpoint vecchi preservando traceability - support - L6 Team CheckpointSubEcosystem Ecosistema MemoryEcosystem - perfetta sincronia armonia",
    hierarchy_level=6,
    team="CheckpointSubEcosystem",
    ecosystem="MemoryEcosystem",
    sub_ecosystem="CheckpointSub",
    inputs=["checkpoint_chain", "prune_policy"],
    outputs=["pruned_checkpoint_list"],
    decision_logic="""Come agente CheckpointPrunerAgent L6 team CheckpointSubEcosystem MemoryEcosystem: Gestisce pruning checkpoint vecchi preservando traceability - support - Riceve HarmonySignal ready da TeamSynchronyProtocol leader CheckpointSubLeader - Legge memoria via MemoryReaderAgent - Esegue task core con Playwright tool se necessario - Valida output con validator - Emite checkpoint condiviso broadcast ALL_TEAM - Handoff interno ack obbligatorio - Self-healing DetectionTeam se fail - Verifica harmony_status synchronized - Scrive memoria via MemoryWriterAgent + checkpoint via CheckpointManagerAgent - Logga decisione via DecisionLoggerAgent - Sincronia perfetta armonia intra-team InterTeamHarmonyProtocol esterno 8-step""",
    ...
    playwright_usage="real operational tool" if "Playwright" in "CheckpointPrunerAgent" or "Search" in "CheckpointPrunerAgent" or "Extractor" in "CheckpointPrunerAgent" else None,
    skill_usage=["SelfHealingSkill","MemoryReadWriteSkill","CheckpointManagementSkill"],
    level_name="L6"
)

class CheckpointPrunerAgent_SynchronizedWrapper:
...
CheckpointPrunerAgent_sync = CheckpointPrunerAgent_SynchronizedWrapper(CheckpointPrunerAgent)
def get_agent():
    return CheckpointPrunerAgent
def get_synchronized_wrapper():
    return CheckpointPrunerAgent_sync
print(f"Agent dedicated CheckpointPrunerAgent L6 Team CheckpointSubEcosystem - perfect synchrony harmony")
