...
"""
AGENTE DEDICATO - File singolo per ReviewSentimentAnalystAgent
...
ReviewSentimentAnalystAgent = Agent(
    ...
    role="Analizza sentiment review data da review sites - senior - L4 - Team ReviewAnalysisResearchTeam - Ecosistema ResearchEcosystem - Lavora in perfetta sincronia e armonia con altri agenti via TeamSynchronyProtocol HarmonySignal",
    hierarchy_level=4,
    team="ReviewAnalysisResearchTeam",
    ecosystem="ResearchEcosystem",
    sub_ecosystem="ReviewSub",
    inputs=["review_data_raw", "normalized_scores"],
    outputs=["sentiment_score", "sentiment_evidence"],
    decision_logic="""Come agente ReviewSentimentAnalystAgent L4 in team ReviewAnalysisResearchTeam ecosistema ResearchEcosystem sub ReviewSub:
    Analizza sentiment review data da review sites - senior
...
class ReviewSentimentAnalystAgent_SynchronizedWrapper:
...
ReviewSentimentAnalystAgent_sync = ReviewSentimentAnalystAgent_SynchronizedWrapper(ReviewSentimentAnalystAgent)

def get_agent():
    return ReviewSentimentAnalystAgent

def get_synchronized_wrapper():
    return ReviewSentimentAnalystAgent_sync

print(f"Agent file dedicated ReviewSentimentAnalystAgent L4 Team ReviewAnalysisResearchTeam Ecosistema ResearchEcosystem - perfect synchrony harmony initialized")
