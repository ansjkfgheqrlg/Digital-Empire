...
"""
AGENTE DEDICATO - File singolo per FeedbackCollectionLeader
...
FeedbackCollectionLeader = Agent(
    ...
    role="Leader FeedbackCollectionTeam outcome collection metrics pattern detection real improvement - L3 - Team FeedbackCollectionTeam - Ecosistema AutoImprovementEcosystem - Lavora in perfetta sincronia e armonia con altri agenti via TeamSynchronyProtocol HarmonySignal",
    hierarchy_level=3,
    team="FeedbackCollectionTeam",
    ecosystem="AutoImprovementEcosystem",
    sub_ecosystem="FeedbackSub",
    inputs=["cycle_completion_signals", "phase_outcomes"],
    outputs=["structured_feedback_data"],
    decision_logic="""Come agente FeedbackCollectionLeader L3 in team FeedbackCollectionTeam ecosistema AutoImprovementEcosystem sub FeedbackSub:
    Leader FeedbackCollectionTeam outcome collection metrics pattern detection real improvement
    ...
    """,
    connections={"reports_to": ["AutoImprovementEcosystemController"], "manages": [], "collaborates_with": ["MemoryManagerLeader","MemoryReaderAgent","CheckpointManagerAgent","TeamSynchronizer_FeedbackCollectionTeam"]},
...
class FeedbackCollectionLeader_SynchronizedWrapper:
...
FeedbackCollectionLeader_sync = FeedbackCollectionLeader_SynchronizedWrapper(FeedbackCollectionLeader)

def get_agent():
    return FeedbackCollectionLeader

def get_synchronized_wrapper():
    return FeedbackCollectionLeader_sync

print(f"Agent file dedicated FeedbackCollectionLeader L3 Team FeedbackCollectionTeam Ecosistema AutoImprovementEcosystem - perfect synchrony harmony initialized")
