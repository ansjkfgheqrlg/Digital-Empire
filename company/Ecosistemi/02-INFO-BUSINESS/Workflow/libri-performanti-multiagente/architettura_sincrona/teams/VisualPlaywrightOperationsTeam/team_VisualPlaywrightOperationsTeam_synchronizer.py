...
# Team definition - non è più un unico file per tutti i team, ma file dedicato per VisualPlaywrightOperationsTeam
# Ogni agente ha suo file dedicato sotto questa cartella e lavora in sincronia perfetta

TEAM_NAME = "VisualPlaywrightOperationsTeam"
ECOSYSTEM = "VisualEcosystem"
SUB_ECOSYSTEM = "PlaywrightSub"
LEADER = "VisualPlaywrightLeader"
MEMBERS = ['VisualPlaywrightLeader', 'VisualPlaywrightNavigatorAgent', 'VisualPlaywrightSaveAgent', 'VisualPlaywrightValidatorAgent', 'VisualPlaywrightCaptureAgent', 'VisualSaveMicroAgent']
...
TEAM_DEFINITION = Team(
    ...
    responsibilities=["Responsabilita principale VisualPlaywrightOperationsTeam in VisualEcosystem", "Gestisce flusso interno con TeamSynchronyProtocol", "Valida output con validator agent", "Crea checkpoint condiviso via CheckpointManagerAgent", "Gestisce self-healing sincronizzato intra-team", "Esegue handoff esterno 8-step sincronizzato via InterTeamHarmonyProtocol"],
    input_source="Handoff package da ecosistema precedente + memory VisualEcosystem + sync signals",
    output_target="Prossimo team/ecosistema + memory VisualEcosystem + checkpoint condiviso + sync ack",
    ...
    external_handoff_protocol={
        ...
        "steps": [
            "1. VisualPlaywrightLeader (LEADER L3) crea handoff package structured output decisions risks checkpoint ref + harmony_status synchronized",
