...
# Team definition - non è più un unico file per tutti i team, ma file dedicato per QualificationAnalysisTeam
# Ogni agente ha suo file dedicato sotto questa cartella e lavora in sincronia perfetta

TEAM_NAME = "QualificationAnalysisTeam"
ECOSYSTEM = "QualificationEcosystem"
SUB_ECOSYSTEM = "AnalysisSub"
LEADER = "QualificationLeader"
MEMBERS = ['QualificationLeader', 'ReproducibilityAnalystAgent', 'AbsurdityDetectorAgent', 'ProductionSpeedAnalystAgent', 'MarketAlignmentAnalystAgent', 'PlanQualityAuditorAgent', 'CompetitionAnalystAgent', 'SustainabilityAnalystAgent', 'BusinessFitAnalystAgent']
...
TEAM_DEFINITION = Team(
    ...
    responsibilities=["Responsabilita principale QualificationAnalysisTeam in QualificationEcosystem", "Gestisce flusso interno con TeamSynchronyProtocol", "Valida output con validator agent", "Crea checkpoint condiviso via CheckpointManagerAgent", "Gestisce self-healing sincronizzato intra-team", "Esegue handoff esterno 8-step sincronizzato via InterTeamHarmonyProtocol"],
    input_source="Handoff package da ecosistema precedente + memory QualificationEcosystem + sync signals",
    output_target="Prossimo team/ecosistema + memory QualificationEcosystem + checkpoint condiviso + sync ack",
    ...
    external_handoff_protocol={
        ...
        "steps": [
            "1. QualificationLeader (LEADER L3) crea handoff package structured output decisions risks checkpoint ref + harmony_status synchronized",
