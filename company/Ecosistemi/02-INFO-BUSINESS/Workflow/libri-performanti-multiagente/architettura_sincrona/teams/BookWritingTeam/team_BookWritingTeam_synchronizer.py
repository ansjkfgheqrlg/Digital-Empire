...
# Team definition - non è più un unico file per tutti i team, ma file dedicato per BookWritingTeam
# Ogni agente ha suo file dedicato sotto questa cartella e lavora in sincronia perfetta

TEAM_NAME = "BookWritingTeam"
ECOSYSTEM = "ProductionEcosystem"
SUB_ECOSYSTEM = "WritingSub"
LEADER = "BookWritingLeader"
MEMBERS = ['BookWritingLeader', 'ChapterWriterAgent', 'ConsistencyCheckerAgent', 'StyleEnforcerAgent', 'ContentQualityReviewerAgent', 'ChapterDependencyManagerAgent', 'WritingProgressTrackerAgent', 'WritingQualityCheckerAgent', 'ProductionLogWriterAgent']
...
TEAM_DEFINITION = Team(
    ...
    responsibilities=["Responsabilita principale BookWritingTeam in ProductionEcosystem", "Gestisce flusso interno con TeamSynchronyProtocol", "Valida output con validator agent", "Crea checkpoint condiviso via CheckpointManagerAgent", "Gestisce self-healing sincronizzato intra-team", "Esegue handoff esterno 8-step sincronizzato via InterTeamHarmonyProtocol"],
    input_source="Handoff package da ecosistema precedente + memory ProductionEcosystem + sync signals",
    output_target="Prossimo team/ecosistema + memory ProductionEcosystem + checkpoint condiviso + sync ack",
    ...
    external_handoff_protocol={
        ...
        "steps": [
            "1. BookWritingLeader (LEADER L3) crea handoff package structured output decisions risks checkpoint ref + harmony_status synchronized",
