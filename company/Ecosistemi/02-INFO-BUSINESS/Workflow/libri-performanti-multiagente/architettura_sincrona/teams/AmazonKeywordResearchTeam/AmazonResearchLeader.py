...
"""
AGENTE DEDICATO - File singolo per AmazonResearchLeader
Team: AmazonKeywordResearchTeam
Ecosistema: ResearchEcosystem / PlaywrightOps
...
AmazonResearchLeader = Agent(
    ...
    role="Leader team gestisce keyword generation search extraction validation BookNicheDecisionSkill - coordina flow interno KeywordGenerator -> Search via NavigatorMicroAgent -> Extractor via CaptureMicroAgent -> Validator - L3 - Team AmazonKeywordResearchTeam - Ecosistema ResearchEcosystem - Lavora in perfetta sincronia e armonia con altri agenti via TeamSynchronyProtocol HarmonySignal",
    hierarchy_level=3,
    team="AmazonKeywordResearchTeam",
    ...
    decision_logic="""Come agente AmazonResearchLeader L3 in team AmazonKeywordResearchTeam ecosistema ResearchEcosystem sub PlaywrightOps:
    Leader team gestisce keyword generation search extraction validation BookNicheDecisionSkill - coordina flow interno KeywordGenerator -> Search via NavigatorMicroAgent -> Extractor via CaptureMicroAgent -> Validator
    LOGICA DECISIONALE ESATTA:
    - Riceve HarmonySignal ready da TeamSynchronyProtocol leader AmazonResearchLeader
    ...
    - Emite HarmonySignal checkpoint con checkpoint_id condiviso via CheckpointManagerAgent broadcast a ALL_TEAM team AmazonKeywordResearchTeam
    ...
    SINCRONIA E ARMONIA: Lavora in perfetta sincronia e armonia con altri agenti del team AmazonKeywordResearchTeam - ogni agente invia ready signal, checkpoint condiviso, handoff con ack, validazione, harmony_status synchronized - InterTeamHarmonyProtocol per handoff esterno 8-step sincronizzato via Memory broker
    """,
    connections={"reports_to": ["ResearchEcosystemController"], "manages": [], "collaborates_with": ["MemoryManagerLeader","MemoryReaderAgent","CheckpointManagerAgent","TeamSynchronizer_AmazonKeywordResearchTeam"]},
    ...
    self_healing_behavior={"detection_triggers": ["missing output","incoherent output","blocked process","failed validation","empty result from research","no-go without alternative","memory write failure","Playwright failure"], "action": "retry con adjusted params da important_notes LearningLog rollback a ultimo checkpoint condiviso valido via CheckpointManagerAgent team AmazonKeywordResearchTeam sincronizzato con altri membri team - se 3 fallimenti escalate a leader AmazonResearchLeader poi controller ResearchEcosystemController poi Supreme", "max_retries": 3, "checkpoint_before": True, "memory_updated": True, "flow_continued": True, "harmony_preserved": True},
...
class AmazonResearchLeader_SynchronizedWrapper:
    ...
    def emit_ready(self):
        signal = HarmonySignal(
            signal_id=f"AmazonKeywordResearchTeam_{self.agent.name}_ready_{int(time.time()*1000)}",
            sender_agent=self.agent.name,
            receiver_agent="AmazonResearchLeader",
            team="AmazonKeywordResearchTeam",
            ecosystem="ResearchEcosystem",
            signal_type="ready",
    ...
    def sync_checkpoint(self, checkpoint_id: str):
        ...
        signal = HarmonySignal(
            signal_id=f"AmazonKeywordResearchTeam_{self.agent.name}_checkpoint_{checkpoint_id}",
            sender_agent=self.agent.name,
            receiver_agent="ALL_TEAM_AmazonKeywordResearchTeam",
            team="AmazonKeywordResearchTeam",
            ecosystem="ResearchEcosystem",
            signal_type="checkpoint",
            payload={"checkpoint_id": checkpoint_id, "shared": True, "agent": self.agent.name, "team": "AmazonKeywordResearchTeam"},
            timestamp=time.strftime("%Y-%m-%dT%H:%M:%SZ"),
            requires_ack=False
        )
        self.harmony_signals.append(signal)
    ...
    def communicate(self, target_agent: str, payload: dict, signal_type: str = "handoff"):
        signal = HarmonySignal(
            signal_id=f"AmazonKeywordResearchTeam_{self.agent.name}_to_{target_agent}_{signal_type}",
            sender_agent=self.agent.name,
    ...
    def validate_harmony(self):
        return {"agent": self.agent.name, "team": "AmazonKeywordResearchTeam", "status": self.status, "checkpoint_shared": self.checkpoint_shared, "signals_count": len(self.harmony_signals), "harmony": "synchronized"}

    def self_heal_synchronized(self, error_type: str, failed_op: str):
        # Self-healing in armonia con team - rollback sincronizzato
        return {
            "phase": "AmazonKeywordResearchTeam",
            ...
            "team_synchrony": "all members rollback to shared checkpoint AmazonKeywordResearchTeam"
        }

# Istanza wrapper sincronizzata
AmazonResearchLeader_sync = AmazonResearchLeader_SynchronizedWrapper(AmazonResearchLeader)

def get_agent():
    return AmazonResearchLeader

def get_synchronized_wrapper():
    return AmazonResearchLeader_sync

print(f"Agent file dedicated AmazonResearchLeader L3 Team AmazonKeywordResearchTeam Ecosistema ResearchEcosystem - perfect synchrony harmony initialized")
