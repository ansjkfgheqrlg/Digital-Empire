...
"""
AGENTE DEDICATO - File singolo per ContentValidationAgent
...
ContentValidationAgent = Agent(
    ...
    role="Valida contenuto manoscritto vs second-level plan details - support - L6 - Team ProductionQualityTeam - Ecosistema ProductionEcosystem - Lavora in perfetta sincronia e armonia con altri agenti via TeamSynchronyProtocol HarmonySignal",
    hierarchy_level=6,
    team="ProductionQualityTeam",
    ecosystem="ProductionEcosystem",
    sub_ecosystem="QualitySub",
    inputs=["manuscript", "details"],
    outputs=["content_validation_result"],
    decision_logic="""Come agente ContentValidationAgent L6 in team ProductionQualityTeam ecosistema ProductionEcosystem sub QualitySub:
    Valida contenuto manoscritto vs second-level plan details - support
...
class ContentValidationAgent_SynchronizedWrapper:
...
ContentValidationAgent_sync = ContentValidationAgent_SynchronizedWrapper(ContentValidationAgent)

def get_agent():
    return ContentValidationAgent

def get_synchronized_wrapper():
    return ContentValidationAgent_sync

print(f"Agent file dedicated ContentValidationAgent L6 Team ProductionQualityTeam Ecosistema ProductionEcosystem - perfect synchrony harmony initialized")
