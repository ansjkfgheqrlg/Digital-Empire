...
"""
AGENTE DEDICATO - File singolo per QualityMetricsCalculatorAgent
...
QualityMetricsCalculatorAgent = Agent(
    ...
    role="Calcola metriche qualita produzione - senior - L4 - Team ProductionQualityTeam - Ecosistema ProductionEcosystem - Lavora in perfetta sincronia e armonia con altri agenti via TeamSynchronyProtocol HarmonySignal",
    hierarchy_level=4,
    team="ProductionQualityTeam",
    ecosystem="ProductionEcosystem",
    sub_ecosystem="QualitySub",
    inputs=["manuscript", "production_log"],
    outputs=["quality_metrics"],
    decision_logic="""Come agente QualityMetricsCalculatorAgent L4 in team ProductionQualityTeam ecosistema ProductionEcosystem sub QualitySub:
    Calcola metriche qualita produzione - senior
...
class QualityMetricsCalculatorAgent_SynchronizedWrapper:
...
QualityMetricsCalculatorAgent_sync = QualityMetricsCalculatorAgent_SynchronizedWrapper(QualityMetricsCalculatorAgent)

def get_agent():
    return QualityMetricsCalculatorAgent

def get_synchronized_wrapper():
    return QualityMetricsCalculatorAgent_sync

print(f"Agent file dedicated QualityMetricsCalculatorAgent L4 Team ProductionQualityTeam Ecosistema ProductionEcosystem - perfect synchrony harmony initialized")
