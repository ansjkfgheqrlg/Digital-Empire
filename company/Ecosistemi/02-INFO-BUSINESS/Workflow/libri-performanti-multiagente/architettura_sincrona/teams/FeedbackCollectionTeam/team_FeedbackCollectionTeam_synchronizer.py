...
# Team definition - non è più un unico file per tutti i team, ma file dedicato per FeedbackCollectionTeam
# Ogni agente ha suo file dedicato sotto questa cartella e lavora in sincronia perfetta

TEAM_NAME = "FeedbackCollectionTeam"
ECOSYSTEM = "AutoImprovementEcosystem"
SUB_ECOSYSTEM = "FeedbackSub"
LEADER = "FeedbackCollectionLeader"
MEMBERS = ['FeedbackCollectionLeader', 'OutcomeCollectorAgent', 'PerformanceMetricsAgent', 'PatternDetectorAgent', 'CycleOutcomeAnalyzerAgent', 'MetricCaptureMicroAgent', 'PatternCheckMicroAgent']
...
TEAM_DEFINITION = Team(
    ...
    responsibilities=["Responsabilita principale FeedbackCollectionTeam in AutoImprovementEcosystem", "Gestisce flusso interno con TeamSynchronyProtocol", "Valida output con validator agent", "Crea checkpoint condiviso via CheckpointManagerAgent", "Gestisce self-healing sincronizzato intra-team", "Esegue handoff esterno 8-step sincronizzato via InterTeamHarmonyProtocol"],
    input_source="Handoff package da ecosistema precedente + memory AutoImprovementEcosystem + sync signals",
    output_target="Prossimo team/ecosistema + memory AutoImprovementEcosystem + checkpoint condiviso + sync ack",
    ...
    external_handoff_protocol={
        ...
        "steps": [
            "1. FeedbackCollectionLeader (LEADER L3) crea handoff package structured output decisions risks checkpoint ref + harmony_status synchronized",
