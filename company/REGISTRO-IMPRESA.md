...
| **YOUTUBE-AUTOMATION-FACTORY** (Apex7Orchestrator: scouting→selezione→script→produzione→pubblicazione→audit, 6 fasi) — tutte le fasi su dati reali (niche-scout Gemini, fetch pubblico YouTube, SEO/critic score reali, manifest `published_videos.json` per l'audit, dashboard con esito PASS/FAIL vero per run), critic persiste su `11-APEX-7-CORE` (`domain="youtube"`) | Gael | test_youtube_apex7.py (11/11) | build TASK-YT-001..005 (2026-07-28) | Art.8 · ADR-010 |
