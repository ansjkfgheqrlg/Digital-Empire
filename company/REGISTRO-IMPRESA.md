...
| **11-APEX-7-CORE / orchestration layer** (`orchestration/`: contracts·dag·bus·gates·healing·evolution·pipeline) — 7 quality gate BLOCCANTI attorno al `RuFLOOrchestrator`, catena di stato Merkle, DAG con circuit breaker, `InstrumentedEventBus` con DLQ. **Wrap puro: `ruflo_core.py` mai modificato** (`git diff` vuoto). Innestato dall'audit dello zip `apex7_orchestrator` (scartati: clone claude-flow, moduli finanziari, swarm simulato). Difetti del motore trovati e mandati in BACKLOG, non corretti di straforo: B-013, B-014. **Nessun consumatore reale ancora agganciato** — gira solo sui propri test | 11-APEX-7-CORE | `test_orchestration.py` (46/46, meta' dei quali verificano che i gate RIFIUTINO) + 5-bis | audit zip Max 2026-08-13 (CP-20260813-001) | Art.2 (nessun "100% PASS" dichiarato senza prova) · ADR-003 (wrap) · ADR-010 (motore canonico) · ADR-005 (B-013/014/015) |
