...
| CONOSCENZA-EMPIRE (`.claude/agents/conoscenza-empire.md`) | LX (accanto a Mandato e MAXIMILIAN) | EMPERATOR | direttiva Max 2026-09-02 | Art.2 (fonte o niente) + P03/P11 (mai riassunti) + P12 (tracciabilita') |
...
| **YOUTUBE-AUTOMATION-FACTORY** (fabbrica video APEX-7 a 6 fasi F1-F6) — **F1-F6 tutte REALI** sul motore condiviso `11-APEX-7-CORE` (`domain="youtube"`): F4 spec Fliki multi-scena dallo script reale, F5 metadati/titolo/tag reali + SEO≥70, F6 audit onesto (nessuna metrica finta senza video pubblicato), dashboard `write_dashboard()` riflette PASS/FAIL VERO per fase. TASK-YT-001..005 chiuse; restano solo P2 (TASK-YT-006 ritiro APEX-7 duplicato in Stream-S7, TASK-YT-007) | 03-CONTENT-FACTORY | `test_youtube_apex7.py` 11/11 + `validate_schemas.py` + `seo_score.py` | build Gael+Claude (CP-20260728-007, CP-20260729-003/004/005/006) | Art.2 (zero dati finti) · ADR-010 · ADR-003 |
...
| **STREAM-S7-BOT** (Solana NFT/Memecoin Bot - R&D Speculativo) — APEX-7 Level 2 proprio (event_bus/memory_interface/quality_gates/gate_agent/meta_agent/orchestrator, 6 gate a rubrica/33 criteri, DLQ+replay, self-judging L6→L7); valutata fusione con `11-APEX-7-CORE` (TASK-YT-006, 2026-07-28) — **NON migrata**: piu' matura del motore condiviso su questi assi, migrarla sarebbe un downgrade su un bot con trade reali su mainnet Solana (ADR-003 wrap-non-riscrittura) | 00-CORE | 5-bis | build Gemini (GEM-07) | R&D Speculativa (0€ Revenue) · ADR-010 |
| **11-APEX-7-CORE / motore condiviso** (`memory_system.py` multi-tenant `domain=...`, `ruflo_core.py`, `BaseAgent`) — Coordination Fabric ufficiale del backbone Ruflo. Fase 1 pilota: **YouTube in uso reale** (`domain="youtube"`, critic persiste su `log_critique()`, TASK-YT-001..005 chiuse 2026-07-28) + **Stream-S7-Bot NON migrato** (motivazione in CP-20260728-012, resta con la sua implementazione propria); gia' in uso da carousel-machine/skill-forge/cold-outreach (`domain="default"`) | 11-APEX-7-CORE | test_multi_tenant.py (4/4) + 5-bis | ADR-009 (promozione ecosistema) + ADR-010 (fusione Ruflo) | Art.8 · ADR-003 (wrap, non riscrittura) · ADR-010 |
| **11-APEX-7-CORE / orchestration layer** (`orchestration/`: contracts·dag·bus·gates·healing·evolution·pipeline) — 7 quality gate BLOCCANTI attorno al `RuFLOOrchestrator`, catena di stato Merkle, DAG con circuit breaker, `InstrumentedEventBus` con DLQ. Innestato dall'audit dello zip `apex7_orchestrator` (scartati: clone claude-flow, moduli finanziari, swarm simulato). **Consumatore agganciato: i 3 stream di `arena_generator.py`** (skill-forge, carousel-machine, cold-outreach) girano certificati, con `<nome>.gate.json` accanto all'output; `strict=True` disponibile e non ancora acceso (ADR-003) | 11-APEX-7-CORE | `test_orchestration.py` (49/49, meta' dei quali verificano che i gate RIFIUTINO) + 5-bis | audit zip Max 2026-08-13 (CP-20260813-001/002) | Art.2 (nessun "100% PASS" dichiarato senza prova) · ADR-003 (wrap, strict off finche' non validato) · ADR-010 (motore canonico) · ADR-011 |
| **11-APEX-7-CORE / calc layer** (`calc/`: core·probabilita·denaro·engine) — **il calcolatore dell'Impero**: 16 moduli puri in 4 categorie (base, probabilita', denaro, guadagni incluso **royalty KDP**). Registro estendibile via decoratore `@registra`. Interfaccia `esegui(dict)->dict` + `catalogo()`, solo JSON: **e' il punto di innesto del ponte verso gli altri orchestration layer** (ponte NON ancora costruito). `esegui_grafo` incatena calcoli sul DAG, `esegui_certificato` li fa passare dai 7 gate. Corretti i 2 errori dello zip che ribaltavano la conclusione (tasse su plusvalenza nominale, confronto risk-free omogeneo) | 11-APEX-7-CORE | `test_calc.py` (39/39, con test REGRESSIONE sui 3 difetti dello zip) + 5-bis | richiesta Max 2026-08-14 (CP-20260814-003) | Art.2 (nessun numero senza fonte: i default escono nelle `assunzioni`) · ADR-010 (motore canonico) · ADR-011 |
| `empire/intelligence/apex7/` (**quinta linea APEX-7**, ~650 righe: agents·backends·memory·orchestrator·quality·ruflo_adapter) — era **orfana**: non censita da ADR-010, che ne contava 4. Regolarizzata da ADR-011 come **deprecata-non-cancellata**. E' la piu' onesta del repo: `backends.py` separa esplicitamente `LocalMockBackend` e `LLMBackend` reale, `ruflo_adapter.py` alza `NotImplementedError` dichiarando cosa serve per collegare RuFLO davvero. I suoi 2 pezzi mancanti al canone (seam backend LLM, contratto RuFLO) **da promuovere** in `11-APEX-7-CORE` — B-015 | MAX | Claude (gate) · 5-bis | import LMarena zip 2026-07-22, censita 2026-08-13 (CP-20260813-002) | ADR-011 (censimento) · ADR-003 (non cancellata) · ADR-005 (B-015) — **blocca la Fase 2 di ADR-010** |
