...
| B-056 | La fabbrica YouTube non ha nessun controllo dei fatti | **Aggiornato 2026-09-10 — l'organo esiste:** `03-AGENTI-E-RUOLI/regolatori/regolatore-fatti.md` (agganciato nel `blocca` di `script-writer`/`title-writer`/`capo-copy`, referenziato in `script-writer.md` §8). **Resta aperto solo il braccio in codice:** `02-AUTOMAZIONI-E-SCRIPTS/verifica_fatti.py` non esiste ancora (verificato 2026-09-10, zero occorrenze) — finché non nasce, il regolatore confronta i fatti a lettura manuale invece che con un'estrazione automatica di entità, gemella di `verifica_originalita`. Quello script resta binario B, aspetta il prossimo gate di categoria. Cronologia: cercato il 2026-09-05 su tutti i `.md`/`.py` di `YOUTUBE-AUTOMATION-FACTORY` *verifica dei fatti*/*fact-check*/*controllo dei fatti* → zero risultati; `regolatori.py:153` misura solo la somiglianza, mai la veridicità | script `verifica_fatti.py`: prima del prossimo ciclo di produzione con sorgenti di cronaca | ⬜ |
...
- **B-067 — NUOVA (2026-09-11).** `engine/piano.py` (libri KDP) scrive il piano settimanale solo in
  ...
  CP-20260911-F4FV, nove giorni dopo). Da fare: `/piano-libri` lancia lo script da solo a fine piano.
...
- **B-091** (2026-09-15, agency-landing) — Il rail dei fatti in fondo all'hero («300 messaggi al giorno, zero a mano», `rail-fatti.tsx`) nomina un numero Outreach PRIMA della sezione «Tre sistemi»: candidato alla legge F6 (copy generico fino ai Tre sistemi). Non nell'ordine di Max del 15/09 sera (dossier 42, P3): decide Max se generalizzarlo.
