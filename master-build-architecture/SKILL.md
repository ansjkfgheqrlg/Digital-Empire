...
## Governance Override — Workflow, Memory, References and Agents

The files below are canonical operational controls and take precedence over outdated status claims in this kernel:

- `governance/WORKFLOW-FIRST.md`: **WF-0 is mandatory**. Every application begins with a user/business workflow; no SRS, UI, API, schema, or implementation is approved before the workflow gate.
- `governance/MEMORY-PROTOCOL.md`: repository-backed memory and safe evidence handling.
- `governance/GRAPHICS-UX-ACCESSIBILITY.md`, `LINKS-AND-INTEGRATIONS.md`, and `DELIVERY-WORKFLOW.md`: reusable product and delivery rules.
- `governance/REFERENCE-LIBRARY.md`: 60+ primary-source references; REF records the exact source consulted for every time-sensitive or external decision.
- `agents/OPERATING-REGISTRY.md`: 55-role operating registry, activation triggers and independence rules. Baseline: ORCH, PM, WFL, DISC, MEM, QA, DOC, SUP.

SUP must block a phase transition when WF-0, traceability, memory evidence, or required specialist review is missing. Report only checks actually executed.
